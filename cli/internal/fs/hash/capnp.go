// Package hash contains the capnp schema and hashing functions for the titan cache
//
...
//
// in crates/titanrepo-lib/src/hash or run `make titan-capnp` in the `cli` directory.
package hash
...
import (
	...
	titan_capnp "github.com/khulnasoft/titan/cli/internal/fs/hash/capnp"
	...
	"github.com/khulnasoft/titan/cli/internal/xxhash"
)
...
type TaskHashable struct {
	...
	PackageDir   titanpath.AnchoredUnixPath
	Task         string
	Outputs      TaskOutputs
	PassThruArgs []string
...
func HashTaskHashable(task *TaskHashable) (string, error) {
	...
	taskMsg, err := titan_capnp.NewRootTaskHashable(seg)
	if err != nil {
		return "", err
	}
	...
	{
		var envMode titan_capnp.TaskHashable_EnvMode
		switch task.EnvMode {
		case util.Infer:
			panic("task inferred status should have already been resolved")
		case util.Loose:
			envMode = titan_capnp.TaskHashable_EnvMode_loose
		case util.Strict:
			envMode = titan_capnp.TaskHashable_EnvMode_strict
		}
...
func HashGlobalHashable(global *GlobalHashable) (string, error) {
	...
	globalMsg, err := titan_capnp.NewRootGlobalHashable(seg)
	if err != nil {
		return "", err
	}
	...
	{
		var envMode titan_capnp.GlobalHashable_EnvMode
		switch global.EnvMode {
		case util.Infer:
			envMode = titan_capnp.GlobalHashable_EnvMode_infer
		case util.Loose:
			envMode = titan_capnp.GlobalHashable_EnvMode_loose
		case util.Strict:
			envMode = titan_capnp.GlobalHashable_EnvMode_strict
		}
...
func HashLockfilePackages(packages []lockfile.Package) (string, error) {
	...
	globalMsg, err := titan_capnp.NewRootLockFilePackages(seg)
	if err != nil {
		return "", err
	}
...
func HashFileHashes(fileHashes map[titanpath.AnchoredUnixPath]string) (string, error) {
	arena := capnp.SingleSegment(nil)
	...
	globalMsg, err := titan_capnp.NewRootFileHashes(seg)
	if err != nil {
		return "", err
	}
...
func HashMessage(msg *capnp.Message) (string, error) {
	...
	// _ = titanpath.AbsoluteSystemPath(".titan/go-hash").WriteFile(bytes, 0644)
...
func assignSortedHashMap(packages map[titanpath.AnchoredUnixPath]string, setEntry func(int, string, string) error) error {
	keys := make([]titanpath.AnchoredUnixPath, len(packages))
...
func assignAnchoredUnixArray(paths titanpath.AnchoredUnixPathArray, fn func(capnp.TextList) error, seg *capnp.Segment) error {

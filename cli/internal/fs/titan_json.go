...
// `experimentalSpaceId` field in a titan.json
...
// pristineTitanJSON is used when marshaling a TitanJSON object into a json string
...
type pristineTitanJSON struct {
	GlobalDependencies   []string                        `json:"globalDependencies,omitempty"`
	GlobalEnv            []string                        `json:"globalEnv,omitempty"`
	GlobalPassThroughEnv []string                        `json:"globalPassThroughEnv"`
	GlobalDotEnv         titanpath.AnchoredUnixPathArray `json:"globalDotEnv"`
...
// TitanJSON represents a titan.json configuration file
...
type rawTaskWithDefaults struct {
	...
	DotEnv         titanpath.AnchoredUnixPathArray `json:"dotEnv"`
}
...
type taskDefinitionHashable struct {
	...
	DotEnv                  titanpath.AnchoredUnixPathArray
}
...
func LoadTitanConfig(dir titanpath.AbsoluteSystemPath, rootPackageJSON *PackageJSON, includeSynthesizedFromRootPackageJSON bool) (*TitanJSON, error) {
	// If the root package.json stil has a `titan` key, log a warning and remove it.
	if rootPackageJSON.LegacyTitanConfig != nil {
		log.Printf("[WARNING] \"titan\" in package.json is no longer supported. Migrate to %s by running \"npx @titan/codemod create-titan-config\"\n", configFile)
		rootPackageJSON.LegacyTitanConfig = nil
	}

	var titanJSON *TitanJSON
	titanFromFiles, err := readTitanConfig(dir.UntypedJoin(configFile))
	...
	for scriptName := range rootPackageJSON.Scripts {
		if !titanJSON.Pipeline.HasTask(scriptName) {
			...
			titanJSON.Pipeline[taskName] = BookkeepingTaskDefinition{
...
// TitanJSONValidation is the signature for a validation function passed to Validate()
type TitanJSONValidation func(*TitanJSON) []error

// Validate calls an array of validation functions on the TitanJSON struct.
// The validations can be customized by the caller.
func (tj *TitanJSON) Validate(validations []TitanJSONValidation) []error {
	allErrors := []error{}
...
// readTitanConfig reads titan.json from a provided path
func readTitanConfig(titanJSONPath titanpath.AbsoluteSystemPath) (*TitanJSON, error) {
	...
	// If there's no titan.json, return an error.
...
// see whether a field was actually in the underlying titan.json
...
func MergeTaskDefinitions(taskDefinitions []BookkeepingTaskDefinition) (*TaskDefinition, error) {
	...
	// Set the default, because the 0-value will be false, and if no titan.jsons had
...
// titan.json into a TaskDefinition struct
func (btd *BookkeepingTaskDefinition) UnmarshalJSON(data []byte) error {
	...
	if task.DotEnv != nil {
		...
		btd.TaskDefinition.DotEnv = make(titanpath.AnchoredUnixPathArray, 0, len(task.DotEnv))

		// Port the raw dotEnv values in.
		for _, dotEnvPath := range task.DotEnv {
			typeCheckedPath, err := titanpath.CheckedToAnchoredUnixPath(dotEnvPath)
			if err != nil {
				return err
			}
...
// UnmarshalJSON deserializes the contents of titan.json into a TitanJSON struct
func (tj *TitanJSON) UnmarshalJSON(data []byte) error {
	raw := &rawTitanJSON{}
	...
	if raw.GlobalDotEnv != nil {
		tj.GlobalDotEnv = make(titanpath.AnchoredUnixPathArray, 0, len(raw.GlobalDotEnv))
...
// UnmarshalJSON deserializes the remoteCache key from titan.json into an in-memory struct
...
// MarshalJSON converts a TitanJSON into the equivalent json object in bytes
// note: we go via rawTitanJSON so that the output format is correct.
// This is used by `titan prune` to generate a pruned titan.json
// and also by --summarize & --dry=json to serialize the known config
// into something we can print to screen
func (tj *TitanJSON) MarshalJSON() ([]byte, error) {
	raw := pristineTitanJSON{}

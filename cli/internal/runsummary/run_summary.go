// Package runsummary implements structs that report on a `titan run` and `titan run --dry`
package runsummary
...
import (
	...
	"github.com/khulnasoft/titan/cli/internal/spinner"
...
// E.g. if `titan run build --dry` is run, and package-a doesn't define a `build` script in package.json,
...
type Meta struct {
	...
	repoRoot           titanpath.AbsoluteSystemPath // used to write run summary
	repoPath           titanpath.RelativeSystemPath
	singlePackage      bool
	shouldSave         bool
...
// RunSummary contains a summary of what happens in the `titan run` command and why.
type RunSummary struct {
	ID                 ksuid.KSUID        `json:"id"`
	Version            string             `json:"version"`
	TitanVersion       string             `json:"titanVersion"`
...
func (rsm *Meta) getPath() titanpath.AbsoluteSystemPath {
	filename := fmt.Sprintf("%s.json", rsm.RunSummary.ID)
	return rsm.repoRoot.UntypedJoin(filepath.Join(".titan", "runs"), filename)
}

// Close wraps up the RunSummary at the end of a `titan run`.
...
// closeDryRun wraps up the Run Summary at the end of `titan run --dry`.
...
func getUser(envVars env.EnvironmentVariableMap, dir titanpath.AbsoluteSystemPath) string {
	var username string

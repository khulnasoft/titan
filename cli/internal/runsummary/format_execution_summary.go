...
import (
	...
	internalUI "github.com/khulnasoft/titan/cli/internal/ui"
...
func (rsm *Meta) printExecutionSummary(ui cli.Ui) {
	...
	if cached == attempted && attempted > 0 {
		...
		if terminalProgram == "Apple_Terminal" {
			fallbackTitanColor := color.New(color.FgHiMagenta, color.Bold).SprintFunc()
			maybeFullTitan = fallbackTitanColor(">>> FULL TITAN")
		} else {
			maybeFullTitan = internalUI.Rainbow(">>> FULL TITAN")
		}
	}
	...
	lineData := []summaryLine{
		...
		{header: "Time", trailer: util.Sprintf("%v${RESET} %v", duration, maybeFullTitan)},
	}

...
func newSpacesClient(spaceID string, api spacesAPIClient) *spacesClient {
	...
	if !c.api.IsLinked() {
		c.errors = append(c.errors, fmt.Errorf("Error: experimentalSpaceId is enabled, but repo is not linked to API. Run `titan link` or `titan login` first"))
		return c
	}
...
type spacesRunPayload struct {
	...
	Type           string              `json:"type,omitempty"`           // hardcoded to "TITAN"
	ExitCode       *int                `json:"exitCode,omitempty"`       // exit code for the full run
	Command        string              `json:"command,omitempty"`        // the thing that kicked off the titan run
	...
	Client         spacesClientSummary `json:"client"`                   // Details about the titan client
...
func newSpacesRunCreatePayload(rsm *Meta) *spacesRunPayload {
	...
	return &spacesRunPayload{
		...
		Client: spacesClientSummary{
			ID:      "titan",
			Name:    "Titan",
			Version: rsm.RunSummary.TitanVersion,
		},
	}
}

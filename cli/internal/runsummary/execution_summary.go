...
import (
	...
	"github.com/khulnasoft/titan/cli/internal/chrometracing"
...
// TaskExecutionSummary contains data about the state of a single task in a titan run.
...
// executionSummary is the state of the entire `titan run`. Individual task state in `Tasks` field
type executionSummary struct {
	...
	command   string                       // a synthesized titan command to produce this invocation
	repoPath  titanpath.RelativeSystemPath // the (possibly empty) path from the titanrepo root to where the command was run
...
// newExecutionSummary creates a executionSummary instance to track events in a `titan run`.`
func newExecutionSummary(command string, repoPath titanpath.RelativeSystemPath, start time.Time, tracingProfile string) *executionSummary {
	if tracingProfile != "" {
...
// writeChromeTracing writes to a profile name if the `--profile` flag was passed to titan run
func writeChrometracing(filename string, terminal cli.Ui) error {
	...
	name := fmt.Sprintf("titan-%s.trace", time.Now().Format(time.RFC3339))
	if filename != "" {
		name = filename
	}
	...
	// so we have to account for that before converting to titanpath.AbsoluteSystemPath

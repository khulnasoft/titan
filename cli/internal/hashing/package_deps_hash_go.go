...
import (
	...
	"github.com/khulnasoft/titan/cli/internal/doublestar"
	"github.com/khulnasoft/titan/cli/internal/encoding/gitoutput"
	"github.com/khulnasoft/titan/cli/internal/fs"
	"github.com/khulnasoft/titan/cli/internal/globby"
...
// to `git hash-object`.
func gitHashObject(anchor titanpath.AbsoluteSystemPath, filesToHash []titanpath.AnchoredSystemPath) (map[titanpath.AnchoredUnixPath]string, error) {
	fileCount := len(filesToHash)
	output := make(map[titanpath.AnchoredUnixPath]string, fileCount)
	...
	if fileCount > 0 {
		...
		// The API of this method specifies that we return a `map[titanpath.AnchoredUnixPath]string`.
...
func manuallyHashFiles(rootPath titanpath.AbsoluteSystemPath, files []titanpath.AnchoredSystemPath, allowMissing bool) (map[titanpath.AnchoredUnixPath]string, error) {
	hashObject := make(map[titanpath.AnchoredUnixPath]string, len(files))
...
func getPackageFileHashesFromGitIndex(rootPath titanpath.AbsoluteSystemPath, packagePath titanpath.AnchoredSystemPath) (map[titanpath.AnchoredUnixPath]string, error) {
	var result map[titanpath.AnchoredUnixPath]string
	...
	var filesToHash []titanpath.AnchoredSystemPath
...
func gitStatus(rootPath titanpath.AbsoluteSystemPath) (map[titanpath.AnchoredUnixPath]statusCode, error) {
	cmd := exec.Command(
		"git",               // Using `git` from $PATH,
	...
	output := make(map[titanpath.AnchoredUnixPath]statusCode, len(entries))
	convertedRootPath := titanpath.AbsoluteSystemPathFromUpstream(rootPath.ToString())
	...
	for _, entry := range entries {
		...
		pathFromStatus := titanpath.AnchoredUnixPathFromUpstream(statusEntry.GetField(gitoutput.Path))
		var outputPath titanpath.AnchoredUnixPath
...
func safeCompileIgnoreFile(filepath titanpath.AbsoluteSystemPath) (*gitignore.GitIgnore, error) {
...
func getPackageFileHashesFromProcessingGitIgnore(rootPath titanpath.AbsoluteSystemPath, packagePath titanpath.AnchoredSystemPath, inputs []string) (map[titanpath.AnchoredUnixPath]string, error) {
	result := make(map[titanpath.AnchoredUnixPath]string)
	absolutePackagePath := packagePath.RestoreAnchor(rootPath)
	...
	err = fs.Walk(absolutePackagePath.ToStringDuringMigration(), func(name string, isDir bool) error {
		convertedName := titanpath.AbsoluteSystemPathFromUpstream(name)
...
func gitLsTree(rootPath titanpath.AbsoluteSystemPath) (map[titanpath.AnchoredUnixPath]string, error) {
	cmd := exec.Command(
		"git",     // Using `git` from $PATH,
	...
	output := make(map[titanpath.AnchoredUnixPath]string, len(entries))

	for _, entry := range entries {
		lsTreeEntry := gitoutput.LsTreeEntry(entry)
		output[titanpath.AnchoredUnixPathFromUpstream(lsTreeEntry.GetField(gitoutput.Path))] = lsTreeEntry[2]
	}
...
func getPackageFileHashesFromInputs(rootPath titanpath.AbsoluteSystemPath, packagePath titanpath.AnchoredSystemPath, inputs []string) (map[titanpath.AnchoredUnixPath]string, error) {
	absolutePackagePath := packagePath.RestoreAnchor(rootPath)
	...
	// Add in package.json and titan.json to input patterns. Both file paths are relative to pkgPath
	//
	// - package.json is an input because if the `scripts` in
	// 		the package.json change (i.e. the tasks that titan executes), we want
	// 		a cache miss, since any existing cache could be invalid.
	// - titan.json because it's the definition of the tasks themselves. The root titan.json
	...
	calculatedInputs = append(calculatedInputs, "titan.json")
	...
	filesToHash := make([]titanpath.AnchoredSystemPath, len(absoluteFilesToHash))
	for i, rawPath := range absoluteFilesToHash {
		...
		filesToHash[i] = titanpath.AnchoredSystemPathFromUpstream(relativePathString)
	}
...
func getTraversePath(rootPath titanpath.AbsoluteSystemPath) (titanpath.RelativeUnixPath, error) {
	...
	return titanpath.RelativeUnixPathFromUpstream(trimmedTraversePath), nil
}

// Don't shell out if we already know where you are in the repository.
// `memoize` is a good candidate for generics.
func memoizeGetTraversePath() func(titanpath.AbsoluteSystemPath) (titanpath.RelativeUnixPath, error) {
	cacheMutex := &sync.RWMutex{}
	cachedResult := map[titanpath.AbsoluteSystemPath]titanpath.RelativeUnixPath{}
	cachedError := map[titanpath.AbsoluteSystemPath]error{}

	return func(rootPath titanpath.AbsoluteSystemPath) (titanpath.RelativeUnixPath, error) {
		cacheMutex.RLock()

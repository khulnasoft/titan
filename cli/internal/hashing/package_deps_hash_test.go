...
func getFixture(id int) titanpath.AbsoluteSystemPath {
	cwd, _ := os.Getwd()
	root := titanpath.AbsoluteSystemPath(filepath.VolumeName(cwd) + string(os.PathSeparator))
	checking := titanpath.AbsoluteSystemPath(cwd)
	...
	for checking != root {
		...
		if !errors.Is(err, os.ErrNotExist) {
			...
			for _, file := range files {
				fileName := titanpath.RelativeSystemPath(file.Name())
				if strings.Index(fileName.ToString(), fmt.Sprintf("%02d-", id)) == 0 {
					return titanpath.AbsoluteSystemPath(fixtureDirectory.Join(fileName))
				}
			}
		}
...
func TestSpecialCharacters(t *testing.T) {
	...
	newlinePath := titanpath.AnchoredUnixPath("new\nline").ToSystemPath()
	quotePath := titanpath.AnchoredUnixPath("\"quote\"").ToSystemPath()
	...
	tests := []struct {
		name        string
		rootPath    titanpath.AbsoluteSystemPath
		filesToHash []titanpath.AnchoredSystemPath
		want        map[titanpath.AnchoredUnixPath]string
		wantErr     bool
	}{
		{
			name:     "Quotes",
...
func getTraversePathInTests(rootPath titanpath.AbsoluteSystemPath) (titanpath.RelativeUnixPath, error) {
...
func Test_gitHashObject(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			filesToHash: []titanpath.AnchoredSystemPath{
				titanpath.AnchoredUnixPath("../root.json").ToSystemPath(),
				titanpath.AnchoredUnixPath("child.json").ToSystemPath(),
				titanpath.AnchoredUnixPath("grandchild/grandchild.json").ToSystemPath(),
			},
			want: map[titanpath.AnchoredUnixPath]string{
		...
		},
		{
			...
			filesToHash: []titanpath.AnchoredSystemPath{
				titanpath.AnchoredUnixPath("null.json").ToSystemPath(),
			},
			want:    nil,
			wantErr: true,
		},
		{
			...
			rootPath: fixturePath,
			filesToHash: []titanpath.AnchoredSystemPath{
				titanpath.AnchoredUnixPath("nonexistent.json").ToSystemPath(),
			},
			want:    nil,
			wantErr: true,
		},
...
func requireGitCmd(t *testing.T, repoRoot titanpath.AbsoluteSystemPath, args ...string) {
	t.Helper()
...
func TestGetPackageDeps(t *testing.T) {
	...
	type PackageDepsOptions struct {
		...
		PackagePath titanpath.AnchoredSystemPath
	...
	tests := []struct {
		...
		expected map[titanpath.AnchoredUnixPath]string
	}{
...
func Test_getPackageFileHashesFromProcessingGitIgnore(t *testing.T) {
	...
	root := t.TempDir()
	repoRoot := titanpath.AbsoluteSystemPathFromUpstream(root)
	pkgName := titanpath.AnchoredUnixPath("child-dir/libA").ToSystemPath()
	type fileHash struct {
		contents string
		hash     string
	}
	files := map[titanpath.AnchoredUnixPath]fileHash{
	...
	files[titanpath.AnchoredUnixPath("child-dir/libA/.gitignore")] = fileHash{contents: "", hash: "3237694bc3312ded18386964a855074af7b066af"}
...
func Test_manuallyHashFiles(t *testing.T) {
	...
	type args struct {
		rootPath     titanpath.AbsoluteSystemPath
		files        []titanpath.AnchoredSystemPath
		allowMissing bool
	}
	tests := []struct {
		name    string
		...
		// Tests for allowMissing = true
		{
			...
			args: args{
				rootPath:     testDir,
				files:        []titanpath.AnchoredSystemPath{"non-existent-file.txt"},
				allowMissing: true,
			},
			want:    map[titanpath.AnchoredUnixPath]string{},
			wantErr: false,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			var got map[titanpath.AnchoredUnixPath]string
			var err error

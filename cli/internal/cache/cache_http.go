...
func (cache *httpCache) Put(anchor titanpath.AbsoluteSystemPath, hash string, duration int, files []titanpath.AnchoredSystemPath) error {
	// if cache.writable {
...
func (cache *httpCache) write(w io.WriteCloser, anchor titanpath.AbsoluteSystemPath, files []titanpath.AnchoredSystemPath, cacheErrorChan chan error) {
...
func (cache *httpCache) Fetch(_ titanpath.AbsoluteSystemPath, key string, _ []string) (ItemStatus, []titanpath.AnchoredSystemPath, error) {
	cache.requestLimiter.acquire()
	defer cache.requestLimiter.release()
...
func (cache *httpCache) retrieve(hash string) (bool, []titanpath.AnchoredSystemPath, int, error) {
...
func restoreTar(root titanpath.AbsoluteSystemPath, reader io.Reader) ([]titanpath.AnchoredSystemPath, error) {
	cache := cacheitem.FromReader(reader, true)
	return cache.Restore(root)
}

func (cache *httpCache) Clean(_ titanpath.AbsoluteSystemPath) {
...
func newHTTPCache(opts Opts, client client, recorder analytics.Recorder, repoRoot titanpath.AbsoluteSystemPath) *httpCache {
	return &httpCache{
		writable:       true,
		client:         client,

...
func (asa *ArtifactSignatureAuthentication) getSecretKey() ([]byte, error) {
	...
	if asa.secretKeyOverride != nil {
		...
		secret = []byte(os.Getenv("TITAN_REMOTE_CACHE_SIGNATURE_KEY"))
	}

	if len(secret) == 0 {
		return nil, errors.New("signature secret key not found. You must specify a secret key in the TITAN_REMOTE_CACHE_SIGNATURE_KEY environment variable")
	}
	return secret, nil
}

...
import (
	...
	"github.com/khulnasoft/titan/cli/internal/cacheitem"
...
func TestPut(t *testing.T) {
	...
	src := titanpath.AbsoluteSystemPath(t.TempDir())
	...
	files := []titanpath.AnchoredSystemPath{
		titanpath.AnchoredUnixPath("child/").ToSystemPath(),       // childDir
		titanpath.AnchoredUnixPath("child/a").ToSystemPath(),      // aPath,
		titanpath.AnchoredUnixPath("b").ToSystemPath(),            // bPath,
		titanpath.AnchoredUnixPath("child/link").ToSystemPath(),   // srcLinkPath,
		titanpath.AnchoredUnixPath("child/broken").ToSystemPath(), // srcBrokenLinkPath,
		titanpath.AnchoredUnixPath("child/circle").ToSystemPath(), // circlePath
	}

	dst := titanpath.AbsoluteSystemPath(t.TempDir())
	dr := &dummyRecorder{}
...
func assertFileMatches(t *testing.T, orig titanpath.AbsoluteSystemPath, copy titanpath.AbsoluteSystemPath) {
	t.Helper()
...
func TestFetch(t *testing.T) {
	...
	cacheDir := titanpath.AbsoluteSystemPath(t.TempDir())
	hash := "the-hash"
	...
	srcBrokenLinkTarget := titanpath.AnchoredUnixPath("missing").ToSystemPath()
	assert.NilError(t, srcBrokenLinkPath.Symlink(srcBrokenLinkTarget.ToString()), "Symlink")

	circlePath := childDir.Join("circle")
	srcCircleLinkTarget := titanpath.AnchoredUnixPath("../child").ToSystemPath()
	...
	inputFiles := []titanpath.AnchoredSystemPath{
		titanpath.AnchoredUnixPath("some-package/child/").ToSystemPath(),       // childDir
		titanpath.AnchoredUnixPath("some-package/child/a").ToSystemPath(),      // aPath,
		titanpath.AnchoredUnixPath("some-package/b").ToSystemPath(),            // bPath,
		titanpath.AnchoredUnixPath("some-package/child/link").ToSystemPath(),   // srcLinkPath,
		titanpath.AnchoredUnixPath("some-package/child/broken").ToSystemPath(), // srcBrokenLinkPath,
		titanpath.AnchoredUnixPath("some-package/child/circle").ToSystemPath(), // circlePath
	}

	putErr := cache.Put(cacheDir.UntypedJoin(hash), hash, 0, inputFiles)
	assert.NilError(t, putErr, "Put")

	outputDir := titanpath.AbsoluteSystemPath(t.TempDir())

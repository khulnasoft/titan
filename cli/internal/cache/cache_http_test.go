...
func (sr *errorResp) PutArtifact(hash string, body []byte, duration int, tag string) error {
	sr.t.Helper()
	outdir := titanpath.AbsoluteSystemPathFromUpstream(sr.t.TempDir())
...
func TestRestoreTar(t *testing.T) {
	...
	expectedFiles := []titanpath.AnchoredSystemPath{
		titanpath.AnchoredUnixPath("extra-file").ToSystemPath(),
		titanpath.AnchoredUnixPath("my-pkg").ToSystemPath(),
		titanpath.AnchoredUnixPath("my-pkg/some-file").ToSystemPath(),
		titanpath.AnchoredUnixPath("my-pkg/link-to-extra-file").ToSystemPath(),
		titanpath.AnchoredUnixPath("my-pkg/broken-link").ToSystemPath(),
	}
...
func Test_httpCache_Put(t *testing.T) {
	...
	assert.ErrorIs(
		t,
		cache.Put(root, "000", 10, []titanpath.AnchoredSystemPath{"one", "two"}),
		clientErr,
	...
	assert.ErrorIs(
		t,
		cache.Put(root, "000", 10, []titanpath.AnchoredSystemPath{"one", "two", "missing"}),
		os.ErrNotExist,
	...
	assert.ErrorIs(
		t,
		cache.Put(root, "000", 10, []titanpath.AnchoredSystemPath{"missing", "one", "two"}),
		os.ErrNotExist,

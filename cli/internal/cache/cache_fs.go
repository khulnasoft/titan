...
type fsCache struct {
	cacheDirectory titanpath.AbsoluteSystemPath
	recorder       analytics.Recorder
}

// newFsCache creates a new filesystem cache
func newFsCache(opts Opts, recorder analytics.Recorder, repoRoot titanpath.AbsoluteSystemPath) (*fsCache, error) {
...
func (f *fsCache) Fetch(anchor titanpath.AbsoluteSystemPath, hash string, _ []string) (ItemStatus, []titanpath.AnchoredSystemPath, error) {
	uncompressedCachePath := f.cacheDirectory.UntypedJoin(hash + ".tar")
	compressedCachePath := f.cacheDirectory.UntypedJoin(hash + ".tar.zst")

	var actualCachePath titanpath.AbsoluteSystemPath
...
func (f *fsCache) Put(anchor titanpath.AbsoluteSystemPath, hash string, duration int, files []titanpath.AnchoredSystemPath) error {
...
func (f *fsCache) Clean(_ titanpath.AbsoluteSystemPath) {

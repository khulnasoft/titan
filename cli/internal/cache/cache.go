...
type Cache interface {
	...
	Fetch(anchor titanpath.AbsoluteSystemPath, hash string, files []string) (ItemStatus, []titanpath.AnchoredSystemPath, error)
...
func DefaultLocation(repoRoot titanpath.AbsoluteSystemPath) titanpath.AbsoluteSystemPath {
	return repoRoot.UntypedJoin("node_modules", ".cache", "titan")
}
...
// resolveCacheDir calculates the location titan should use to cache artifacts,
// based on the options supplied by the user.
func (o *Opts) resolveCacheDir(repoRoot titanpath.AbsoluteSystemPath) titanpath.AbsoluteSystemPath {
	if o.OverrideDir != "" {
...
func newSyncCache(opts Opts, repoRoot titanpath.AbsoluteSystemPath, client client, recorder analytics.Recorder, onCacheRemoved OnCacheRemoved) (Cache, error) {
...
func (mplex *cacheMultiplexer) Put(anchor titanpath.AbsoluteSystemPath, key string, duration int, files []titanpath.AnchoredSystemPath) error {
...
func (mplex *cacheMultiplexer) storeUntil(anchor titanpath.AbsoluteSystemPath, key string, duration int, files []titanpath.AnchoredSystemPath, stopAt int) error {
...
func (mplex *cacheMultiplexer) Fetch(anchor titanpath.AbsoluteSystemPath, key string, files []string) (ItemStatus, []titanpath.AnchoredSystemPath, error) {
...
func (mplex *cacheMultiplexer) Clean(anchor titanpath.AbsoluteSystemPath) {
	for _, cache := range mplex.caches {
		cache.Clean(anchor)
	}
}

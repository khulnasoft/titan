...
type CookieJar struct {
	...
	dir     titanpath.AbsoluteSystemPath
	serial  uint64
	mu      sync.Mutex
	cookies map[titanpath.AbsoluteSystemPath]chan error
	closed  bool
}
...
func NewCookieJar(cookieDir titanpath.AbsoluteSystemPath, timeout time.Duration) (*CookieJar, error) {
	...
	return &CookieJar{
		timeout: timeout,
		dir:     cookieDir,
		cookies: make(map[titanpath.AbsoluteSystemPath]chan error),
	}, nil
}
...
func (cj *CookieJar) OnFileWatchError(err error) {
	...
	cj.cookies = make(map[titanpath.AbsoluteSystemPath]chan error)
}
...
func (cj *CookieJar) notifyCookie(cookie titanpath.AbsoluteSystemPath, err error) {
	cj.mu.Lock()
...
func touchCookieFile(cookie titanpath.AbsoluteSystemPath) error {

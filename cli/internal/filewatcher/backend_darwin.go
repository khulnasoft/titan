...
func (f *fseventsBackend) AddRoot(someRoot titanpath.AbsoluteSystemPath, excludePatterns ...string) error {
...
func waitForCookie(root titanpath.AbsoluteSystemPath, events <-chan []fsevents.Event, timeout time.Duration) error {
	// This cookie needs to be in a location that we're watching, and at this point we can't guarantee
	// what the root is, or if something like "node_modules/.cache/titan" would make sense. As a compromise, ensure
	// that we clean it up even in the event of a failure.
	cookiePath := root.UntypedJoin(".titan-cookie")
...
func toFileEvent(flags fsevents.EventFlags, path titanpath.AbsoluteSystemPath) FileEvent {

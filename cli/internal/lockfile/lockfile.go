...
type Lockfile interface {
	...
	ResolvePackage(workspacePath titanpath.AnchoredUnixPath, name string, version string) (Package, error)
...
type closureMsg struct {
	workspace titanpath.AnchoredUnixPath
	closure   mapset.Set
}

// AllTransitiveClosures computes closures for all workspaces
func AllTransitiveClosures(
	workspaces map[titanpath.AnchoredUnixPath]map[string]string,
	lockFile Lockfile,
) (map[titanpath.AnchoredUnixPath]mapset.Set, error) {
	...
	closures := make(map[titanpath.AnchoredUnixPath]mapset.Set, len(workspaces))
...
func transitiveClosure(
	workspaceDir titanpath.AnchoredUnixPath,
...
func rustTransitiveDeps(content []byte, packageManager string, workspaces map[titanpath.AnchoredUnixPath]map[string]string, resolutions map[string]string) (map[titanpath.AnchoredUnixPath]mapset.Set, error) {
	...
	resolvedWorkspaces := make(map[titanpath.AnchoredUnixPath]mapset.Set, len(workspaceDeps))
	for workspace, dependencies := range workspaceDeps {
		...
		workspacePath := titanpath.AnchoredUnixPath(workspace)

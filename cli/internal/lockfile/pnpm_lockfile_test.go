...
func Test_PnpmAliasesOverlap(t *testing.T) {
	...
	closures, err := AllTransitiveClosures(map[titanpath.AnchoredUnixPath]map[string]string{"packages/a": {"@scope/parent": "^1.0.0", "another": "^1.0.0", "special": "npm:Special@1.2.3"}}, lockfile)
	assert.NilError(t, err)

	closure, ok := closures[titanpath.AnchoredUnixPath("packages/a")]
	assert.Assert(t, ok)

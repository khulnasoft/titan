...
import (
	...
	"github.com/khulnasoft/titan/cli/internal/hashing"
	"github.com/khulnasoft/titan/cli/internal/inference"
...
type Tracker struct {
	...
	PackageInputsExpandedHashes map[string]map[titanpath.AnchoredUnixPath]string
	...
	packageTaskOutputs     map[string][]titanpath.AnchoredSystemPath
...
func NewTracker(rootNode string, globalHash string, envAtExecutionStart env.EnvironmentVariableMap, pipeline fs.Pipeline) *Tracker {
	return &Tracker{
		...
		packageTaskOutputs:     make(map[string][]titanpath.AnchoredSystemPath),
...
func (th *Tracker) CalculateFileHashes(
	...
	hashObjects := make(map[string]map[titanpath.AnchoredUnixPath]string, len(hashTasks))
...
func (th *Tracker) CalculateTaskHash(logger hclog.Logger, packageTask *nodes.PackageTask, dependencySet dag.Set, frameworkInference bool, args []string) (string, error) {
	...
	if frameworkInference {
		...
		if framework != nil {
			...
			excludePrefix, exists := th.EnvAtExecutionStart["TITAN_CI_VENDOR_ENV_KEY"]
...
func (th *Tracker) GetExpandedInputs(packageTask *nodes.PackageTask) map[titanpath.AnchoredUnixPath]string {
	expandedInputs := th.PackageInputsExpandedHashes[packageTask.TaskID]
	inputsCopy := make(map[titanpath.AnchoredUnixPath]string, len(expandedInputs))
...
func (th *Tracker) GetExpandedOutputs(taskID string) []titanpath.AnchoredSystemPath {
	th.mu.RLock()
	defer th.mu.RUnlock()
	outputs, ok := th.packageTaskOutputs[taskID]

	if !ok {
		return []titanpath.AnchoredSystemPath{}
	}
...
func (th *Tracker) SetExpandedOutputs(taskID string, outputs []titanpath.AnchoredSystemPath) {
	th.mu.Lock()
	defer th.mu.Unlock()

...
func BuildPackageGraph(repoRoot titanpath.AbsoluteSystemPath, rootPackageJSON *fs.PackageJSON, packageManagerName string) (*Context, error) {
	c := &Context{}
...
func (c *Context) parsePackageJSON(repoRoot titanpath.AbsoluteSystemPath, pkgJSONPath titanpath.AbsoluteSystemPath) error {
	c.mutex.Lock()
	defer c.mutex.Unlock()
	...
	if pkgJSONPath.FileExists() {
		...
		pkg.PackageJSONPath = titanpath.AnchoredSystemPathFromUpstream(relativePkgJSONPath)
		pkg.Dir = titanpath.AnchoredSystemPathFromUpstream(filepath.Dir(relativePkgJSONPath))
...
func (c *Context) externalWorkspaceDeps() map[titanpath.AnchoredUnixPath]map[string]string {
	workspaces := make(map[titanpath.AnchoredUnixPath]map[string]string, len(c.WorkspaceInfos.PackageJSONs))
	for _, pkg := range c.WorkspaceInfos.PackageJSONs {
...
func (c *Context) populateExternalDeps(repoRoot titanpath.AbsoluteSystemPath, rootPackageJSON *fs.PackageJSON, warnings *Warnings) error {

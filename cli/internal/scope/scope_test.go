...
import (
	...
	"github.com/khulnasoft/titan/cli/internal/packagemanager"
...
func (m *mockLockfile) ResolvePackage(workspacePath titanpath.AnchoredUnixPath, name string, version string) (lockfile.Package, error) {
...
func (m *mockLockfile) Subgraph(workspaces []titanpath.AnchoredSystemPath, packages []string) (lockfile.Lockfile, error) {
	return nil, nil
}
...
func TestResolvePackages(t *testing.T) {
	...
	workspaceInfos := workspace.Catalog{
		PackageJSONs: map[string]*fs.PackageJSON{
			...
			"app1": {
				Dir:  titanpath.AnchoredUnixPath("app/app1").ToSystemPath(),
				Name: "app1",
			},
			"app2": {
				Dir:  titanpath.AnchoredUnixPath("app/app2").ToSystemPath(),
				Name: "app2",
			},
			"app2-a": {
				Dir:  titanpath.AnchoredUnixPath("app/app2-a").ToSystemPath(),
				Name: "app2-a",
			},
			"libA": {
				Dir:  titanpath.AnchoredUnixPath("libs/libA").ToSystemPath(),
				Name: "libA",
			},
			"libB": {
				Dir:                    titanpath.AnchoredUnixPath("libs/libB").ToSystemPath(),
				Name:                   "libB",
			...
			"libC": {
				Dir:  titanpath.AnchoredUnixPath("libs/libC").ToSystemPath(),
				Name: "libC",
			},
			"libD": {
				Dir:  titanpath.AnchoredUnixPath("libs/libD").ToSystemPath(),
				Name: "libD",
			},
		},
	}
	...
	testCases := []struct {
		...
		{
			name:                "Only titan.json changed",
			changed:             []string{"titan.json"},
	...
	for i, tc := range testCases {
		t.Run(fmt.Sprintf("test #%v %v", i, tc.name), func(t *testing.T) {
			...
			pkgs, isAllPackages, err := ResolvePackages(&Opts{
				...
				PackageManager: &packagemanager.PackageManager{Lockfile: tc.lockfile, UnmarshalLockfile: readLockfile, GetLockfileName: func(_ titanpath.AbsoluteSystemPath) string { return tc.lockfile }},

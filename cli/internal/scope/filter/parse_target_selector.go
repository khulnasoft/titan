...
type TargetSelector struct {
	...
	parentDir           titanpath.RelativeSystemPath
	namePattern         string
	fromRef             string
...
func ParseTargetSelector(rawSelector string) (*TargetSelector, error) {
	...
	fromRef := ""
	toRefOverride := ""
	var parentDir titanpath.RelativeSystemPath
	namePattern := ""
	...
	if len(matches) > 0 && len(matches[0]) > 0 {
		...
		if len(rawParentDir) > 0 {
			...
			} else if relPath, err := titanpath.CheckedToRelativeSystemPath(rawParentDir); err == nil {
				parentDir = relPath
			} else {
...
func isSelectorByLocation(rawSelector string) (titanpath.RelativeSystemPath, bool) {
	if rawSelector[0:1] != "." {
		return "", false
	}
	...
	if len(rawSelector) == 1 || rawSelector[1:2] == "/" || rawSelector[1:2] == "\\" {
		return titanpath.MakeRelativeSystemPath(rawSelector), true
	}

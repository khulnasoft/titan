...
import (
	...
	"github.com/khulnasoft/titan/cli/internal/analytics"
	"github.com/khulnasoft/titan/cli/internal/cache"
	"github.com/khulnasoft/titan/cli/internal/ci"
	"github.com/khulnasoft/titan/cli/internal/cmdutil"
	"github.com/khulnasoft/titan/cli/internal/context"
	"github.com/khulnasoft/titan/cli/internal/core"
	"github.com/khulnasoft/titan/cli/internal/daemon"
	"github.com/khulnasoft/titan/cli/internal/daemonclient"
	...
	"github.com/khulnasoft/titan/cli/internal/process"
	"github.com/khulnasoft/titan/cli/internal/runsummary"
	"github.com/khulnasoft/titan/cli/internal/scm"
	"github.com/khulnasoft/titan/cli/internal/scope"
	"github.com/khulnasoft/titan/cli/internal/signals"
...
func ExecuteRun(ctx gocontext.Context, helper *cmdutil.Helper, signalWatcher *signals.Watcher, executionState *titanstate.ExecutionState) error {
...
func optsFromArgs(args *titanstate.ParsedArgsFromRust) (*Opts, error) {
	...
	// See comment on Graph in titanstate.go for an explanation on Graph's representation.
	// If flag is passed...
...
func (r *run) run(ctx gocontext.Context, targets []string, executionState *titanstate.ExecutionState) error {
	startAt := time.Now()
	...
	if ui.IsCI && !r.opts.runOpts.NoDaemon {
		r.base.Logger.Debug("skipping titand since we appear to be in a non-interactive context")
		...
		if err != nil {
			...
			defer func() { _ = titandClient.Close() }()
			r.base.Logger.Debug("running in daemon mode")
			daemonClient := daemonclient.New(titandClient)
	...
	titanJSON, err := g.GetTitanConfigFromWorkspace(util.RootPkgName, r.opts.runOpts.SinglePackage)
	if err != nil {
		return err
	}
	...
	// If a spaceID wasn't passed as a flag, read it from the titan.json config.
	// If that is not set either, we'll still end up with a blank string.
	if r.opts.runOpts.ExperimentalSpaceID == "" {
		r.opts.runOpts.ExperimentalSpaceID = titanJSON.SpaceID
	}
	...
	globalHashInputs, err := getGlobalHashInputs(
		...
		titanJSON.GlobalDeps,
		envAtExecutionStart,
		titanJSON.GlobalEnv,
		titanJSON.GlobalPassThroughEnv,
		r.opts.runOpts.EnvMode,
		r.opts.runOpts.FrameworkInference,
		titanJSON.GlobalDotEnv,
	)
	...
	titanCache, err := r.initCache(ctx, rs, analyticsClient)

	if err != nil {
		if errors.Is(err, cache.ErrNoCachesEnabled) {
			r.base.UI.Warn("No caches are enabled. You can try \"titan login\", \"titan link\", or ensuring you are not passing --remote-only to enable caching")
		} else {
	...
	if globalEnvMode == util.Infer && titanJSON.GlobalPassThroughEnv != nil {

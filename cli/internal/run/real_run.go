...
import (
	...
	"github.com/khulnasoft/titan/cli/internal/runcache"
...
func RealRun(
	...
	defer func() {
		_ = spinner.WaitFor(ctx, titanCache.Shutdown, base.UI, "...Finishing writing to cache...", 1500*time.Millisecond)
	}()
	colorCache := colorcache.New()

	runCache := runcache.New(titanCache, base.RepoRoot, rs.Opts.runcacheOpts, colorCache)
...
func (ec *execContext) exec(ctx gocontext.Context, packageTask *nodes.PackageTask, ui cli.Ui, outWriter io.Writer) (*runsummary.TaskExecutionSummary, error) {
	...
	passThroughEnv.Add("TITAN_HASH", hash)

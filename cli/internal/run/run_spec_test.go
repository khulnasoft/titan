...
func TestSynthesizeCommand(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			tasks:          []string{"build"},
			expected:       "titan run build --filter=my-app",
		},
		{
			tasks:    []string{"build"},
			only:     true,
			expected: "titan run build --only",
		},
		{
			filterPatterns: []string{"my-app"},
			tasks:          []string{"build"},
		...
		},
		{
			...
			expected:        "titan run build --filter=my-app -- -v --foo=bar",
		},
		{
			legacyFilter: scope.LegacyFilter{
		...
		},
		{
			...
			tasks:           []string{"build"},
			passThroughArgs: []string{"-v", "--foo=bar"},
			expected:        "titan run build --filter=other-app --filter=my-app -- -v --foo=bar",
		},
		{
			legacyFilter: scope.LegacyFilter{
				Entrypoints:         []string{"my-app"},
			...
			filterPatterns: []string{"other-app"},
			tasks:          []string{"build"},
			expected:       "titan run build --filter=other-app --filter=...my-app...[some-ref]...",
		},
		{
			filterPatterns:  []string{"my-app"},
			tasks:           []string{"build"},
			...
			expected:        "titan run build --filter=my-app --parallel --continue",
		},
		{
			filterPatterns: []string{"my-app"},
			tasks:          []string{"build"},
			dryRun:         true,
			expected:       "titan run build --filter=my-app --dry",
		},
		{
			filterPatterns: []string{"my-app"},
			tasks:          []string{"build"},
			dryRun:         true,
			dryRunJSON:     true,
			expected:       "titan run build --filter=my-app --dry=json",
		},
	}

...
// PackageType represents the mode in which titan is running.
...
func candidateDirectoryWorkspaceGlobs(directory titanpath.AbsoluteSystemPath) []string {
...
func isOneOfTheWorkspaces(globs []string, nearestPackageJSONDir titanpath.AbsoluteSystemPath, currentPackageJSONDir titanpath.AbsoluteSystemPath) bool {
...
// titan should be in when operating at that directory.
func InferRoot(directory titanpath.AbsoluteSystemPath) (titanpath.AbsoluteSystemPath, PackageType) {
	// Go doesn't have iterators, so this is very not-elegant.

	// Scenarios:
	// 0. Has a titan.json but doesn't have a peer package.json. directory + multi
	// 1. Nearest titan.json, check peer package.json/pnpm-workspace.yaml.
	//    A. Has workspaces, multi package mode.
	//    B. No workspaces, single package mode.
	// 2. If no titan.json find the closest package.json parent.

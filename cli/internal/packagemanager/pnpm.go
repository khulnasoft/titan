...
import (
	...
	"github.com/khulnasoft/titan/cli/internal/yaml"
)
...
func readPnpmWorkspacePackages(workspaceFile titanpath.AbsoluteSystemPath) ([]string, error) {
...
func getPnpmWorkspaceGlobs(rootpath titanpath.AbsoluteSystemPath) ([]string, error) {
	...
	if len(pkgGlobs) == 0 {
		return nil, fmt.Errorf("pnpm-workspace.yaml: no packages found. Titanrepo requires pnpm workspaces and thus packages to be defined in the root pnpm-workspace.yaml")
	}
...
func getPnpmWorkspaceIgnores(pm PackageManager, rootpath titanpath.AbsoluteSystemPath) ([]string, error) {
...
var nodejsPnpm = PackageManager{
	...
	prunePatches: func(pkgJSON *fs.PackageJSON, patches []titanpath.AnchoredUnixPath) error {
		return pnpmPrunePatches(pkgJSON, patches)
	},
}

func pnpmPrunePatches(pkgJSON *fs.PackageJSON, patches []titanpath.AnchoredUnixPath) error {
	pkgJSON.Mu.Lock()

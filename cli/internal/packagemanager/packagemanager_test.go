...
func Test_GetWorkspaces(t *testing.T) {
	...
	rootPath := map[string]titanpath.AbsoluteSystemPath{
	...
	want := map[string][]string{
		"nodejs-bun": {
			...
			filepath.ToSlash(filepath.Join(cwd, "../../../examples/with-yarn/packages/eslint-config/package.json")),
			filepath.ToSlash(filepath.Join(cwd, "../../../examples/with-yarn/packages/typescript-config/package.json")),

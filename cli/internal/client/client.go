...
func NewClient(config titanstate.APIClientConfig, logger hclog.Logger, titanVersion string) *APIClient {
	client := &APIClient{
...
func (c *APIClient) userAgent() string {
	return fmt.Sprintf("titan %v %v %v (%v)", c.titanVersion, runtime.Version(), runtime.GOOS, runtime.GOARCH)
}

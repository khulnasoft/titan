...
import (
	...
	"github.com/khulnasoft/titan/cli/internal/colorcache"
	...
	"github.com/khulnasoft/titan/cli/internal/logstreamer"
...
type LogReplayer = func(logger hclog.Logger, output *cli.PrefixedUi, logFile titanpath.AbsoluteSystemPath)
...
// RunCache represents the interface to the cache for a single `titan run`
...
func New(cache cache.Cache, repoRoot titanpath.AbsoluteSystemPath, opts Opts, colorCache *colorcache.ColorCache) *RunCache {
	rc := &RunCache{
...
type TaskCache struct {
	...
	LogFileName       titanpath.AbsoluteSystemPath
}
...
func (tc *TaskCache) SaveOutputs(ctx context.Context, logger hclog.Logger, terminal cli.Ui, duration int) error {
	...
	relativePaths := make([]titanpath.AnchoredSystemPath, len(filesToBeCached))
...
func defaultLogReplayer(logger hclog.Logger, output *cli.PrefixedUi, logFileName titanpath.AbsoluteSystemPath) {

...
func (ci *CacheItem) Restore(anchor titanpath.AbsoluteSystemPath) ([]titanpath.AnchoredSystemPath, error) {
	...
	dirCache := &cachedDirTree{
		anchorAtDepth: []titanpath.AbsoluteSystemPath{anchor},
	}
...
func restoreEntry(dirCache *cachedDirTree, anchor titanpath.AbsoluteSystemPath, header *tar.Header, reader *tar.Reader) (titanpath.AnchoredSystemPath, error) {
...
func canonicalizeName(name string) (titanpath.AnchoredSystemPath, error) {
	...
	return titanpath.AnchoredUnixPathFromUpstream(noTrailingSlash).ToSystemPath(), nil
}

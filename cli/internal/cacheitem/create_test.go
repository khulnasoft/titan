...
func createEntry(t *testing.T, anchor titanpath.AbsoluteSystemPath, fileDefinition createFileDefinition) error {
	t.Helper()
...
func createDir(t *testing.T, anchor titanpath.AbsoluteSystemPath, fileDefinition createFileDefinition) error {
	t.Helper()
...
func createFile(t *testing.T, anchor titanpath.AbsoluteSystemPath, fileDefinition createFileDefinition) error {
	t.Helper()
	path := fileDefinition.Path.RestoreAnchor(anchor)
...
func createSymlink(t *testing.T, anchor titanpath.AbsoluteSystemPath, fileDefinition createFileDefinition) error {
	t.Helper()
	path := fileDefinition.Path.RestoreAnchor(anchor)
...
func TestCreate(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			files: []createFileDefinition{
				{
					Path:     titanpath.AnchoredSystemPath("hello world.txt"),
					FileMode: 0 | 0644,
				},
			},
		...
		},
		{
			...
			files: []createFileDefinition{
				{
					Path:     titanpath.AnchoredSystemPath("one"),
					Linkname: "two",
					FileMode: 0 | os.ModeSymlink | 0777,
				},
				{
					Path:     titanpath.AnchoredSystemPath("two"),
					Linkname: "three",
					FileMode: 0 | os.ModeSymlink | 0777,
				},
				{
					Path:     titanpath.AnchoredSystemPath("three"),
					Linkname: "real",
					FileMode: 0 | os.ModeSymlink | 0777,
				},
				{
					Path:     titanpath.AnchoredSystemPath("real"),
					FileMode: 0 | 0644,
				},
			},
		...
		},
		{
			...
			files: []createFileDefinition{
				{
					Path:     titanpath.AnchoredSystemPath("parent"),
					FileMode: 0 | os.ModeDir | 0755,
				},
				{
					Path:     titanpath.AnchoredSystemPath("parent/child"),
					FileMode: 0 | 0644,
				},
			},
		...
		},
		{
			...
			files: []createFileDefinition{
				{
					Path:     titanpath.AnchoredSystemPath("fifo"),
	...
	for _, tt := range tests {
		getTestFunc := func(compressed bool) func(t *testing.T) {
			return func(t *testing.T) {
				inputDir := titanpath.AbsoluteSystemPath(t.TempDir())
				archiveDir := titanpath.AbsoluteSystemPath(t.TempDir())
				var archivePath titanpath.AbsoluteSystemPath
				if compressed {
					archivePath = titanpath.AnchoredSystemPath("out.tar.zst").RestoreAnchor(archiveDir)
				} else {
					archivePath = titanpath.AnchoredSystemPath("out.tar").RestoreAnchor(archiveDir)
				}

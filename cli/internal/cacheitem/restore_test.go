...
type restoreFile struct {
	Name     titanpath.AnchoredUnixPath
	Linkname string
	fs.FileMode
}

// generateTar is used specifically to generate tar files that Titanrepo would
...
func generateTar(t *testing.T, files []tarFile) titanpath.AbsoluteSystemPath {
	t.Helper()
	testDir := titanpath.AbsoluteSystemPath(t.TempDir())
...
func compressTar(t *testing.T, archivePath titanpath.AbsoluteSystemPath) titanpath.AbsoluteSystemPath {
	t.Helper()
...
func generateAnchor(t *testing.T) titanpath.AbsoluteSystemPath {
	t.Helper()
	testDir := titanpath.AbsoluteSystemPath(t.TempDir())
...
func assertFileExists(t *testing.T, anchor titanpath.AbsoluteSystemPath, diskFile restoreFile) {
	t.Helper()
...
func TestOpen(t *testing.T) {
	...
	type wantOutput struct {
		unix    []titanpath.AnchoredSystemPath
		windows []titanpath.AnchoredSystemPath
	}
	...
	tests := []struct {
	...
	}{
		{
			...
			wantOutput: wantOutput{
				unix: titanpath.AnchoredUnixPathArray{
					"one",
					"one/two",
					"one/two/three",
		...
		},
		{
			...
			wantOutput: wantOutput{
				unix: titanpath.AnchoredUnixPathArray{"target", "source"}.ToSystemPathArray(),
			},
		},
		{
			name: "nested file",
			...
			wantOutput: wantOutput{
				unix: titanpath.AnchoredUnixPathArray{"folder", "folder/file"}.ToSystemPathArray(),
			},
		},
		{
			name: "nested symlink",
			...
			wantOutput: wantOutput{
				unix: titanpath.AnchoredUnixPathArray{"folder", "folder/symlink", "folder/symlink/folder-sibling"}.ToSystemPathArray(),
			},
		},
		{
			...
			wantOutput: wantOutput{
				unix: titanpath.AnchoredUnixPathArray{"real", "three", "two", "one"}.ToSystemPathArray(),
			},
		},
		{
			...
			wantOutput: wantOutput{
				unix: titanpath.AnchoredUnixPathArray{"folder-not-file", "folder-not-file/subfile"}.ToSystemPathArray(),
			},
			wantErr: wantErr{
				unix:    syscall.EISDIR,
		...
		// 	},
		// 	wantOutput: wantOutput{
		// 		unix:    titanpath.AnchoredUnixPathArray{"one"}.ToSystemPathArray(),
		// 		windows: nil,
		// 	},
		// 	wantErr: wantErr{
		// 		unix:    os.ErrExist,
		...
		},
		{
			...
			wantOutput: wantOutput{
				unix: titanpath.AnchoredUnixPathArray{"real", "one"}.ToSystemPathArray(),
			},
		},
		{
			...
			wantOutput: wantOutput{
				unix: titanpath.AnchoredUnixPathArray{"escape"}.ToSystemPathArray(),
			},
			wantErr: wantErr{
				unix:    errTraversal,
		...
		},
		{
			...
			wantOutput: wantOutput{
				unix:    titanpath.AnchoredUnixPathArray{"back\\slash\\file"}.ToSystemPathArray(),
				windows: titanpath.AnchoredUnixPathArray{}.ToSystemPathArray(),
			},
			wantErr: wantErr{
				unix:    nil,
	...
	for _, tt := range tests {
		getTestFunc := func(compressed bool) func(t *testing.T) {
			return func(t *testing.T) {
				var archivePath titanpath.AbsoluteSystemPath
				if compressed {
...
func Test_canonicalizeLinkname(t *testing.T) {
	...
	anchor := titanpath.AbsoluteSystemPath(filepath.Join("path", "to", "anchor"))

	tests := []struct {
		name             string
		processedName    titanpath.AnchoredSystemPath
		linkname         string
		...
		{
			name:             "hello world",
			processedName:    titanpath.AnchoredSystemPath("source"),
			linkname:         "target",
		...
		},
		{
			...
			processedName:    titanpath.AnchoredUnixPath("child/source").ToSystemPath(),
...
func Test_canonicalizeName(t *testing.T) {
	tests := []struct {
		name     string
		fileName string
		want     titanpath.AnchoredSystemPath
		wantErr  error
	}{
		{
			name:     "hello world",
...
func TestCacheItem_Restore(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			want: titanpath.AnchoredUnixPathArray{"target", "source", "one", "one/two"}.ToSystemPathArray(),
		},
	}
	for _, tt := range tests {
		getTestFunc := func(compressed bool) func(t *testing.T) {

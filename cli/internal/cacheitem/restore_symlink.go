...
func restoreSymlink(dirCache *cachedDirTree, anchor titanpath.AbsoluteSystemPath, header *tar.Header) (titanpath.AnchoredSystemPath, error) {
...
func restoreSymlinkMissingTarget(dirCache *cachedDirTree, anchor titanpath.AbsoluteSystemPath, header *tar.Header) (titanpath.AnchoredSystemPath, error) {
	processedName, canonicalizeNameErr := canonicalizeName(header.Name)
	if canonicalizeNameErr != nil {
		return "", canonicalizeNameErr
	}
...
func actuallyRestoreSymlink(dirCache *cachedDirTree, anchor titanpath.AbsoluteSystemPath, processedName titanpath.AnchoredSystemPath, header *tar.Header) (titanpath.AnchoredSystemPath, error) {
...
func topologicallyRestoreSymlinks(dirCache *cachedDirTree, anchor titanpath.AbsoluteSystemPath, symlinks []*tar.Header, tr *tar.Reader) ([]titanpath.AnchoredSystemPath, error) {
	restored := make([]titanpath.AnchoredSystemPath, 0)
...
func canonicalizeLinkname(anchor titanpath.AbsoluteSystemPath, processedName titanpath.AnchoredSystemPath, linkname string) string {
	...
	// Take for example: `/Users/titanbot/weird-filenames/\foo\/lol`

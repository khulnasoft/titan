...
func restoreDirectory(dirCache *cachedDirTree, anchor titanpath.AbsoluteSystemPath, header *tar.Header) (titanpath.AnchoredSystemPath, error) {
...
func (cr *cachedDirTree) getStartingPoint(path titanpath.AnchoredSystemPath) (titanpath.AbsoluteSystemPath, []titanpath.RelativeSystemPath) {
	pathSegmentStrings := strings.Split(path.ToString(), string(os.PathSeparator))
	pathSegments := make([]titanpath.RelativeSystemPath, len(pathSegmentStrings))
	for index, pathSegmentString := range pathSegmentStrings {
		pathSegments[index] = titanpath.RelativeSystemPathFromUpstream(pathSegmentString)
	}
...
func (cr *cachedDirTree) Update(anchor titanpath.AbsoluteSystemPath, newSegment titanpath.RelativeSystemPath) {
...
func safeMkdirAll(dirCache *cachedDirTree, anchor titanpath.AbsoluteSystemPath, processedName titanpath.AnchoredSystemPath, mode int64) error {
...
func checkPath(originalAnchor titanpath.AbsoluteSystemPath, accumulatedAnchor titanpath.AbsoluteSystemPath, segment titanpath.RelativeSystemPath) (titanpath.AbsoluteSystemPath, error) {
	...
	if filepath.IsAbs(linkTarget) {
		absoluteLinkTarget := titanpath.AbsoluteSystemPathFromUpstream(linkTarget)
	...
	relativeLinkTarget := titanpath.RelativeSystemPathFromUpstream(linkTarget)

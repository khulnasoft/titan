...
type Task struct {
	Name string
	// TaskDefinition contains the config for the task from titan.json
...
type Engine struct {
	...
	// isSinglePackage is used to load titan.json correctly
...
func (e *Engine) getTaskDefinition(pkg string, taskName string, taskID string) (*Task, error) {
	...
	// An error here means titan.json exists, but didn't define the task.
...
func (e *Engine) Prepare(options *EngineBuildingOptions) error {
	...
	for len(traversalQueue) > 0 {
		...
		if pkg == util.RootPkgName && !e.rootEnabledTasks.Includes(taskName) {
			return fmt.Errorf("%v needs an entry in titan.json before it can be depended on because it is a task run from the root package", taskID)
		}
		...
		if pkg != ROOT_NODE_NAME {
			if _, ok := e.completeGraph.WorkspaceInfos.PackageJSONs[pkg]; !ok {
				...
				// or the workspace really doesn't exist and titan.json is misconfigured.
		...
		// https: //titan.build/repo/docs/reference/command-line-reference/run#--only
		if tasksOnly {
...
func (e *Engine) ValidatePersistentDependencies(graph *graph.CompleteGraph, concurrency int) error {
	...
	if len(validationErrors) > 0 {
		...
		return fmt.Errorf("You have %v persistent tasks but `titan` is configured for concurrency of %v. Set --concurrency to at least %v", persistentCount, concurrency, persistentCount+1)
	}
...
func (e *Engine) getTaskDefinitionChain(taskID string, taskName string) ([]fs.BookkeepingTaskDefinition, error) {
	...
	if e.isSinglePackage {
		if len(taskDefinitions) == 0 {
			return nil, fmt.Errorf("Could not find \"%s\" in root titan.json", taskID)
		}
		return taskDefinitions, nil
	}

	// If the taskID is a root task (e.g. //#build), we don't need to look
	// for a workspace task, since these can only be defined in the root titan.json.
	taskIDPackage, _ := util.GetPackageTaskFromId(taskID)
	if taskIDPackage != util.RootPkgName && taskIDPackage != ROOT_NODE_NAME {
		// If there is an error, we can ignore it, since titan.json config is not required in the workspace.
		if workspaceTitanJSON, err := e.completeGraph.GetTitanConfigFromWorkspace(taskIDPackage, e.isSinglePackage); err != nil {
			// swallow the error where the config file doesn't exist, but bubble up other things
			if !errors.Is(err, os.ErrNotExist) {
				return nil, err
			}
		} else {
			// Run some validations on a workspace titan.json. Note that these validations are on
			// the whole struct, and not relevant to the taskID we're looking at right now.
			validationErrors := workspaceTitanJSON.Validate([]fs.TitanJSONValidation{
				validateNoPackageTaskSyntax,
				validateExtends,
			})

			if len(validationErrors) > 0 {
				fullError := errors.New("Invalid titan.json")
			...
			if workspaceDefinition, ok := workspaceTitanJSON.Pipeline[taskName]; ok {
	...
	if len(taskDefinitions) == 0 {
		return nil, fmt.Errorf("Could not find \"%s\" in root titan.json or \"%s\" in workspace", taskID, taskName)
	}

	return taskDefinitions, nil
}

func validateNoPackageTaskSyntax(titanJSON *fs.TitanJSON) []error {
	errors := []error{}

	for taskIDOrName := range titanJSON.Pipeline {
...
func validateExtends(titanJSON *fs.TitanJSON) []error {
	extendErrors := []error{}
	extends := titanJSON.Extends

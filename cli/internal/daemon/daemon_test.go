...
// Since node is required for titan development, it makes a decent
// candidate.
func testBin() string {
...
func waitForFile(t *testing.T, filename titanpath.AbsoluteSystemPath, timeout time.Duration) {
	t.Helper()
...
func TestCaughtSignal(t *testing.T) {
	...
	// registered with the titanserver, we've registered all of our

...
import (
	...
	"github.com/khulnasoft/titan/cli/internal/daemon/connector"
	"github.com/khulnasoft/titan/cli/internal/fs"
	"github.com/khulnasoft/titan/cli/internal/server"
...
func getRepoHash(repoRoot titanpath.AbsoluteSystemPath) string {
...
func getDaemonFileRoot(repoRoot titanpath.AbsoluteSystemPath) titanpath.AbsoluteSystemPath {
	tempDir := fs.TempDir("titand")
	hexHash := getRepoHash(repoRoot)
	return tempDir.UntypedJoin(hexHash)
}

func getLogFilePath(repoRoot titanpath.AbsoluteSystemPath) (titanpath.AbsoluteSystemPath, error) {
	hexHash := getRepoHash(repoRoot)
	base := repoRoot.Base()
...
func getUnixSocket(repoRoot titanpath.AbsoluteSystemPath) titanpath.AbsoluteSystemPath {
	root := getDaemonFileRoot(repoRoot)
	return root.UntypedJoin("titand.sock")
}

func getPidFile(repoRoot titanpath.AbsoluteSystemPath) titanpath.AbsoluteSystemPath {
	root := getDaemonFileRoot(repoRoot)
	return root.UntypedJoin("titand.pid")
}
...
func ExecuteDaemon(ctx context.Context, helper *cmdutil.Helper, signalWatcher *signals.Watcher, executionState *titanstate.ExecutionState) error {
...
var errInactivityTimeout = errors.New("titand shut down from inactivity")
...
func tryAcquirePidfileLock(pidPath titanpath.AbsoluteSystemPath) (lockfile.Lockfile, error) {
...
func (d *daemon) runTitanServer(parentContext context.Context, rpcServer rpcServer, signalWatcher *signals.Watcher) error {
	...
	if err != nil {
		return errors.Wrapf(err, "failed to lock the pid file at %v. Is another titan daemon running?", lock)
	}
...
func GetClient(ctx context.Context, repoRoot titanpath.AbsoluteSystemPath, logger hclog.Logger, titanVersion string, opts ClientOpts) (*Client, error) {
	...
	if strings.HasSuffix(bin, "go-titan") {
		bin = filepath.Join(filepath.Dir(bin), "titan")
	} else if strings.HasSuffix(bin, "go-titan.exe") {
		bin = filepath.Join(filepath.Dir(bin), "titan.exe")
	}

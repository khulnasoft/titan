...
import (
	...
	"github.com/khulnasoft/titan/cli/internal/run"
...
func initializeOutputFiles(helper *cmdutil.Helper, parsedArgs *titanstate.ParsedArgsFromRust) error {
	if parsedArgs.Trace != "" {
...
// RunWithExecutionState runs titan with the ParsedArgsFromRust that is passed from the Rust side.
func RunWithExecutionState(executionState *titanstate.ExecutionState, titanVersion string) int {
	util.InitPrintf()
	// TODO: replace this with a context
	signalWatcher := signals.NewWatcher()
	helper := cmdutil.NewHelper(titanVersion, &executionState.CLIArgs)

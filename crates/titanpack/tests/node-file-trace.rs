...
fn node_file_trace_memory(#[case] input: CaseInput) {
    node_file_trace(
        ...
        1,
        120,
        |_| TitanTasks::new(MemoryBackend::default()),
        |tt| {
            let b = tt.backend();
...
fn node_file_trace_rocksdb(#[case] input: CaseInput) {
    ...
    node_file_trace(
        ...
        240,
        |directory_path| {
            TitanTasks::new(MemoryBackendWithPersistedGraph::new(
...
fn node_file_trace<B: Backend + 'static>(
    ...
    run_count: i32,
    timeout_len: u64,
    create_titan_tasks: impl Fn(&Path) -> Arc<TitanTasks<B>>,
    handle_timeout_error: impl Fn(&Arc<TitanTasks<B>>),
) {
    lazy_static! {
    ...
    r.block_on(async move {
        ...
        for _ in 0..run_count {
            ...
            let tt = create_titan_tasks(directory_path.as_path());

...
Currently `titanpack` splits into `titanpack-core`, `titanpack-css`
, `titanpack-ecmascript` and `titanpack` (facade).

The `titanpack-*` crates (except for `titanpack-core`) add support for a certain
type of asset.
...
- `titanpack-node` (node native modules)
- `titanpack-wasm` (WASM in node and on the web)
- `titanpack-image` (images on the web)
- probably more? (e.g. `titanpack-svg`).

## `titan-tasks`

### `#[titan_tasks::value]`

`#[titan_tasks::value]` is a macro to generate a `XxxVc`
...
`titan-tasks` values can also implement traits,
see `#[titan_tasks::value_trait]` for examples.

### `#[titan_tasks::function]`

`#[titan_tasks::function]` infuses a function with `titan-tasks` magic.
...
- `titan-tasks` will take care of re-executing the function when any dependency
  has changed.

From the outside `#[titan_tasks::function]`s will always return an `XxxVc` (not
a `Result<XxxVc>` or `Future<Output = XxxVc>`).

From the inside you can write an `async fn() -> Result<XxxVc>` and titan-tasks
will hide that async and error in the `XxxVc`.

`titan-tasks` functions are mostly pure. Data is immutable once stored.
...
A similar problem exists with these `#[titan_tasks::function]`. We need a global
...
```rust
// target/debug/build/titan-tasks-{hash}/out/register.rs

{
crate::nothing::NOTHINGVC_IMPL_NEW_FUNCTION.register(r##"titan-tasks@TODO::::nothing::NothingVc::new"##);
crate::display::VALUETOSTRING_TRAIT_TYPE.register(r##"titan-tasks@TODO::::display::ValueToString"##);
crate::primitives::STRING_VALUE_TYPE.register(r##"titan-tasks@TODO::::primitives::String"##);
crate::primitives::BOOL_VALUE_TYPE.register(r##"titan-tasks@TODO::::primitives::Bool"##);
crate::nothing::NOTHING_VALUE_TYPE.register(r##"titan-tasks@TODO::::nothing::Nothing"##);
crate::native_function::NATIVEFUNCTION_VALUE_TYPE.register(r##"titan-tasks@TODO::::native_function::NativeFunction"##);
crate::completion::COMPLETION_VALUE_TYPE.register(r##"titan-tasks@TODO::::completion::Completion"##);
}
```

This code is generated by the build script by looking for
all [`#[titan_tasks::function]`](#titan_tasksfunction)
and [`#[titan_tasks::value]`](#titan_tasksvalue) in the source code.
...
[registry]: https://github.com/khulnasoft/titan/blob/678639772cadac8e96b5ccde3c2865678d1263c1/crates/titan-tasks/src/registry.rs
[new_with_any_serialization]: https://github.com/khulnasoft/titan/blob/678639772cadac8e96b5ccde3c2865678d1263c1/crates/titan-tasks/src/value_type.rs#L138
[any_as_serialize]: https://github.com/khulnasoft/titan/blob/678639772cadac8e96b5ccde3c2865678d1263c1/crates/titan-tasks/src/value_type.rs#L89-L99
[anydeserializeseed]: https://github.com/khulnasoft/titan/blob/678639772cadac8e96b5ccde3c2865678d1263c1/crates/titan-tasks/src/magic_any.rs#L174-L207

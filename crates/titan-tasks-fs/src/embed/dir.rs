...
use anyhow::Result;
use titan_tasks::{TransientInstance, Vc};
...
macro_rules! embed_directory {
    ($name:tt, $path:tt) => {{        // make sure the path contains `$CARGO_MANIFEST_DIR`
        ...
        titan_tasks_fs::embed_directory_internal!($name, $path)
    }};
}
...
macro_rules! embed_directory_internal {
    ($name:tt, $path:tt) => {{
        ...
        use titan_tasks_fs::embed::include_dir;

        let path = $path.replace("$CARGO_MANIFEST_DIR", env!("CARGO_MANIFEST_DIR"));

        titan_tasks_fs::embed::directory_from_relative_path($name.to_string(), path)
    }};
}
...
macro_rules! embed_directory_internal {
    ($name:tt, $path:tt) => {{
        ...
        static dir: include_dir::Dir<'static> = titan_tasks_fs::embed::include_dir!($path);

        titan_tasks_fs::embed::directory_from_include_dir(
            $name.to_string(),
            titan_tasks::TransientInstance::new(&dir),
        )
    }};
}

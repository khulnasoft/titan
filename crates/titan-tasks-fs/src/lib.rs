...
#[titan_tasks::value(cell = "new", eq = "manual")]
...
impl DiskFileSystem {
    ...
    /// has to be called within a titan-tasks function
    fn register_invalidator(&self, path: &Path) -> Result<()> {
        let invalidator = titan_tasks::get_invalidator();
    ...
    /// registers the path as an invalidator for the current task,
    /// has to be called within a titan-tasks function. It removes and returns
...
impl FileSystem for DiskFileSystem {
    ...
    async fn write(
        ...
        // be freed immediately. Given this is an output file, it's unlikely any Titan
...
// Not titan-tasks functions, only delegating
impl FileSystemPath {
...
#[titan_tasks::value(shared, serialization = "none")]
...
pub enum FileLinesContent {
    Lines(#[titan_tasks(trace_ignore)] Vec<FileLine>),
    Unparseable,
    NotFound,
}
...
mod tests {
    ...
    async fn with_extension() {
        ...
        titan_tasks_testing::VcStorage::with(async {

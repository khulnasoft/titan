...
//!
//! ```sh
//! titanpack-convert-trace [/path/to/trace.log]
//! ```
//!
//! ## Options:
//!
...
use titanpack_trace_utils::tracing::{TraceRow, TraceValue};
...
fn main() {
    ...
    let arg = args
        .iter()
        .next()
        .map_or(".titanpack/trace.log", String::as_str);
    ...
    fn get_name<'a>(
        ...
        match &*name {
            ...
            "titan_tasks::resolve_call" | "titan_tasks::resolve_trait_call" => {
                if let Some(v) = values.get("name") {
                    return format!("*{v} ({name})").into();
                }
            }

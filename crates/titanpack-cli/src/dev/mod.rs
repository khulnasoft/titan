...
use titanpack::evaluate_context::node_build_environment;
use titanpack_cli_utils::issue::{ConsoleUi, LogOptions};
use titanpack_core::{
...
use titanpack_dev::DevChunkingContext;
use titanpack_dev_server::{
...
use titanpack_env::dotenv::load_env;
use titanpack_node::execution_context::ExecutionContext;
...
pub(crate) mod titan_tasks_viz;
pub(crate) mod web_entry_source;

pub struct TitanpackDevServerBuilder {
...
impl TitanpackDevServerBuilder {
    pub fn new(
        titan_tasks: Arc<TitanTasks<MemoryBackend>>,
        project_dir: String,
        root_dir: String,
    ) -> TitanpackDevServerBuilder {
        TitanpackDevServerBuilder {
            titan_tasks,
            project_dir,
            root_dir,
    ...
    pub fn entry_request(mut self, entry_asset_path: EntryRequest) -> TitanpackDevServerBuilder {
        self.entry_requests.push(entry_asset_path);
        self
    }

    pub fn eager_compile(mut self, eager_compile: bool) -> TitanpackDevServerBuilder {
        self.eager_compile = eager_compile;
        self
    }

    pub fn hostname(mut self, hostname: IpAddr) -> TitanpackDevServerBuilder {
        self.hostname = Some(hostname);
        self
    }

    pub fn port(mut self, port: u16) -> TitanpackDevServerBuilder {
        self.port = Some(port);
        self
    }

    pub fn browserslist_query(mut self, browserslist_query: String) -> TitanpackDevServerBuilder {
        self.browserslist_query = browserslist_query;
        self
    }

    pub fn log_level(mut self, log_level: IssueSeverity) -> TitanpackDevServerBuilder {
        self.log_level = log_level;
        self
    }

    pub fn show_all(mut self, show_all: bool) -> TitanpackDevServerBuilder {
        self.show_all = show_all;
        self
    }

    pub fn allow_retry(mut self, allow_retry: bool) -> TitanpackDevServerBuilder {
        self.allow_retry = allow_retry;
        self
    }

    pub fn log_detail(mut self, log_detail: bool) -> TitanpackDevServerBuilder {
    ...
    pub async fn build(self) -> Result<DevServer> {
        ...
        let titan_tasks = self.titan_tasks;
...
async fn source(
    ...
    eager_compile: bool,
    titan_tasks: TransientInstance<TitanTasks<MemoryBackend>>,
    browserslist_query: String,
    ...
    let project_path: Vc<titan_tasks_fs::FileSystemPath> = fs.root().join(project_relative);

    let env = load_env(project_path);
    let build_output_root = output_fs.root().join(".titanpack/build".to_string());
    ...
    let viz = Vc::upcast(titan_tasks_viz::TitanTasksSource::new(titan_tasks.into()));
    ...
    let source = Vc::upcast(PrefixedRouterContentSource::new(
        Default::default(),
        vec![
            ("__titanpack__".to_string(), introspect),
            ("__titan_tasks__".to_string(), viz),
        ],
        main_source,
    ));
...
pub async fn start_server(args: &DevArguments) -> Result<()> {
    ...
    let tt = TitanTasks::new(MemoryBackend::new(
        args.common
            .memory_limit
    ...
    let mut server = TitanpackDevServerBuilder::new(tt, project_dir, root_dir)
    ...
    let stats_future = async move {
        if args.common.log_detail {
            println!(
                ...
                memory = FormatBytes(TitanMalloc::memory_usage())
            );
        }
...
async fn profile_timeout<T>(tt: &TitanTasks<MemoryBackend>, future: impl Future<Output = T>) -> T {

...
use titanpack_build::{BuildChunkingContext, MinifyType};
...
pub struct TitanpackBuildBuilder {
...
impl TitanpackBuildBuilder {
    pub fn new(
        titan_tasks: Arc<TitanTasks<MemoryBackend>>,
        project_dir: String,
        root_dir: String,
    ) -> Self {
        TitanpackBuildBuilder {
            titan_tasks,
            project_dir,
            root_dir,
            entry_requests: vec![],
    ...
    pub async fn build(self) -> Result<()> {
        let task = self.titan_tasks.spawn_once_task::<(), _>(async move {
        ...
        self.titan_tasks.wait_task_completion(task, true).await?;
...
pub async fn build(args: &BuildArguments) -> Result<()> {
    ...
    let mut builder = TitanpackBuildBuilder::new(tt, project_dir, root_dir)

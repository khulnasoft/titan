...
pub use references::{AnalyzeEcmascriptModuleResult, TITANPACK_HELPER};
...
pub struct EcmascriptOptions {
    ...
    /// External imports should used `__titanpack_import__` instead of
    /// `__titanpack_require__` and become async module references.
...
pub struct EcmascriptModuleAsset {
    ...
    #[serde(skip)]
    last_successful_analysis: titan_tasks::State<Option<MemoizedSuccessfulAnalysis>>,
}

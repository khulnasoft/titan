...
pub(crate) async fn analyse_ecmascript_module_internal(
    ...
    for r in import_references.iter_mut() {
        ...
        // passing that to other titan tasks functions later.
        *r = r.resolve().await?;
    }
        ...
        match effect {
            ...
            Effect::ImportedBinding {
                ...
                if let Some(r) = import_references.get(esm_reference_index) {
                    if let Some("__titanpack_module_id__") = export.as_deref() {
                        analysis
...
#[titan_tasks::value(transparent, serialization = "none")]
pub struct AstPath(#[titan_tasks(trace_ignore)] Vec<AstParentKind>);

pub static TITANPACK_HELPER: &str = "__titanpackHelper";

pub fn is_titanpack_helper_import(import: &ImportDecl) -> bool {
    import.with.as_ref().map_or(false, |asserts| {
        asserts.props.iter().any(|assert| {
            assert
                ...
                .map_or(false, |ident| &*ident.sym == TITANPACK_HELPER)
        })
    })
}
...
fn detect_dynamic_export(p: &Program) -> DetectedDynamicExportType {
    ...
    if let Program::Module(m) = p {
        ...
        if m.body.iter().any(|item| {
            item.as_module_decl().map_or(false, |module_decl| {
                module_decl
                    .as_import()
                    .map_or(true, |import| !is_titanpack_helper_import(import))
            })
        }) {
    ...
    impl Visit for Visitor {
        ...
        fn visit_ident(&mut self, i: &Ident) {
            ...
            if &*i.sym == "__titanpack_export_value__" {
                self.value = true;
                self.found = true;
            }
            if &*i.sym == "__titanpack_export_namespace__" {
                self.namespace = true;
                self.found = true;
            }
        }

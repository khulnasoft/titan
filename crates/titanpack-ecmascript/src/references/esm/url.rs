...
impl CodeGenerateable for UrlAssetReference {
    ...
    ├───────────────────────────────┼─────────────────────────────────────────────────────────────────────────┼────────────────────────────────────────────────┼───────────────────────┤
    │ Relative                      │ __titanpack_relative_url__(__titanpack_require__(urlId))                │ __titanpack_relative_url__(url)                │ new URL(url, base)    │
    │ Full(RenderingClient::Client) │ new URL(__titanpack_require__(urlId), location.origin)                  │ new URL(url, location.origin)                  │ new URL(url, base)    │
    │ Full(RenderingClient::..)     │ new URL(__titanpack_resolve_module_id_path__(urlId))                    │ new URL(url, base)                             │ new URL(url, base)    │
    ...
    async fn code_generation(
        ...
        match rewrite_behavior {
            UrlRewriteBehavior::Relative => {
                ...
                // if the referenced url is in the module graph of titanpack, replace it into
                // the chunk item will be emitted into output path to point the
                // static asset path. for the `new URL()` call, replace it into
                // pseudo url object `__titanpack_relative_url__`
                // which is injected by titanpack's runtime to resolve into the relative path
                // omitting the base.
                match &*referenced_asset {
                    ReferencedAsset::Some(asset) => {
                        ...
                        visitors.push(create_visitor!(ast_path, visit_mut_expr(new_expr: &mut Expr) {
                            ...
                            if should_rewrite_to_relative {
                                *new_expr = quote!(
                                    "new __titanpack_relative_url__(__titanpack_require__($id))" as Expr,
                    ...
                    ReferencedAsset::OriginalReferenceTypeExternal(request) => {
                        ...
                        visitors.push(create_visitor!(ast_path, visit_mut_expr(new_expr: &mut Expr) {
                            ...
                            if should_rewrite_to_relative {
                                *new_expr = quote!(
                                    "new __titanpack_relative_url__($id)" as Expr,
            ...
            UrlRewriteBehavior::Full => {
                ...
                // the dev server.
                //
                // By default for the remaining environments, titanpack's runtime have overriden
                // `import.meta.url`.
                ...
                match &*referenced_asset {
                    ReferencedAsset::Some(asset) => {
                        ...
                        // with __titanpack_require__ which returns the asset_url.
                        //
                        // Otherwise, the envioronment should provide an absolute path to the actual
                        // output asset; delegate those calculation to the
                        // runtime fn __titanpack_resolve_module_id_path__.
                        let url_segment_resolver = if rewrite_url_base.is_some() {
                            quote!(
                                "__titanpack_require__($id)" as Expr,
                                id: Expr = module_id_to_lit(&id),
                            )
                        } else {
                            quote!(
                                "__titanpack_resolve_module_id_path__($id)" as Expr,
                                id: Expr = module_id_to_lit(&id),
                            )
                        };
                                ...
                                if let Some(ExprOrSpread { box expr, spread: None }) = args.get_mut(1) {
                                    if let Some(rewrite) = &rewrite_url_base {
                                        *expr = rewrite.clone();
                                        ...
                                        // If rewrite for the base doesn't exists, means __titanpack_resolve_module_id_path__

...
pub trait SourceProvider: Send + Clone + 'static {
    /// must call a titan-tasks function internally
                        ...
                        run_once_with_reason(tt.clone(), reason, async move {
                            ...
                            if hyper_tungstenite::is_upgrade_request(&request) {
                                ...
                                if path == "/_next/webpack-hmr" {
                                    ...
                                    // without titanpack, which may be making requests in
                                    // development.
                                    println!(
                                        "A non-titanpack next.js client is trying to connect."
                                    );
                                    println!(
                                        "Make sure to reload/close any browser window which has \
                                         been opened without --titan."
                                    );
                                }

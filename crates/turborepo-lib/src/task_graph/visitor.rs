...
pub enum Error {
    ...
    #[error(
        "root task {task_name} ({command}) looks like it invokes titan and might cause a loop"
    )]
    RecursiveTitan { task_name: String, command: String },
...
impl<'a> Visitor<'a> {
    ...
    pub async fn visit(&self, engine: Arc<Engine>) -> Result<Vec<TaskError>, Error> {
        ...
        while let Some(message) = node_stream.recv().await {
            ...
            match command {
                Some(cmd) if info.package() == ROOT_PKG_NAME && titan_regex().is_match(&cmd) => {
                    package_task_event.track_recursive_error();
                    return Err(Error::RecursiveTitan {
    ...
    fn output_client(&self, task_id: &TaskId) -> OutputClient<impl std::io::Write> {
        let behavior = match self.opts.run_opts.log_order {
            crate::opts::ResolvedLogOrder::Stream if self.run_tracker.spaces_enabled() => {
                titanrepo_ui::OutputClientBehavior::InMemoryBuffer
            }
            crate::opts::ResolvedLogOrder::Stream => {
                titanrepo_ui::OutputClientBehavior::Passthrough
            }
            crate::opts::ResolvedLogOrder::Grouped => titanrepo_ui::OutputClientBehavior::Grouped,
        };
...
fn titan_regex() -> &'static Regex {
    static RE: OnceLock<Regex> = OnceLock::new();
    RE.get_or_init(|| Regex::new(r"(?:^|\s)titan(?:$|\s)").unwrap())
}
...
impl ExecContext {
    ...
    async fn execute_inner(
        ...
        cmd.env("TITAN_HASH", &self.task_hash);
        ...
        let mut process = match self.manager.spawn(cmd, Duration::from_millis(500)) {
            Some(Ok(child)) => child,
            // Titan was unable to spawn a process
            Some(Err(e)) => {

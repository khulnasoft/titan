...
use serde::Serialize;
use titanpath::{AnchoredSystemPathBuf, RelativeUnixPathBuf};
use titanrepo_cache::CacheHitMetadata;
use titanrepo_env::{DetailedMap, EnvironmentVariableMap};
...
impl From<titanrepo_cache::CacheSource> for CacheSource {
    fn from(value: titanrepo_cache::CacheSource) -> Self {
        match value {
            titanrepo_cache::CacheSource::Local => Self::Local,
            titanrepo_cache::CacheSource::Remote => Self::Remote,
        }
    }
}
...
impl TaskEnvVarSummary {
    pub fn new(
    ...
    ) -> Result<Self, titanrepo_env::Error> {
        ...
        let pass_through = task_definition
            .pass_through_env
            .as_deref()
            .map(|pass_through_env| -> Result<_, titanrepo_env::Error> {

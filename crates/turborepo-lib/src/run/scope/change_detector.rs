...
use titanrepo_repository::package_graph::{ChangedPackagesError, PackageGraph, WorkspaceName};
...
impl<'a> PackageChangeDetector for SCMChangeDetector<'a> {
    fn changed_packages(
        ...
        if !from_ref.is_empty() {
            changed_files = self
                .scm
                .changed_files(self.titan_root, Some(from_ref), to_ref)?;
        }
...
impl<'a> SCMChangeDetector<'a> {
    const DEFAULT_GLOBAL_DEPS: [&'static str; 2] = ["package.json", "titan.json"];
    ...
    ) -> Result<bool, titanrepo_scm::Error> {
    ...
    ) -> Result<HashSet<&'b AnchoredSystemPathBuf>, titanrepo_scm::Error> {
    ...
    fn get_changed_packages<'b>(
        ...
        graph: &PackageGraph,
    ) -> Result<HashSet<WorkspaceName>, titanrepo_scm::Error> {
    ...
    fn get_changes_from_lockfile(
        ...
        let lockfile_path = self
            .pkg_graph
            .package_manager()
            .lockfile_path(self.titan_root);

        // no changes to lockfile? then no changes because of lockfile
        if !SCMChangeDetector::lockfile_changed(self.titan_root, changed_files, &lockfile_path)? {
            return Ok(vec![]);
        }
    ...
    fn lockfile_changed(
        ...
        let lockfile_path_relative = titan_root
...
pub enum ChangeDetectError {
    ...
    Lockfile(titanrepo_lockfiles::Error),
}
...
mod test {
    ...
    fn test_lockfile_changed(
        ...
        let titan_root = titanpath::AbsoluteSystemPathBuf::new(titan_root).unwrap();
        let lockfile_path = titanpath::AbsoluteSystemPathBuf::new(lockfile_path).unwrap();
        let changed_files = changed_files
            .iter()
            .map(|s| titanpath::AnchoredSystemPathBuf::from_raw(s).unwrap())
            .collect();
        let changes =
            SCMChangeDetector::lockfile_changed(&titan_root, &changed_files, &lockfile_path);

...
use titanpath::{AbsoluteSystemPath, RelativeUnixPathBuf};
use titanrepo_repository::{package_graph::ROOT_PKG_NAME, package_json::PackageJson};
...
#[serde(rename_all = "camelCase")]
// The raw deserialized titan.json file.
pub struct RawTitanJson {
...
const CONFIG_FILE: &str = "titan.json";
                ...
                if let Some(dependency) = dependency.strip_prefix(ENV_PIPELINE_DELIMITER) {
                    println!(
                        "[DEPRECATED] Declaring an environment variable in \"dependsOn\" is \
                         deprecated, found {}. Use the \"env\" key or use `npx @titan/codemod \
...
impl RawTitanJson {
    pub(crate) fn read(path: &AbsoluteSystemPath) -> Result<RawTitanJson, Error> {
        let contents = path.read()?;
        let raw_titan_json: RawTitanJson =
            serde_json::from_reader(json_comments::StripComments::new(contents.as_slice()))?;

        Ok(raw_titan_json)
    }

    /// Produces a new titan.json without any tasks that reference non-existent
    /// workspaces
...
impl TryFrom<RawTitanJson> for TitanJson {
    type Error = Error;

    fn try_from(raw_titan: RawTitanJson) -> Result<Self, Error> {
        let mut global_env = HashSet::new();
        let mut global_file_dependencies = HashSet::new();

        if let Some(global_env_from_titan) = raw_titan.global_env {
            gather_env_vars(global_env_from_titan, "globalEnv", &mut global_env)?;
        }

        // TODO: In the rust port, warnings should be refactored to a post-parse
        // validation step
        for value in raw_titan.global_dependencies.into_iter().flatten() {
        ...
        Ok(TitanJson {
            ...
            global_pass_through_env: raw_titan
            ...
            global_dot_env: raw_titan
                ...
                .transpose()?,
            pipeline: raw_titan.pipeline.unwrap_or_default(),
            // copy these over, we don't need any changes here.
            remote_cache: raw_titan.remote_cache,
            extends: raw_titan.extends.unwrap_or_default(),
            // Directly to space_id, we don't need to keep the struct
            space_id: raw_titan.experimental_spaces.and_then(|s| s.id),
        })
    }
}

impl TitanJson {
    /// Loads titan.json by reading the file at `dir` and optionally combining
    ...
    pub fn load(
        ...
        if root_package_json.legacy_titan_config.is_some() {
            println!(
                "[WARNING] \"titan\" in package.json is no longer supported. Migrate to {} by \
                 running \"npx @titan/codemod create-titan-config\"\n",
                CONFIG_FILE
            );
        }

        let titan_from_files = Self::read(&dir.join_component(CONFIG_FILE));

        let mut titan_json = match (include_synthesized_from_root_package_json, titan_from_files) {
            // If the file didn't exist, throw a custom error here instead of propagating
            (false, Err(Error::Io(_))) => return Err(Error::NoTitanJSON),
            ...
            // tasks
            (true, Ok(mut titan_from_files)) => {
                let mut pipeline = Pipeline::default();
                for (task_name, task_definition) in titan_from_files.pipeline {
                ...
                titan_from_files.pipeline = pipeline;
    ...
    /// Reads a `RawTitanJson` from the given path
    /// and then converts it into `TitanJson`
    pub(crate) fn read(path: &AbsoluteSystemPath) -> Result<TitanJson, Error> {
        let raw_titan_json = RawTitanJson::read(path)?;
        raw_titan_json.try_into()
    }
    ...
    pub fn validate(&self, validations: &[TitanJSONValidation]) -> Vec<Error> {
        validations
            .iter()
...
type TitanJSONValidation = fn(&TitanJson) -> Vec<Error>;

pub fn validate_no_package_task_syntax(titan_json: &TitanJson) -> Vec<Error> {
    titan_json
        .pipeline
        .keys()
...
pub fn validate_extends(titan_json: &TitanJson) -> Vec<Error> {
    match titan_json.extends.first() {
        Some(package_name) if package_name != ROOT_PKG_NAME || titan_json.extends.len() > 1 => {
...
mod tests {
    ...
    use titanpath::{AbsoluteSystemPath, RelativeUnixPathBuf};
    use titanrepo_repository::package_json::PackageJson;

    use super::RawTitanJson;
    use crate::{
        cli::OutputLogsMode,
        config::{
            titan::{Pipeline, RawTaskDefinition},
            TitanJson,
        },
    ...
    fn test_get_root_titan_no_synthesizing(
        ...
        fs::write(repo_root.join_component("titan.json"), titan_json_content)?;

        let titan_json = TitanJson::load(repo_root, &root_package_json, false)?;
    ...
    #[test_case(
        Some("{}"),
        PackageJson {
            legacy_titan_config: Some(serde_json::Value::String("build".to_string())),
            ..PackageJson::default()
        },
        TitanJson::default()
    )]
    ...
    fn test_get_root_titan_with_synthesizing(
        ...
        if let Some(content) = titan_json_content {
            fs::write(repo_root.join_component("titan.json"), content)?;
        }

        let titan_json = TitanJson::load(repo_root, &root_package_json, true)?;
        assert_eq!(titan_json, expected_titan_json);
    ...
    #[test]
    fn test_titan_task_pruning() {
        let json: RawTitanJson = serde_json::from_value(serde_json::json!({
            "pipeline": {
                "//#top": {},
                "build": {},
                "a#build": {},
        ...
        let expected: RawTitanJson = serde_json::from_value(serde_json::json!({
            "pipeline": {
                "//#top": {},
                "build": {},
                "a#build": {},
    ...
    fn test_parsing_output_mode(output_mode: &str, expected: Option<OutputLogsMode>) {
        let json: Result<RawTitanJson, _> = serde_json::from_value(serde_json::json!({
            "pipeline": {
                "build": {

...
pub struct DaemonClient<T> {
    client: proto::titand_client::TitandClient<tonic::transport::Channel>,
    connect_settings: T,
}

impl DaemonClient<()> {
    pub fn new(client: proto::titand_client::TitandClient<tonic::transport::Channel>) -> Self {
        Self {
            client,
            connect_settings: (),
        }
...
impl DaemonClient<DaemonConnector> {
    ...
    pub fn pid_file(&self) -> &titanpath::AbsoluteSystemPathBuf {
        &self.connect_settings.pid_file
    }

    pub fn sock_file(&self) -> &titanpath::AbsoluteSystemPathBuf {
...
pub enum DaemonError {
    ...
    /// The server is running a different version of titanrepo.
...
mod test {
    ...
    fn test_format_repo_relative_glob() {
        let raw_glob = ["some", ".titan", "titan-foo:bar.log"].join(MAIN_SEPARATOR_STR);
        #[cfg(windows)]
        let expected = "some/.titan/titan-foo";
        #[cfg(not(windows))]
        let expected = "some/.titan/titan-foo\\:bar.log";

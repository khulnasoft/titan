...
fn unlink_spaces(base: &mut CommandBase) -> Result<(), cli::Error> {
    ...
    // Space config is _also_ in titan.json.
    let result = remove_spaces_from_titan_json(base)?;
...
fn remove_spaces_from_titan_json(base: &CommandBase) -> Result<UnlinkSpacesResult, Error> {
    let titan_json_path = base.repo_root.join_component("titan.json");
    let titan_json =
        fs::read_to_string(&titan_json_path).map_err(|e| config::Error::FailedToReadConfig {
            config_path: titan_json_path.clone(),
            error: e,
        })?;

    let output = unset_path(&titan_json, &["experimentalSpaces", "id"], true)?;
    if let Some(output) = output {
        fs::write(&titan_json_path, output).map_err(|e| config::Error::FailedToSetConfig {
            config_path: titan_json_path.clone(),
            error: e,
        })?;

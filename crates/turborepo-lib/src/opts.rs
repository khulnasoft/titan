...
use thiserror::Error;
use titanpath::AnchoredSystemPathBuf;
use titanrepo_cache::CacheOpts;
...
impl<'a> Opts<'a> {
    pub fn synthesize_command(&self) -> String {
        let mut cmd = format!("titan run {}", self.run_opts.tasks.join(" "));
...
impl<'a> TryFrom<&'a RunArgs> for RunOpts<'a> {
    ...
    fn try_from(args: &'a RunArgs) -> Result<Self, Self::Error> {
        ...
        let (is_github_actions, log_order, log_prefix) = match args.log_order {
            LogOrder::Auto if titanrepo_ci::Vendor::get_constant() == Some("GITHUB_ACTIONS") => (
                true,
...
mod test {
    ...
    #[test_case(
        ...
        "titan run build --filter=my-app -- -v --foo=bar"
    )]
    #[test_case(
        TestCaseOpts {
    ...
    #[test_case(
        ...
        "titan run build --filter=other-app --filter=my-app -- -v --foo=bar"
    )]
    #[test_case    (
        TestCaseOpts {
            legacy_filter: Some(LegacyFilter {
        ...
        "titan run build --filter=other-app --filter=...my-app...[some-ref]..."
    )]
    #[test_case    (
        TestCaseOpts {
            filter_patterns: vec!["my-app".to_string()],
        ...
        "titan run build --filter=my-app --parallel --continue"
    )]
    #[test_case    (
        TestCaseOpts {
            filter_patterns: vec!["my-app".to_string()],
        ...
        "titan run build --filter=my-app --dry"
    )]
    #[test_case    (
        TestCaseOpts {
            filter_patterns: vec!["my-app".to_string()],
        ...
        "titan run build --filter=my-app --dry=json"
    )]

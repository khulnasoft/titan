                ...
                for arg in &args.args {
                    match arg {
                        GenericArgument::Type(ty) => {
                            if let Type::Path(type_path) = ty {
                                ...
                                arg.span()
                                    .unwrap()
                                    .error(
                                        "#[titan_tasks::value_impl] does not support this type \
                                         argument",
                                    )
                                    .emit();
                            }
                        ...
                        _ => arg
                            .span()
                            .unwrap()
                            .error("#[titan_tasks::value_impl] does not support this type argument")
                            .emit(),
                    }
                }
            }
    ...
    match ty {
        ...
        _ => {
            ty.span()
                .unwrap()
                .error(format!(
                    "#[titan_tasks::value_impl] does not support the type {}, expected T or \
                     Box<dyn Trait>",
                    ty.to_token_stream()
                ))
                .emit();

...
impl SCM {
    pub fn get_hashes_for_files(
        ...
        if allow_missing {
            self.hash_existing_of(titan_root, files.iter())
        } else {
            self.hash_files(titan_root, files.iter())
        }
    }

    #[tracing::instrument(skip(self, titan_root, package_path, inputs))]
    pub fn get_package_file_hashes<S: AsRef<str>>(
        ...
        match self {
            ...
            SCM::Git(git) => git
                .get_package_file_hashes(titan_root, package_path, inputs)
                .or_else(|e| {
                    debug!(
    ...
    pub fn hash_files(
        ...
        match self {
            SCM::Manual => crate::manual::hash_files(titan_root, files, false),
            SCM::Git(git) => git.hash_files(titan_root, files),
        }
    }
    ...
    pub fn hash_existing_of(
        ...
        crate::manual::hash_files(titan_root, files, true)
    }
}
...
impl Git {
    fn get_package_file_hashes<S: AsRef<str>>(
        ...
        if inputs.is_empty() {
            self.get_package_file_hashes_from_index(titan_root, package_path)
        } else {
            self.get_package_file_hashes_from_inputs(titan_root, package_path, inputs)
        }
    }

    #[tracing::instrument(skip(self, titan_root))]
    fn get_package_file_hashes_from_index(
        &self,
        titan_root: &AbsoluteSystemPath,
        package_path: &AnchoredSystemPath,
    ) -> Result<GitHashes, Error> {
        let full_pkg_path = titan_root.resolve(package_path);
    ...
    #[tracing::instrument(skip(self, titan_root, inputs))]
    fn get_package_file_hashes_from_inputs<S: AsRef<str>>(
        ...
        let mut inputs = inputs
            ...
            .collect::<Vec<String>>();
        // Add in package.json and titan.json to input patterns. Both file paths are
        // relative to pkgPath
        //
        // - package.json is an input because if the `scripts` in the package.json
        //   change (i.e. the tasks that titan executes), we want a cache miss, since
        //   any existing cache could be invalid.
        // - titan.json because it's the definition of the tasks themselves. The root
        //   titan.json is similarly included in the global hash. This file may not
        //   exist in the workspace, but that is ok, because it will get ignored
        //   downstream.
        inputs.push("package.json".to_string());
        inputs.push("titan.json".to_string());

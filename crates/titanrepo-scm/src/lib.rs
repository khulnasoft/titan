...
//! Titanrepo's library for interacting with source control management (SCM).
...
fn find_git_root(titan_root: &AbsoluteSystemPath) -> Result<AbsoluteSystemPathBuf, Error> {
    let rev_parse = Command::new("git")
        .args(["rev-parse", "--show-cdup"])
        .current_dir(titan_root)
        .output()?;
    ...
    if let Some(line) = lines.next() {
        ...
        titan_root.join_unix_path(tail).map_err(|e| e.into())
    } else {
        let stderr = String::from_utf8_lossy(&rev_parse.stderr);
...
mod tests {
    ...
    fn test_symlinked_git_root() {
        ...
        let titan_root = link.join_component("inside");
        let result = find_git_root(&titan_root).unwrap();

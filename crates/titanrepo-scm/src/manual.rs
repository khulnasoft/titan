...
use titanpath::{AbsoluteSystemPath, AnchoredSystemPath, IntoUnix};
...
pub(crate) fn get_package_file_hashes_from_processing_gitignore<S: AsRef<str>>(
    ...
    let full_package_path = titan_root.resolve(package_path);
    let mut hashes = GitHashes::new();
...
mod tests {
    ...
    fn test_hash_files(files: &[&str], allow_missing: bool, want_err: bool) {
        let (_tmp, titan_root) = tmp_dir();
        let test_file = titan_root.join_component("existing-file.txt");
        ...
        match hash_files(&titan_root, files, allow_missing) {
    ...
    fn test_hash_symlink() {
        let (_tmp, titan_root) = tmp_dir();
        let from_to_file = titan_root.join_component("symlink-from-to-file");
        let from_to_dir = titan_root.join_component("symlink-from-to-dir");
        let broken = titan_root.join_component("symlink-broken");

        let to_file = titan_root.join_component("the-file-target");
        to_file.create_with_contents("contents").unwrap();

        let to_dir = titan_root.join_component("the-dir-target");
    ...
    fn test_get_package_file_hashes_from_processing_gitignore() {
        ...
        let root_ignore_file = titan_root.join_component(".gitignore");
        root_ignore_file
            .create_with_contents(root_ignore_contents)
            .unwrap();
        let pkg_ignore_file = titan_root.resolve(&pkg_path).join_component(".gitignore");
        ...
        for (raw_unix_path, contents, expected_hash) in file_hash.iter() {
            ...
            let file_path = titan_root.join_unix_path(unix_path).unwrap();
        ...
        let hashes =
            get_package_file_hashes_from_processing_gitignore::<&str>(&titan_root, &pkg_path, &[])
                .unwrap();

...
impl SCM {
    ...
    pub fn changed_files(
        ...
        match self {
            Self::Git(git) => git.changed_files(titan_root, from_commit, to_commit),
            Self::Manual => Err(Error::GitRequired(titan_root.to_owned())),
        }
    }
...
pub fn changed_files(
    ...
    let titan_root = AbsoluteSystemPathBuf::try_from(titan_root.as_path())?;
    let files = scm.changed_files(&titan_root, from_commit, to_commit)?;
    Ok(files
        .into_iter()
        .map(|f| f.to_string())
        .collect::<HashSet<_>>())
...
impl Git {
    ...
    fn changed_files(
        ...
        let titan_root_relative_to_git_root = self.root.anchor(titan_root)?;
        let pathspec = titan_root_relative_to_git_root.as_str();

        let mut files = HashSet::new();

        let output = self.execute_git_command(&["diff", "--name-only", to_commit], pathspec)?;

        self.add_files_from_stdout(&mut files, titan_root, output);
    ...
    fn add_files_from_stdout(
        ...
        for line in stdout.lines() {
            ...
            let anchored_to_titan_root_file_path = self
                .reanchor_path_from_git_root_to_titan_root(titan_root, path)
                .unwrap();
            files.insert(anchored_to_titan_root_file_path);
        }
    }

    fn reanchor_path_from_git_root_to_titan_root(
        &self,
        titan_root: &AbsoluteSystemPath,
        path: &RelativeUnixPath,
    ) -> Result<AnchoredSystemPathBuf, Error> {
        let absolute_file_path = self.root.join_unix_path(path)?;
        let anchored_to_titan_root_file_path = titan_root.anchor(&absolute_file_path)?;
        Ok(anchored_to_titan_root_file_path)
    }
...
mod tests {
    ...
    use titanpath::{AbsoluteSystemPathBuf, PathError};
    use which::which;
    ...
    fn test_deleted_files() -> Result<(), Error> {
        ...
        let titanrepo_root = repo_root.path().to_owned();
        let files = changed_files(git_root, titanrepo_root, Some(&first_commit_sha), "HEAD")?;
    ...
    fn test_changed_files() -> Result<(), Error> {
        ...
        // Test that `titan_root` filters out files not in the specified directory
        let files = changed_files(
            repo_root.path().to_path_buf(),
    ...
    // Tests that we can use a subdir as the titan_root path
    // (occurs when the monorepo is nested inside a subdirectory of git repository)
    #[test]
    fn test_changed_files_with_subdir_as_titan_root() -> Result<(), Error> {
        let (repo_root, repo) = setup_repository()?;
    ...
    fn test_error_cases() -> Result<(), Error> {
        ...
        let titan_root = tempfile::tempdir()?;
        let titan_root_is_not_subdir_of_git_root = changed_files(
            repo_root.path().to_path_buf(),
            titan_root.path().to_path_buf(),
            None,
            "HEAD",
        );

        assert_matches!(
            titan_root_is_not_subdir_of_git_root,

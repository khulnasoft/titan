...
use titan_tasks::Vc;
use titan_tasks_fetch::{fetch, register, FetchErrorKind};
use titan_tasks_fs::{DiskFileSystem, FileSystem, FileSystemPath};
use titan_tasks_testing::{register, run};
use titanpack_core::issue::{Issue, IssueSeverity, StyledString};
...
async fn invalidation_does_not_invalidate() {
    run! {
        ...
        // Assert that a second request is never sent -- the result is cached via titan tasks

...
use thiserror::Error;
use titanpath::{AbsoluteSystemPath, AbsoluteSystemPathBuf, PathError, RelativeUnixPath};
...
impl Display for NoPackageManager {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        or run `npx @titan/codemod add-package-manager` in the root of your monorepo.")
    }
}
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let err = match self.package_manager {
            PackageManager::Pnpm | PackageManager::Pnpm6 => {
                "pnpm-workspace.yaml: no packages found. Titanrepo requires pnpm workspaces and \
                 thus packages to be defined in the root pnpm-workspace.yaml"
            }
            PackageManager::Yarn | PackageManager::Berry => {
                "package.json: no workspaces found. Titanrepo requires yarn workspaces to be \
                 defined in the root package.json"
            }
            PackageManager::Npm => {
                "package.json: no workspaces found. Titanrepo requires npm workspaces to be \
                 defined in the root package.json"
            }
            PackageManager::Bun => {
                "package.json: no workspaces found. Titanrepo requires bun workspaces to be \
                 defined in the root package.json"
            }
        };
        write!(f, "{}", err)
...
impl PackageManager {
    ...
    pub fn parse_lockfile(
        ...
        Ok(match self {
            PackageManager::Npm => Box::new(titanrepo_lockfiles::NpmLockfile::load(contents)?),
            PackageManager::Pnpm | PackageManager::Pnpm6 => {
                Box::new(titanrepo_lockfiles::PnpmLockfile::from_bytes(contents)?)
            }
            PackageManager::Yarn => {
                Box::new(titanrepo_lockfiles::Yarn1Lockfile::from_bytes(contents)?)
            }
            PackageManager::Bun => {
                Box::new(titanrepo_lockfiles::BunLockfile::from_bytes(contents)?)
            }
            PackageManager::Berry => Box::new(titanrepo_lockfiles::BerryLockfile::load(
                contents,
                Some(titanrepo_lockfiles::BerryManifest::with_resolutions(
                    root_package_json
                        .resolutions
                        .iter()
                        .flatten()
    ...
    pub fn lockfile_path(&self, titan_root: &AbsoluteSystemPath) -> AbsoluteSystemPathBuf {
        titan_root.join_component(self.lockfile_name())
    }
...
mod tests {
    ...
    fn repo_root() -> AbsoluteSystemPathBuf {
        ...
        panic!("Couldn't find Titanrepo root from {}", cwd);
    }

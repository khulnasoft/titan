...
pub struct PackageJson {
    ...
    #[serde(rename = "titan", skip_serializing_if = "Option::is_none")]
    pub legacy_titan_config: Option<serde_json::Value>,
...
mod test {
    ...
    #[test]
    fn test_legacy_titan_config() -> Result<()> {
        let contents = r#"{"titan": {}}"#;
        let package_json = serde_json::from_str::<PackageJson>(contents)?;

        assert!(package_json.legacy_titan_config.is_some());

        let contents = r#"{"titan": { "globalDependencies": [".env"] } }"#;
        let package_json = serde_json::from_str::<PackageJson>(contents)?;

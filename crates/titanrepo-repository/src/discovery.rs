...
pub struct WorkspaceData {
    ...
    pub titan_json: Option<AbsoluteSystemPathBuf>,
}
    ...
    async fn discover_packages(&mut self) -> Result<DiscoveryResponse, Error> {
        ...
        iter(package_paths)
            .then(|path| async move {
                let potential_titan = path
                    .parent()
                    .expect("non-root")
                    .join_component("titan.json");
                let potential_titan_exists = tokio::fs::try_exists(potential_titan.as_path()).await;

                Ok(WorkspaceData {
                    package_json: path,
                    titan_json: potential_titan_exists
                        .unwrap_or_default()
                        .then_some(potential_titan),
                })
            })

...
// example)
static TITAN_PURE_OUTPUT_ARGS: [&str; 6] = [
    "--json",
    "--dry",
    "--dry-run",
    "--dry=json",
...
static TITAN_SKIP_NOTIFIER_ARGS: [&str; 5] =
    ["--help", "--h", "--version", "--v", "--no-update-notifier"];

fn titan_version_has_shim(version: &str) -> bool {
...
struct ShimArgs {
    ...
    remaining_titan_args: Vec<String>,
...
impl ShimArgs {
    pub fn parse() -> Result<Self, Error> {
        ...
        let mut remaining_titan_args = Vec::new();
    ...
    fn has_json_flags(&self) -> bool {
        self.remaining_titan_args
            .iter()
            .any(|arg| TITAN_PURE_OUTPUT_ARGS.contains(&arg.as_str()))
    }
    ...
    fn has_notifier_skip_flags(&self) -> bool {
        self.remaining_titan_args
            .iter()
            .any(|arg| TITAN_SKIP_NOTIFIER_ARGS.contains(&arg.as_str()))
    }
...
#[derive(Debug)]
pub struct LocalTitanState {
    bin_path: PathBuf,
    version: String,
}

impl LocalTitanState {
    // Hoisted strategy:
    // - `bun install`
    // - `npm install`
    // - `yarn`
    ...
    fn generate_nested_path(root_path: &AbsoluteSystemPath) -> Option<AbsoluteSystemPathBuf> {
        Some(root_path.join_components(&["node_modules", "titan", "node_modules"]))
    }
    ...
    fn generate_linked_path(root_path: &AbsoluteSystemPath) -> Option<AbsoluteSystemPathBuf> {
        // root_path/node_modules/titan is a symlink. Canonicalize the symlink to what
        ...
        // well.
        let canonical_path =
            fs_canonicalize(root_path.as_path().join("node_modules").join("titan")).ok()?;
    ...
    fn generate_unplugged_path(root_path: &AbsoluteSystemPath) -> Option<AbsoluteSystemPathBuf> {
        let platform_package_name = TitanState::platform_package_name();
    ...
    // We support six per-platform packages and one `titan` package which handles
    ...
    // _worst_ outcome is that we run global `titan`.
    //
    ...
    pub fn infer(root_path: &AbsoluteSystemPath) -> Option<Self> {
        let platform_package_name = TitanState::platform_package_name();
        let binary_name = TitanState::binary_name();
            ...
            .filter_map(|search_function| search_function(root_path))
        {
            ...
            match fs_canonicalize(&bin_path) {
                Ok(bin_path) => {
                    ...
                    debug!("Local titan path: {}", bin_path.display());
                    debug!("Local titan version: {}", &local_version);
                    return Some(Self {
                        bin_path,
                        version: local_version,
                    });
                }
                Err(_) => debug!("No local titan binary found at: {}", bin_path),
            }
        }
    ...
    fn supports_skip_infer_and_single_package(&self) -> bool {
        titan_version_has_shim(&self.version)
    }
...
fn run_correct_titan(
    ...
    if let Some(titan_state) = LocalTitanState::infer(&repo_state.root) {
        ...
        if titan_state.local_is_self() {
            ...
            Ok(Payload::Rust(spawn_local_titan(
                &repo_state,
                titan_state,
                shim_args,
            )))
...
fn spawn_local_titan(
    ...
    let local_titan_path = fs_canonicalize(&local_titan_state.bin_path).map_err(|_| {
        Error::LocalTitanPath(local_titan_state.bin_path.to_string_lossy().to_string())
    })?;
    debug!(
    ...
    let supports_skip_infer_and_single_package =
        local_titan_state.supports_skip_infer_and_single_package();
    ...
    let child = spawn_child(command).map_err(Error::LocalTitanProcess)?;

    let exit_status = child.wait().map_err(Error::LocalTitanProcess)?;
    let exit_code = exit_status.code().unwrap_or_else(|| {
        debug!("go-titan failed to report exit code");
        #[cfg(unix)]
        {
            ...
            debug!(
                "go-titan caught signal {:?}. Core dumped? {}",
                signal, core_dumped
            );
        }
        2
...
fn try_check_for_updates(args: &ShimArgs, current_version: &str) {
    if args.should_check_for_update() {
        ...
        let _ = check_for_updates(
            "titan",
            "https://github.com/khulnasoft/titan",
            Some(&footer),
            current_version,
...
pub fn run() -> Result<Payload, Error> {
    ...
    let subscriber = TitanSubscriber::new_with_verbosity(args.verbosity, &ui);
    ...
    match RepoState::infer(&args.cwd) {
        Ok(repo_state) => {
            debug!("Repository Root: {}", repo_state.root);
            run_correct_titan(repo_state, args, &subscriber, ui)
        }
        Err(err) => {
...
mod test {
    ...
    use super::titan_version_has_shim;
    ...
    fn test_skip_infer_version_constraint() {
        ...
        assert!(titan_version_has_shim(release));
        assert!(titan_version_has_shim(canary));
        assert!(titan_version_has_shim(newer_canary));
        assert!(titan_version_has_shim(newer_minor_canary));
        assert!(titan_version_has_shim(new));
        assert!(titan_version_has_shim(new_major));
        assert!(!titan_version_has_shim(old));
        assert!(!titan_version_has_shim(old_canary));
    }

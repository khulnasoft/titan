...
/// A basic logger that logs to stderr using the TitanFormatter.
/// The first generic parameter refers to the previous layer, which
/// is in this case the default layer (`Registry`).
type StdErrLog = fmt::Layer<Registry, DefaultFields, TitanFormatter, StdErrWrapper>;
...
/// - with evel `TRACE` or higher for the `titanrepo` target
...
impl TitanSubscriber {
    ...
    /// - If the `TITAN_LOG_VERBOSITY` env var is set, it will be used to set
    ...
    ///   `TITAN_LOG_VERBOSITY` global setting, but not per-module settings.
    ///
    /// `TitanSubscriber` has optional loggers that can be enabled later:
    ...
    pub fn new_with_verbosity(verbosity: usize, ui: &UI) -> Self {
        ...
        let env_filter = || {
            let filter = EnvFilter::builder()
                ...
                .with_env_var("TITAN_LOG_VERBOSITY")
                .from_env_lossy();
        ...
        let daemon_filter = tracing_subscriber::filter::targets::Targets::new()
            ...
            .with_target("titanrepo", Level::TRACE);

        let stderr = fmt::layer()
            .with_writer(StdErrWrapper {})
            .event_format(TitanFormatter::new_with_ansi(!ui.should_strip_ansi))
...
impl<S, N> FormatEvent<S, N> for TitanFormatter
where

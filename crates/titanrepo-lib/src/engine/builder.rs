...
use super::Engine;
use crate::{
    config::{validate_extends, validate_no_package_task_syntax, RawTaskDefinition, TitanJson},
...
pub enum Error {
    ...
    #[error(
        "{task_id} needs an entry in titan.json before it can be depended on because it is a task \
    ...
    #[error("Could not find \"{task_id}\" in root titan.json or \"{task_name}\" in workspace")]
    MissingWorkspaceTask { task_id: String, task_name: String },
    #[error(transparent)]
    Config(#[from] crate::config::Error),
    #[error("Invalid titan.json:\n{error_lines}")]
...
pub struct EngineBuilder<'a> {
    ...
    is_single: bool,
    titan_jsons: Option<HashMap<WorkspaceName, TitanJson>>,
...
impl<'a> EngineBuilder<'a> {
    ...
    pub fn with_titan_jsons(
        mut self,
        titan_jsons: Option<HashMap<WorkspaceName, TitanJson>>,
    ) -> Self {
        self.titan_jsons = titan_jsons;
        self
    }
    ...
    pub fn build(mut self) -> Result<super::Engine, Error> {
        ...
        let mut titan_jsons = self.titan_jsons.take().unwrap_or_default();
        ...
        for (workspace, task) in self.workspaces.iter().cartesian_product(self.tasks.iter()) {
            ...
            if self.has_task_definition(&mut titan_jsons, workspace, task, &task_id)? {
    ...
    fn has_task_definition(
        &self,
        titan_jsons: &mut HashMap<WorkspaceName, TitanJson>,
        workspace: &WorkspaceName,
        task_name: &TaskName<'static>,
        task_id: &TaskId,
    ) -> Result<bool, Error> {
        let titan_json = self
            .titan_json(titan_jsons, workspace)
            // If there was no titan.json in the workspace, fallback to the root titan.json
            .or_else(|e| {
                if e.is_missing_titan_json() && !matches!(workspace, WorkspaceName::Root) {
                    Ok(None)
                } else {
                    Err(e)
                }
        ...
        let Some(titan_json) = titan_json else {
            return self.has_task_definition(titan_jsons, &WorkspaceName::Root, task_name, task_id);
        };

        let task_id_as_name = task_id.as_task_name();
        if titan_json.pipeline.contains_key(&task_id_as_name)
            || titan_json.pipeline.contains_key(task_name)
        {
            Ok(true)
        } else if !matches!(workspace, WorkspaceName::Root) {
            self.has_task_definition(titan_jsons, &WorkspaceName::Root, task_name, task_id)
        } else {
            Ok(false)
        }
    }
    ...
    fn task_definition_chain(
        ...
        let root_titan_json = self
            .titan_json(titan_jsons, &WorkspaceName::Root)?
            .ok_or(Error::Config(crate::config::Error::NoTitanJSON))?;

        if let Some(root_definition) = root_titan_json.task(task_id, task_name) {
        ...
        if task_id.package() != ROOT_PKG_NAME {
            match self.titan_json(titan_jsons, &WorkspaceName::from(task_id.package())) {
                ...
                Err(e) if e.is_missing_titan_json() => (),
                Err(e) => {
                    return Err(e);
                }
            }
    ...
    fn titan_json<'b>(
        &self,
        titan_jsons: &'b mut HashMap<WorkspaceName, TitanJson>,
        workspace: &WorkspaceName,
    ) -> Result<Option<&'b TitanJson>, Error> {
        if titan_jsons.get(workspace).is_none() {
            let json = self.load_titan_json(workspace)?;
            titan_jsons.insert(workspace.clone(), json);
        }
        Ok(titan_jsons.get(workspace))
    }

    fn load_titan_json(&self, workspace: &WorkspaceName) -> Result<TitanJson, Error> {
...
impl Error {
    fn is_missing_titan_json(&self) -> bool {
        matches!(self, Self::Config(crate::config::Error::NoTitanJSON))
    }
}
...
mod test {
    ...
    use super::*;
    use crate::{config::RawTitanJson, engine::TaskNode};
    ...
    impl Lockfile for MockLockfile {
        fn resolve_package(
            ...
            _name: &str,
            _version: &str,
        ) -> Result<Option<titanrepo_lockfiles::Package>, titanrepo_lockfiles::Error> {
            unreachable!()
        }
        ...
        fn all_dependencies(
            &self,
            _key: &str,
        ) -> Result<Option<HashMap<String, String>>, titanrepo_lockfiles::Error> {
            unreachable!()
        }
        ...
        fn subgraph(
            ...
            _packages: &[String],
        ) -> Result<Box<dyn Lockfile>, titanrepo_lockfiles::Error> {
            unreachable!()
        }

        fn encode(&self) -> Result<Vec<u8>, titanrepo_lockfiles::Error> {
            unreachable!()
        }
    ...
    impl PackageDiscovery for MockDiscovery {
        async fn discover_packages(
            ...
            titanrepo_repository::discovery::DiscoveryResponse,
            titanrepo_repository::discovery::Error,
        > {
            Ok(titanrepo_repository::discovery::DiscoveryResponse {
    ...
    fn test_titan_json_loading() {
        ...
        let engine_builder = EngineBuilder::new(&repo_root, &package_graph, false)
            .with_titan_jsons(Some(vec![].into_iter().collect()));

        let a_titan_json = repo_root.join_components(&["packages", "a", "titan.json"]);
        a_titan_json.ensure_dir().unwrap();

        let result = engine_builder.load_titan_json(&WorkspaceName::from("a"));
        assert!(
            result.is_err() && result.unwrap_err().is_missing_titan_json(),
        ...
        let titan_json = engine_builder
            .load_titan_json(&WorkspaceName::from("a"))
            .unwrap();
        assert_eq!(titan_json.pipeline.len(), 1);
    }

    fn titan_json(value: serde_json::Value) -> TitanJson {
        let raw: RawTitanJson = serde_json::from_value(value).unwrap();
        TitanJson::try_from(raw).unwrap()
    }
    ...
    fn test_task_definition(
        ...
        let has_def = engine_builder
            .has_task_definition(&mut titan_jsons, &workspace, &task_name, &task_id)
            .unwrap();
    ...
    fn test_default_engine() {
        ...
        let engine = EngineBuilder::new(&repo_root, &package_graph, false)
            .with_titan_jsons(Some(titan_jsons))

...
fn call_titan_gen(command: &str, tag: &String, raw_args: &str) -> Result<i32, Error> {
    debug!(
        "Running @titan/gen@{} with command `{}` and args {:?}",
        tag, command, raw_args
    );
    let npx_path = which("npx").map_err(Error::NpxNotFound)?;
    let mut npx = Command::new(npx_path);
    npx.arg("--yes")
        .arg(format!("@titan/gen@{}", tag))
        .arg("raw")
        .arg(command)
        .args(["--json", raw_args])
...
pub fn run(
    ...
    if let Some(box GenerateCommand::Workspace(workspace_args)) = command {
        ...
        call_titan_gen("workspace", tag, &raw_args)?;
    } else {
        ...
        call_titan_gen("run", tag, &raw_args)?;
    }

...
#[cfg(not(test))]
use titanrepo_ui::CYAN;
use titanrepo_ui::{BOLD, GREY, UNDERLINE};
use titanrepo_vercel_api::{CachingStatus, Space, Team};
...
pub enum Error {
    ...
    #[error("User not found. Please login to Titanrepo first by running {command}.")]
    ...
    EnableCaching,
    #[error(
        "Could not persist selected space ({space_id}) to `experimentalSpaces.id` in titan.json"
    )]
    WriteToTitanJson {
        space_id: String,
        #[source]
...
pub(crate) const REMOTE_CACHING_INFO: &str = "  Remote Caching shares your cached Titanrepo task \
...
pub async fn link(
    ...
    let token = base.config()?.token().ok_or_else(|| Error::TokenNotFound {
        command: base.ui.apply(BOLD.apply_to("`npx titan login`")),
    })?;
    ...
    {}
      ...
      ",
            ...
            if modify_gitignore {
                add_titan_to_gitignore(base).map_err(|error| config::Error::FailedToSetConfig {
    ...
    {}
        ",
                ...
                GREY.apply_to("To disable Remote Caching, run `npx titan unlink`")
            );
            Ok(())
        }
        LinkTarget::Spaces => {
      ...
      ",
            ...
            add_space_id_to_titan_json(base, &space.id)?;
    ...
    {}
        ",
                ...
                GREY.apply_to(
                    "To remove Spaces integration, run `npx titan unlink --target spaces`"
                )
            );
...
fn select_team<'a>(
    ...
    let prompt = format!(
        ...
        base.ui.apply(BOLD.apply_to(
            "Which Vercel scope (and Remote Cache) do you want associated with this Titanrepo?",
        )),
        base.ui
...
fn select_space<'a>(base: &CommandBase, spaces: &'a [Space]) -> Result<SelectedSpace<'a>, Error> {
    ...
    let prompt = format!(
        "{}\n  {}",
        base.ui.apply(
            BOLD.apply_to("Which Vercel space do you want associated with this Titanrepo?",)
        ),
        base.ui
            .apply(CYAN.apply_to("[Use arrows to move, type to filter]"))
    );
...
fn add_titan_to_gitignore(base: &CommandBase) -> Result<(), io::Error> {
    ...
    if !gitignore_path.exists() {
        ...
        writeln!(gitignore, ".titan")?;
    } else {
        let gitignore = File::open(&gitignore_path)?;
        let mut lines = io::BufReader::new(gitignore).lines();
        let has_titan = lines.any(|line| line.map_or(false, |line| line.trim() == ".titan"));
        if !has_titan {
...
fn add_space_id_to_titan_json(base: &CommandBase, space_id: &str) -> Result<(), Error> {
    let titan_json_path = base.repo_root.join_component("titan.json");
    let titan_json = titan_json_path
        .read_existing_to_string_or(Ok("{}"))
        .map_err(|error| config::Error::FailedToReadConfig {
            config_path: titan_json_path.clone(),
            error,
        })?;
    ...
    fs::write(titan_json_path, output).map_err(|error| Error::WriteToTitanJson {
...
mod test {
    ...
    use titanrepo_ui::UI;
    use titanrepo_vercel_api_mock::start_test_server;

    use crate::{
        cli::LinkTarget,
        commands::{link, CommandBase},
        config::{RawTitanJson, TitanrepoConfigBuilder},
        Args,
    };
    ...
    async fn test_link_remote_cache() -> Result<()> {
        ...
        let repo_config_path = repo_root.join_components(&[".titan", "config.json"]);
        ...
        base.config
            .set(
                TitanrepoConfigBuilder::new(&base)
        ...
        // read the config
        let updated_config = TitanrepoConfigBuilder::new(&base).build().unwrap();
        let team_id = updated_config.team_id();

        assert!(
            team_id == Some(titanrepo_vercel_api_mock::EXPECTED_USER_ID)
                || team_id == Some(titanrepo_vercel_api_mock::EXPECTED_TEAM_ID)
        );
    ...
    async fn test_link_spaces() {
        ...
        // titan config
        let titan_json_file = base.repo_root.join_component("titan.json");

        fs::write(
            titan_json_file.as_path(),
        ...
        // verify space id is added to titan.json
        let titan_json_contents = fs::read_to_string(&titan_json_file).unwrap();
        let titan_json: RawTitanJson = serde_json::from_str(&titan_json_contents).unwrap();
        assert_eq!(
            titan_json.experimental_spaces.unwrap().id.unwrap(),
            titanrepo_vercel_api_mock::EXPECTED_SPACE_ID
        );
    }
}

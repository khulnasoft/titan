...
pub enum Error {
    ...
    #[error(transparent)]
    Lockfile(#[from] titanrepo_lockfiles::Error),
    #[error("titan doesn't support workspaces at file system root")]
...
fn titan_json() -> &'static AnchoredSystemPath {
    static PATH: OnceLock<&'static AnchoredSystemPath> = OnceLock::new();
    PATH.get_or_init(|| AnchoredSystemPath::new("titan.json").unwrap())
}
...
pub async fn prune(
    ...
    if matches!(
        ...
        titanrepo_repository::package_manager::PackageManager::Bun
    ) {
    ...
    prune.copy_titan_json(&workspace_names)?;
...
impl<'a> Prune<'a> {
    ...
    fn copy_file(
            ...
            )
        {
            ...
            titanrepo_fs::copy_file(&from_path, docker_to)?;
        }
        Ok(())
    }
    ...
    fn copy_directory(
        ...
        let full_to = self.full_directory.resolve(path);
        titanrepo_fs::recursive_copy(&from_path, full_to)?;
        if matches!(destination, Some(CopyDestination::All)) {
            let out_to = self.out_directory.resolve(path);
            titanrepo_fs::recursive_copy(&from_path, out_to)?;
        }
        if self.docker
            && matches!(
                destination,
        ...
        {
            let docker_to = self.docker_directory().resolve(path);
            titanrepo_fs::recursive_copy(&from_path, docker_to)?;
        }
        Ok(())
    }
    ...
    fn copy_workspace(&self, package_json_path: &AnchoredSystemPath) -> Result<(), Error> {
        ...
        titanrepo_fs::recursive_copy(original_dir, &target_dir)?;
    ...
    fn copy_titan_json(&self, workspaces: &[String]) -> Result<(), Error> {
        let original_titan_path = self.root.resolve(titan_json());

        let new_titan_path = self.full_directory.resolve(titan_json());

        let titan_json_contents = match original_titan_path.read() {
            Ok(contents) => contents,
            Err(e) if e.kind() == std::io::ErrorKind::NotFound => {
                // If titan.json doesn't exist skip copying
                return Ok(());
            }
            Err(e) => return Err(e.into()),
        };
        let titan_json: RawTitanJson = serde_json::from_reader(json_comments::StripComments::new(
            titan_json_contents.as_slice(),
        ))?;

        let pruned_titan_json = titan_json.prune_tasks(workspaces);
        new_titan_path.create_with_contents(serde_json::to_string_pretty(&pruned_titan_json)?)?;

use titanrepo_api_client::{APIClient, Client};
use titanrepo_auth::{
...
use titanrepo_telemetry::events::command::{CommandEventBuilder, LoginMethod};
use titanrepo_ui::{BOLD, CYAN, UI};
use titanrepo_vercel_api::TokenMetadata;
...
/// Entry point for `titan login --sso-team`.
...
/// Entry point for `titan login`. Checks for the existence of an auth file
...
/// token and writes it to `auth.json` in the Titan config directory.
...
mod tests {
    ...
    use titanrepo_auth::{mocks::*, AuthFile, TITANREPO_AUTH_FILE_NAME};
    ...
    fn setup_base(auth_path: &AbsoluteSystemPathBuf, port: u16) -> CommandBase {
        ...
        let auth_file_path =
            AbsoluteSystemPathBuf::try_from(temp_dir.path().join(TITANREPO_AUTH_FILE_NAME))
                .unwrap();
        ...
        let ui = titanrepo_ui::UI::new(false);

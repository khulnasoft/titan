...
pub async fn daemon_client(command: &DaemonCommand, base: &CommandBase) -> Result<(), DaemonError> {
    ...
    let pid_file = base.daemon_file_root().join_component("titand.pid");
    let sock_file = base.daemon_file_root().join_component("titand.sock");
...
pub async fn daemon_server(
    base: &CommandBase,
    idle_time: &String,
    logging: &TitanSubscriber,
) -> Result<(), DaemonError> {
    let (log_folder, log_file) = {
        let directories = directories::ProjectDirs::from("com", "titanrepo", "titanrepo")
        ...
        let log_file =
            log_folder.join_component(format!("{}-titan.log", base.repo_hash()).as_str());
    ...
    let server = crate::daemon::TitanGrpcService::new(

...
pub use duration::TitanDuration;
...
pub enum Error {
    ...
    SpacesRequest(#[from] titanrepo_api_client::Error),
    ...
    Env(#[source] titanrepo_env::Error),
...
// for rendering a single-package run of titan. Notably, we want to always omit
...
impl<'a> From<&'a RunSummary<'a>> for SinglePackageRunSummary<'a> {
    fn from(run_summary: &'a RunSummary<'a>) -> Self {
        ...
        SinglePackageRunSummary {
            ...
            titan_version: run_summary.titan_version,
...
impl<'a> RunSummary<'a> {
    ...
    async fn send_to_space(
        ...
        let spinner = tokio::spawn(async {
            ...
            titanrepo_ui::start_spinner("...sending run summary...");
        });
    ...
    fn get_path(&self) -> AbsoluteSystemPathBuf {
        ...
        self.repo_root
            .join_components(&[".titan", "runs", &filename])
    }

...
pub struct ExecutionSummary<'a> {
    // a synthesized titan command to produce this invocation
    command: String,
    // the (possibly empty) path from the titanrepo root to where the command was run
...
impl<'a> ExecutionSummary<'a> {
    pub fn new(
        ...
        let duration = TitanDuration::new(&start_time, &end_time);
        Self {
            command,
    ...
    pub fn print(&self, ui: UI, path: AbsoluteSystemPathBuf, failed_tasks: Vec<&TaskSummary>) {
        let maybe_full_titan = if self.cached == self.attempted && self.attempted > 0 {
            match std::env::var("TERM_PROGRAM").as_deref() {
                Ok("Apple_Terminal") => color!(ui, MAGENTA, ">>> FULL TITAN").to_string(),
                _ => ui.rainbow(">>> FULL TITAN").to_string(),
            }
        } else {
            String::new()
        };

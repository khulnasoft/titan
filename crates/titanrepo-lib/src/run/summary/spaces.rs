...
impl SpacesClient {
    pub fn new(
        ...
        if !is_linked {
            eprintln!(
                "Error: experimentalSpaceId is enabled, but repo is not linked to API. Run `titan \
                 link` or `titan login` first"
            );
            return None;
        }
    ...
    fn from(value: SpacesTaskInformation) -> Self {
        ...
        let cache = cache_status.map_or_else(
            ...
            |CacheHitMetadata { source, time_saved }| SpacesCacheStatus {
                status: titanrepo_api_client::spaces::CacheStatus::Hit,
                source: Some(match source {
                    titanrepo_cache::CacheSource::Local => {
                        titanrepo_api_client::spaces::CacheSource::Local
                    }
                    titanrepo_cache::CacheSource::Remote => {
                        titanrepo_api_client::spaces::CacheSource::Remote
                    }
                }),
                time_saved,
            },

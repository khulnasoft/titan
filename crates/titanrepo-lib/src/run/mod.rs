...
impl<'a> Run<'a> {
    ...
    async fn run_with_analytics(
        ...
        let is_ci_or_not_tty = titanrepo_ci::is_ci() || !std::io::stdout().is_terminal();

        let daemon = match (is_ci_or_not_tty, opts.run_opts.daemon) {
            (true, None) => {
                debug!("skipping titand since we appear to be in a non-interactive context");
                None
            }
            (_, Some(true)) | (false, None) => {
                let connector = DaemonConnector {
                    ...
                    pid_file: self.base.daemon_file_root().join_component("titand.pid"),
                    sock_file: self.base.daemon_file_root().join_component("titand.sock"),
                };
            ...
            (_, Some(false)) => {
                debug!("skipping titand since --no-daemon was passed");
                None
            }
        };
        ...
        let root_titan_json =
            TitanJson::load(&self.base.repo_root, &root_package_json, is_single_package)?;

        let team_id = root_titan_json
            .remote_cache
            .as_ref()
            .and_then(|configuration_options| configuration_options.team_id.clone())
            .unwrap_or_default();

        let signature = root_titan_json
            .remote_cache
            .as_ref()
        ...
        if opts.run_opts.experimental_space_id.is_none() {
            opts.run_opts.experimental_space_id = root_titan_json.space_id.clone();
        }
        ...
        let filtered_pkgs = {
            ...
            if is_all_packages {
                for target in self.targets() {
                    ...
                    if root_titan_json.pipeline.contains_key(&task_name) {
        ...
        let mut engine =
            self.build_engine(&pkg_dep_graph, &opts, &root_titan_json, &filtered_pkgs)?;
        ...
        let mut global_hash_inputs = get_global_hash_inputs(
            ...
            &root_titan_json.global_deps,
            &env_at_execution_start,
            &root_titan_json.global_env,
            root_titan_json.global_pass_through_env.as_deref(),
            opts.run_opts.env_mode,
            opts.run_opts.framework_inference,
            root_titan_json.global_dot_env.as_deref(),
        )?;
        ...
        if let Some(subscriber) = signal_handler.subscribe() {
            ...
            tokio::spawn(async move {
                ...
                let spinner = titanrepo_ui::start_spinner("...Finishing writing to cache...");
        ...
        if matches!(global_env_mode, EnvMode::Infer)
            && root_titan_json.global_pass_through_env.is_some()
        {
        ...
        if opts.run_opts.parallel {
            ...
            engine = self.build_engine(&pkg_dep_graph, &opts, &root_titan_json, &filtered_pkgs)?;
        }
    ...
    fn build_engine(
        ...
        .with_root_tasks(root_titan_json.pipeline.keys().cloned())
        .with_titan_jsons(Some(
            Some((WorkspaceName::Root, root_titan_json.clone()))
                .into_iter()
                .collect(),
        ))

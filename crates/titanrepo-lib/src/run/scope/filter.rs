...
impl PackageInference {
    // calculate, based on the directory that global titan was invoked in,
    // the pieces of a filter spec that we will infer. If titan was invoked
    // somewhere between the root and packages, scope titan invocations to the
    // packages below where titan was invoked. If titan was invoked at or within
    // a particular package, scope the titan invocation to just that package.
    pub fn calculate(
        ...
        let full_inference_path = titan_root.resolve(pkg_inference_path);
        for (workspace_name, workspace_entry) in pkg_graph.workspaces() {
            let pkg_path = titan_root.resolve(workspace_entry.package_path());
            ...
            if inferred_path_is_below && (&pkg_path as &AbsoluteSystemPath) != titan_root {
    ...
    pub fn apply(&self, selector: &mut TargetSelector) {
        ...
        if selector.parent_dir != titanpath::AnchoredSystemPathBuf::default() {
...
pub struct FilterResolver<'a, T: PackageChangeDetector> {
    ...
    titan_root: &'a AbsoluteSystemPath,
...
impl<'a> FilterResolver<'a, SCMChangeDetector<'a>> {
    pub(crate) fn new(
        ...
        Self::new_with_change_detector(pkg_graph, titan_root, inference, scm, change_detector)
    }
}
...
mod test {
    ...
    use titanpath::{AbsoluteSystemPathBuf, AnchoredSystemPathBuf, RelativeUnixPathBuf};
    ...
    fn make_project<T: PackageChangeDetector>(
        ...
        let titan_root = Box::leak(Box::new(
        ...
        let graph = {
            let rt = tokio::runtime::Builder::new_current_thread()
                ...
                .unwrap();
            rt.block_on(
                PackageGraph::builder(titan_root, Default::default())
        ...
        let scm = Box::leak(Box::new(titanrepo_scm::SCM::new(titan_root)));

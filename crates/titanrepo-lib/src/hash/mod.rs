//! hash module
//!
//! This module contains the hash functions used by titanrepo for certain
...
mod proto_capnp {
    use titanrepo_env::ResolvedEnvMode;
...
pub struct TaskHashable<'a> {
    ...
    // task
    pub(crate) package_dir: Option<titanpath::RelativeUnixPathBuf>,
    ...
    pub(crate) dot_env: &'a [titanpath::RelativeUnixPathBuf],
}

#[derive(Debug, Clone)]
pub struct GlobalHashable<'a> {
    pub global_cache_key: &'static str,
    pub global_file_hash_map: &'a HashMap<titanpath::RelativeUnixPathBuf, String>,
    ...
    pub dot_env: &'a [titanpath::RelativeUnixPathBuf],
}

pub struct LockFilePackages(pub Vec<titanrepo_lockfiles::Package>);

#[derive(Debug, Clone)]
pub struct FileHashes(pub HashMap<titanpath::RelativeUnixPathBuf, String>);
...
impl From<LockFilePackages> for Builder<HeapAllocator> {
    fn from(LockFilePackages(packages): LockFilePackages) -> Self {
...

        {
            ...
            for (i, titanrepo_lockfiles::Package { key, version }) in packages.iter().enumerate() {
...
mod test {
    ...
    use titanrepo_env::ResolvedEnvMode;
    use titanrepo_lockfiles::Package;

    use super::{
        FileHashes, GlobalHashable, LockFilePackages, TaskHashable, TaskOutputs, TitanHash,
    };
    use crate::cli::EnvMode;
    ...
    fn task_hashable() {
        let task_hashable = TaskHashable {
            ...
            package_dir: Some(titanpath::RelativeUnixPathBuf::new("package_dir").unwrap()),
            ...
            dot_env: &[titanpath::RelativeUnixPathBuf::new("dotenv".to_string()).unwrap()],
        };
    ...
    fn global_hashable() {
        let global_file_hash_map = vec![(
            titanpath::RelativeUnixPathBuf::new("global_file_hash_map").unwrap(),
    ...
    fn file_hashes(pairs: Vec<(String, String)>, expected: &str) {
        let file_hashes = FileHashes(
            pairs
                .into_iter()
                .map(|(a, b)| (titanpath::RelativeUnixPathBuf::new(a).unwrap(), b))
                .collect(),
        );

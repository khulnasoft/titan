...
use crate::{
    ...
    hash::{FileHashes, LockFilePackages, TaskHashable, TitanHash},
    opts::Opts,
    run::task_id::TaskId,
        ...
        let framework_slug = if do_framework_inference {
            ...
            if let Some(framework) = infer_framework(workspace, is_monorepo) {
                ...
                if let Some(exclude_prefix) =
                    self.env_at_execution_start.get("TITAN_CI_VENDOR_ENV_KEY")
                {
...
pub fn get_external_deps_hash(
    transitive_dependencies: &Option<HashSet<titanrepo_lockfiles::Package>>,
) -> String {

...
use crate::{
    ...
    get_version,
    tracing::TitanSubscriber,
    Payload,
};

mod error;

// Global titan sets this environment variable to its cwd so that local
// titan can use it for package inference.
pub const INVOCATION_DIR_ENV_VAR: &str = "TITAN_INVOCATION_DIR";
...
pub struct Args {
    ...
    #[clap(long, global = true)]
    #[serde(skip)]
    /// Skip any attempts to infer which version of Titan the project is
    /// configured to use
    pub skip_infer: bool,
    /// Disable the titan update notification
    #[clap(long, global = true)]
    #[serde(skip)]
    ...
    /// The directory in which to run titan
    #[clap(long, global = true, value_parser)]
    ...
    #[clap(long, global = true)]
    pub no_color: bool,
    /// When enabled, titan will precede HTTP requests with an OPTIONS request
    /// for authorization
    #[clap(long, global = true)]
    pub preflight: bool,
    ...
    /// Force a check for a new version of titan
...
pub enum DaemonCommand {
    /// Restarts the titan daemon
    Restart,
    /// Ensures that the titan daemon is running
    Start,
    /// Reports the status of the titan daemon
    Status {
        /// Pass --json to report status in JSON format
        #[clap(long)]
        json: bool,
    },
    /// Stops the titan daemon
    Stop,
    /// Stops the titan daemon if it is already running, and removes any stale
    /// daemon state
    Clean,
}
...
pub enum Command {
    ...
    /// Runs the Titanrepo background daemon
    Daemon {
        /// Set the idle timeout for titand
    ...
    Info {
        ...
        // We output titan info as json. Currently just for internal testing
        #[clap(long)]
        json: bool,
    },
    ...
    ///
    /// By default, titan executes tasks in topological order (i.e.
...
pub struct GenerateWorkspaceArgs {
    ...
    /// The root of your repository (default: directory with root titan.json)
    #[clap(short = 'r', long)]
...
pub struct RunArgs {
    ...
    /// Run titan in single-package mode
    #[clap(long)]
    ...
    #[clap(long, env = "TITAN_FORCE", default_missing_value = "true")]
    ...
    /// Use "strict" to use an allowlist specified in titan.json.
    /// Use "infer" to defer to existence of "passThroughEnv" or
    /// "globalPassThroughEnv" in titan.json. (default infer)
    ...
    // -----------------------
    /// Force titan to either use or not use the local daemon. If unset
    /// titan will use the default detection logic.
    ...
    /// all output. Use "hash-only" to show only titan-computed
    ...
    /// titan decide based on its own heuristics. (default auto)
    #[clap(long, env = "TITAN_LOG_ORDER", value_enum, default_value_t = LogOrder::Auto)]
    ...
    /// File to write titan's performance profile output into.
    ...
    #[clap(long, env = "TITAN_REMOTE_ONLY", value_name = "BOOL", action = ArgAction::Set, default_value = "false", default_missing_value = "true", num_args = 0..=1)]
    pub remote_only: bool,
    /// Treat remote cache as read only
    #[clap(long, env = "TITAN_REMOTE_CACHE_READ_ONLY", value_name = "BOOL", action = ArgAction::Set, default_value = "false", default_missing_value = "true", num_args = 0..=1)]
    #[serde(skip)]
    ...
    /// Generate a summary of the titan run
    #[clap(long, env = "TITAN_RUN_SUMMARY", default_missing_value = "true")]
    pub summarize: Option<Option<bool>>,

    /// Use "none" to remove prefixes from task logs. Use "task" to get task id
    /// prefixing. Use "auto" to let titan decide how to prefix the logs
    ...
    // > Usage: titan [OPTIONS] [TASKS]... [-- <FORWARDED_ARGS>...] [COMMAND]
    #[clap(hide = true)]
...
///
/// Scenarios:
/// 1. inference failed, we're running this global titan. no repo state
/// 2. --skip-infer was passed, assume we're local titan and run. no repo state
/// 3. There is no local titan, we're running the global one. repo state exists
/// 4. titan binary path is set, and it's this one. repo state exists
///
/// # Arguments
///
/// * `repo_state`: If we have done repository inference and NOT executed
/// local titan, such as in the case where `TITAN_BINARY_PATH` is set,
...
pub async fn run(
    ...
    #[allow(unused_variables)] logger: &TitanSubscriber,
    ui: UI,
    ...
    if titanrepo_telemetry::config::is_telemetry_internal_test() {
    ...
    if let Command::Run(run_args) = &mut command {
        ...
        if cli_args.cwd.is_none() {
            if let Ok(invocation_dir) = env::var(INVOCATION_DIR_ENV_VAR) {
                ...
                // If repo state doesn't exist, we're either local titan running at the root
                // (cwd), or inference failed.
                // If repo state does exist, we're global titan, and want to calculate
    ...
    let cli_result = match cli_args.command.as_ref().unwrap() {
        ...
        Command::Run(args) => {
            ...
            // to the rust codepath for running titan
        ...
        Command::Completion { shell } => {
            CommandEventBuilder::new("completion")
                .with_parent(&root_telemetry)
                .track_call();
            generate(*shell, &mut Args::command(), "titan", &mut io::stdout());
            Ok(Payload::Rust(Ok(0)))
        }
    };
...
mod test {
    ...
    #[test_case::test_case(
		&["titan", "run", "build", "--framework-inference"],
        Args {
            command: Some(Command::Run(Box::new(RunArgs {
                tasks: vec!["build".to_string()],
                framework_inference: true,
    ...
    #[test_case::test_case(
		&["titan", "run", "build", "--framework-inference", "true"],
        Args {
            command: Some(Command::Run(Box::new(RunArgs {
                tasks: vec!["build".to_string()],
                framework_inference: true,
    ...
    #[test_case::test_case(
		&["titan", "run", "build", "--env-mode", "infer"],
        Args {
            command: Some(Command::Run(Box::new(RunArgs {
                tasks: vec!["build".to_string()],
                env_mode: EnvMode::Infer,
    ...
    #[test_case::test_case(
		&["titan", "run", "build", "--env-mode", "loose"],
        Args {
            command: Some(Command::Run(Box::new(RunArgs {
                tasks: vec!["build".to_string()],
    ...
    #[test_case::test_case(
		&["titan", "run", "build", "--env-mode", "strict"],
        Args {
            command: Some(Command::Run(Box::new(RunArgs {
                tasks: vec!["build".to_string()],
    ...
    #[test_case::test_case(
		&["titan", "run", "build", "--cache-dir", "foobar"],
        Args {
            command: Some(Command::Run(Box::new(RunArgs {
                tasks: vec!["build".to_string()],
    ...
    #[test_case::test_case(
		&["titan", "run", "build", "--cache-workers", "100"],
        Args {
            command: Some(Command::Run(Box::new(RunArgs {
                tasks: vec!["build".to_string()],
                cache_workers: 100,
    ...
    #[test_case::test_case(
		&["titan", "run", "build", "--concurrency", "20"],
        Args {
            command: Some(Command::Run(Box::new(RunArgs {
                tasks: vec!["build".to_string()],
    ...
    #[test_case::test_case(
		&["titan", "run", "build", "--filter", "water", "--filter", "earth", "--filter", "fire", "--filter", "air"],
        Args {
            command: Some(Command::Run(Box::new(RunArgs {
                tasks: vec!["build".to_string()],
                filter: vec![
    ...
    #[test_case::test_case(
		&["titan", "run", "build", "-F", "water", "-F", "earth", "-F", "fire", "-F", "air"],
        Args {
            command: Some(Command::Run(Box::new(RunArgs {
                tasks: vec!["build".to_string()],
                filter: vec![
    ...
    #[test_case::test_case(
		&["titan", "run", "build", "--filter", "water", "-F", "earth", "--filter", "fire", "-F", "air"],
        Args {
            command: Some(Command::Run(Box::new(RunArgs {
                tasks: vec!["build".to_string()],
                filter: vec![
    ...
    #[test_case::test_case(
		&["titan", "run", "build", "--global-deps", ".env"],
        Args {
            command: Some(Command::Run(Box::new(RunArgs {
                tasks: vec!["build".to_string()],
    ...
    #[test_case::test_case(
		&[ "titan", "run", "build", "--global-deps", ".env", "--global-deps", ".env.development"],
        Args {
            command: Some(Command::Run(Box::new(RunArgs {
                tasks: vec!["build".to_string()],
    ...
    #[test_case::test_case(
		&["titan", "run", "build", "--graph", "out.html"],
        Args {
            command: Some(Command::Run(Box::new(RunArgs {
                tasks: vec!["build".to_string()],
    ...
    #[test_case::test_case(
		&["titan", "run", "build", "--ignore", "foo.js", "--ignore", "bar.js"],
        Args {
            command: Some(Command::Run(Box::new(RunArgs {
                tasks: vec!["build".to_string()],
    ...
    #[test_case::test_case(
		&["titan", "run", "build", "--include-dependencies"],
        Args {
            command: Some(Command::Run(Box::new(RunArgs {
                tasks: vec!["build".to_string()],
    ...
    #[test_case::test_case(
		&["titan", "run", "build", "--output-logs", "full"],
        Args {
            command: Some(Command::Run(Box::new(RunArgs {
                tasks: vec!["build".to_string()],
    ...
    #[test_case::test_case(
		&["titan", "run", "build", "--output-logs", "none"],
        Args {
            command: Some(Command::Run(Box::new(RunArgs {
                tasks: vec!["build".to_string()],
    ...
    #[test_case::test_case(
		&["titan", "run", "build", "--output-logs", "hash-only"],
        Args {
            command: Some(Command::Run(Box::new(RunArgs {
                tasks: vec!["build".to_string()],
    ...
    #[test_case::test_case(
		&["titan", "run", "build", "--log-order", "stream"],
        Args {
            command: Some(Command::Run(Box::new(RunArgs {
                tasks: vec!["build".to_string()],
    ...
    #[test_case::test_case(
		&["titan", "run", "build", "--log-order", "grouped"],
        Args {
            command: Some(Command::Run(Box::new(RunArgs {
                tasks: vec!["build".to_string()],
    ...
    #[test_case::test_case(
		&["titan", "run", "build", "--log-prefix", "auto"],
        Args {
            command: Some(Command::Run(Box::new(RunArgs {
                tasks: vec!["build".to_string()],
    ...
    #[test_case::test_case(
		&["titan", "run", "build", "--log-prefix", "none"],
        Args {
            command: Some(Command::Run(Box::new(RunArgs {
                tasks: vec!["build".to_string()],
    ...
    #[test_case::test_case(
		&["titan", "run", "build", "--log-prefix", "task"],
        Args {
            command: Some(Command::Run(Box::new(RunArgs {
                tasks: vec!["build".to_string()],
    ...
    #[test_case::test_case(
		&["titan", "run", "build", "--profile", "profile_out"],
        Args {
            command: Some(Command::Run(Box::new(RunArgs {
                tasks: vec!["build".to_string()],
    ...
    #[test_case::test_case(
		&["titan", "run", "build", "--remote-only", "true"],
        Args {
            command: Some(Command::Run(Box::new(RunArgs {
                tasks: vec!["build".to_string()],
                remote_only: true,
    ...
    #[test_case::test_case(
		&["titan", "run", "build", "--remote-only", "false"],
        Args {
            command: Some(Command::Run(Box::new(RunArgs {
                tasks: vec!["build".to_string()],
                remote_only: false,
    ...
    #[test_case::test_case(
		&["titan", "run", "build", "--scope", "foo", "--scope", "bar"],
        Args {
            command: Some(Command::Run(Box::new(RunArgs {
                tasks: vec!["build".to_string()],
    ...
    fn test_serde() {
        ...
        assert_eq!(
            serde_json::to_string(&Args::try_parse_from(["titan", "run", "build"]).unwrap())
                .unwrap()
    ...
    fn test_multi_daemon() {
        assert_matches!(
            Args::try_parse_from(["titan", "run", "build", "--daemon", "--no-daemon"]),
    ...
    fn test_parse_bin() {
        assert_eq!(
            Args::try_parse_from(["titan", "bin"]).unwrap(),
            Args {
    ...
    fn test_parse_login() {
        assert_eq!(
            Args::try_parse_from(["titan", "login"]).unwrap(),
            Args {
    ...
    fn test_parse_logout() {
        assert_eq!(
            Args::try_parse_from(["titan", "logout"]).unwrap(),
            Args {
    ...
    fn test_parse_unlink() {
        assert_eq!(
            Args::try_parse_from(["titan", "unlink"]).unwrap(),
            Args {
    ...
    fn test_parse_prune() {
        ...
        assert_eq!(
            Args::try_parse_from(["titan", "prune", "foo"]).unwrap(),
            Args {
        ...
        assert_eq!(
            Args::try_parse_from(["titan", "prune", "--scope", "bar"]).unwrap(),
            Args {
        ...
        assert_eq!(
            Args::try_parse_from(["titan", "prune", "foo", "bar"]).unwrap(),
            Args {
                command: Some(Command::Prune {
                    scope: None,
        ...
        assert_eq!(
            Args::try_parse_from(["titan", "prune", "--docker", "foo"]).unwrap(),
            Args {
                command: Some(Command::Prune {
                    scope: None,
                    scope_arg: Some(vec!["foo".into()]),
        ...
        assert_eq!(
            Args::try_parse_from(["titan", "prune", "--out-dir", "dist", "foo"]).unwrap(),
            Args {
                command: Some(Command::Prune {
                    scope: None,
                    scope_arg: Some(vec!["foo".into()]),
    ...
    fn test_pass_through_args() {
        assert_eq!(
            Args::try_parse_from(["titan", "run", "build", "--", "--script-arg=42"]).unwrap(),
            Args {
                command: Some(Command::Run(Box::new(RunArgs {
                    tasks: vec!["build".to_string()],
    ...
    fn test_parse_prune_no_mixed_arg_and_flag() {
        assert!(Args::try_parse_from(["titan", "prune", "foo", "--scope", "bar"]).is_err(),);
    }
    ...
    fn test_parse_gen() {
        ...
        assert_eq!(
            Args::try_parse_from(["titan", "gen"]).unwrap(),
            Args {
    ...
    fn test_go_fallback_conflicts_with_remote_read_only() {
        ...
        assert!(Args::try_parse_from(["titan", "build", "--go-fallback"]).is_ok(),);
        assert!(Args::try_parse_from(["titan", "build", "--remote-cache-read-only",]).is_ok(),);
    }

    #[test]
    fn test_profile_usage() {
        assert!(Args::try_parse_from(["titan", "build", "--profile", ""]).is_err());
        assert!(Args::try_parse_from(["titan", "build", "--anon-profile", ""]).is_err());
        assert!(Args::try_parse_from(["titan", "build", "--profile", "foo.json"]).is_ok());
        assert!(Args::try_parse_from(["titan", "build", "--anon-profile", "foo.json"]).is_ok());
        assert!(Args::try_parse_from([
            "titan",
            "build",
            "--profile",

...
use super::{proto::titand_client::TitandClient, DaemonClient};
...
pub struct DaemonConnector {
    ...
    pub pid_file: titanpath::AbsoluteSystemPathBuf,
    pub sock_file: titanpath::AbsoluteSystemPathBuf,
}
...
impl DaemonConnector {
    ...
    async fn get_connection(
        &self,
        path: titanpath::AbsoluteSystemPathBuf,
    ) -> Result<TitandClient<tonic::transport::Channel>, DaemonConnectorError> {
...
#[cfg(target_os = "windows")]
fn win(
    path: Arc<titanpath::AbsoluteSystemPathBuf>,
...
pub enum FileWaitError {
    ...
    InvalidPath(titanpath::AbsoluteSystemPathBuf),
}
...
mod test {
    ...
    fn pid_path(tmp_path: &Path) -> AbsoluteSystemPathBuf {
        AbsoluteSystemPathBuf::try_from(tmp_path.join("titand.pid")).unwrap()
    }

    fn sock_path(tmp_path: &Path) -> AbsoluteSystemPathBuf {
        AbsoluteSystemPathBuf::try_from(tmp_path.join("titand.sock")).unwrap()
    }
    ...
    #[tonic::async_trait]
    impl proto::titand_server::Titand for DummyServer {
        async fn shutdown(
            &self,
    ...
    async fn handles_kill_live_server() {
        ...
        let server_fut = tonic::transport::Server::builder()
            .add_service(proto::titand_server::TitandServer::new(DummyServer {

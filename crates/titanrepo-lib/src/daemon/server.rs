...
pub struct TitanGrpcService<S, PDA, PDB> {
    watcher_tx: watch::Sender<Option<Arc<FileWatching>>>,
...
impl<S> TitanGrpcService<S, WatchingPackageDiscovery, LocalPackageDiscoveryBuilder>
where
    S: Future<Output = CloseReason>,
{
    /// Create a gRPC server providing the Titand interface. external_shutdown
...
impl<S, PDA, PDB> TitanGrpcService<S, PDA, PDB>
where
    S: Future<Output = CloseReason>,
    ...
    PDB::Output: PackageDiscovery + Send + 'static,
{
    ...
    pub async fn serve(self) -> CloseReason {
        ...
        // so we use a private struct with just the pieces of state needed to handle
        // RPCs.
        let service = TitanGrpcServiceInner {
        ...
        let server_fut = {
            let service = ServiceBuilder::new()
                ...
                .service(crate::daemon::proto::titand_server::TitandServer::new(
                    service,
                ));
        ...
        // Now that the server has exited, the TitanGrpcService instance should be
...
struct TitanGrpcServiceInner<PD> {
...
impl<PD> TitanGrpcServiceInner<PD> {
...
#[tonic::async_trait]
impl<PD> proto::titand_server::Titand for TitanGrpcServiceInner<PD>
where
                ...
                tonic::Response::new(proto::DiscoverPackagesResponse {
                    package_files: packages
                        ...
                        .map(|d| proto::PackageFiles {
                            ...
                            titan_json: d.titan_json.map(|t| t.to_string()),
                        })
                        .collect(),
...
impl<T> NamedService for TitanGrpcServiceInner<T> {
    const NAME: &'static str = "titanrepo.Daemon";
}
...
mod test {
    ...
    async fn timeout() {
        ...
        let server = TitanGrpcService::new(
            repo_root.clone(),
            daemon_root,
            log_file,

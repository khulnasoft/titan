...
pub fn panic_handler(panic_info: &std::panic::PanicInfo) {
    ...
    let report = Report::new("titan", get_version(), Method::Panic, explanation, cause);

    let report_message = match report.persist() {
        Ok(f) => {
            format!(
                "A report has been written to {}\n
Please open an issue at https://github.com/khulnasoft/titan/issues/new/choose \
...
Please open an issue at \
                 https://github.com/khulnasoft/titan/issues/new/choose and include the following \
                 error in your issue: {}",
                e
            )
        }

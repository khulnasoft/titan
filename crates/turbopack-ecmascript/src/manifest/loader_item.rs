        ...
        writedoc!(
            ...
            r#"
                __titanpack_export_value__((__titanpack_import__) => {{
                    return Promise.all({chunks_server_data}.map((chunk) => __titanpack_load__(chunk))).then(() => {{
                        return __titanpack_require__({item_id});
                    }}).then((chunks) => {{
                        return Promise.all(chunks.map((chunk) => __titanpack_load__(chunk)));
                    }}).then(() => {{
                        return __titanpack_import__({dynamic_id});
                    }});
                }});
            "#,

...
use titan_tasks::{ValueDefault, Vc};
...
/// the titan_task caching requirements.
#[titan_tasks::value(
    transparent,
    serialization = "none",
    eq = "manual",
...
#[derive(Debug)]
pub struct TransformPlugin(#[titan_tasks(trace_ignore)] Box<dyn CustomTransformer + Send + Sync>);
...
#[titan_tasks::value(transparent, serialization = "auto_for_input")]
#[derive(Debug, Clone, PartialOrd, Ord, Hash)]
                ...
                let config = Options {
                    ...
                    refresh: if *refresh {
                        Some(swc_core::ecma::transforms::react::RefreshOptions {
                            refresh_reg: "__titanpack_refresh__.register".to_string(),
                            refresh_sig: "__titanpack_refresh__.signature".to_string(),
                            ..Default::default()
                        })
                    } else {
                        None
...
impl Issue for UnsupportedServerActionIssue {
    ...
    fn title(&self) -> Vc<StyledString> {
        StyledString::Text(
            "Server actions (\"use server\") are not yet supported in Titanpack".into(),
        )
        .cell()
    }

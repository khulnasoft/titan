...
use titan_tasks::{trace::TraceRawVcs, TryFlatJoinIterExt, TryJoinIterExt, Vc};
use titanpack_core::chunk::{AsyncModuleInfo, ChunkableModule};
...
fn add_async_dependency_handler(program: &mut Program, idents: &IndexSet<String>) {
    ...
    let stmt = quote!(
        "var __titanpack_async_dependencies__ = __titanpack_handle_async_dependencies__($deps);"
            as Stmt,
    ...
    let stmt = quote!(
        "($deps = __titanpack_async_dependencies__.then ? (await \
         __titanpack_async_dependencies__)() : __titanpack_async_dependencies__);" as Stmt,

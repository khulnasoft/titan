...
use titan_tasks::{primitives::Regex, Value, ValueToString, Vc};
use titan_tasks_fs::{DirectoryContent, DirectoryEntry, FileSystemPath};
use titanpack_core::{
...
/// wrapped in `__titanpack_require_context__`;
#[titan_tasks::value]
#[derive(Hash, Debug)]
    ...
    async fn code_generation(
        ...
        visitors.push(create_visitor!(path, visit_mut_expr(expr: &mut Expr) {
            if let Expr::Call(_) = expr {
                *expr = quote!(
                    "__titanpack_require_context__(__titanpack_require__($id))" as Expr,
...
impl EcmascriptChunkItem for RequireContextChunkItem {
    ...
    async fn content(&self) -> Result<Vc<EcmascriptChunkItemContent>> {
        ...
        let expr = quote_expr!(
            "__titanpack_export_value__($obj);",

    ...
    async fn code_generation(
        ...
        let path = as_abs_path(self.path).await?.as_str().map_or_else(
            ...
            |path| {
                ...
                quote!(
                    "`file://${__titanpack_resolve_absolute_path__($formatted)}`" as Expr,
        ...
        let visitor = create_visitor!(visit_mut_program(program: &mut Program) {
            // [NOTE] url property is lazy-evaluated, as it should be computed once titanpack_runtime injects a function

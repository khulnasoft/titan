        ...
        let visitor = match &*pm {
            ...
            PatternMapping::SingleLoader(_) => {
                create_visitor!(exact path, visit_mut_call_expr(call_expr: &mut CallExpr) {
                    ...
                    call_expr.callee = Callee::Expr(quote_expr!(
                        "__titanpack_require__($arg)",
                        arg: Expr = expr
                    ));
                    call_expr.args = vec![
                        ExprOrSpread { spread: None, expr: quote_expr!("__titanpack_import__") },
                    ];
                })
            }
            ...
            | PatternMapping::OriginalReferenceExternal => {
                create_visitor!(exact path, visit_mut_call_expr(call_expr: &mut CallExpr) {
                    ...
                    if import_externals {
                        call_expr.callee = Callee::Expr(quote_expr!("__titanpack_external_import__"));
                        call_expr.args = vec![
                        ...
                        call_expr.args = vec![
                            ExprOrSpread { spread: None, expr: quote_expr!(
                                "() => __titanpack_external_require__($arg, true)",
                                arg: Expr = expr
                            ) },
                        ];
                    }

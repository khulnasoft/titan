            ...
            .map(|element| async move {
                Ok(match element {
                    ...
                    AmdDefineDependencyElement::Require => {
                        ResolvedElement::Expr(quote!("__titanpack_require__" as Expr))
                    }
                })
            })
            .try_join()
...
///   factory(
///     __titanpack_require__(dep1),
///     __titanpack_require__(dep2),
///   ),
/// );
/// ```
fn transform_amd_factory(
            ...
            ResolvedElement::PatternMapping(pm, req) => match &**pm {
                ...
                pm => {
                    ...
                    if pm.is_internal_import() {
                        quote_expr!("__titanpack_require__($arg)", arg: Expr = arg)
                    } else {
                        quote_expr!("__titanpack_external_require__($arg)", arg: Expr = arg)
                    }
                }
            },
    ...
    match factory_type {
        ...
        AmdDefineFactoryType::Function => {
            // (r => r !== undefined && __titanpack_export_value__(r))(...([...]))
            *callee = Callee::Expr(quote_expr!(
                "r => r !== undefined && __titanpack_export_value__(r)"
            ));
            args.push(ExprOrSpread {
        ...
        AmdDefineFactoryType::Value => {
            // __titanpack_export_value__(...)
            *callee = Callee::Expr(quote_expr!("__titanpack_export_value__"));
            args.push(ExprOrSpread {
                expr: factory,
                spread: None,
            });

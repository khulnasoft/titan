...
use titan_tasks::{trace::TraceRawVcs, Upcast, ValueToString, Vc};
use titan_tasks_fs::rope::Rope;
use titanpack_core::{
...
impl EcmascriptChunkItemContent {
    ...
    pub async fn module_factory(self: Vc<Self>) -> Result<Vc<Code>> {
        ...
        if this.options.async_module.is_some() {
            args.push("a: __titanpack_async_module__");
        }
        if this.options.externals {
            args.push("x: __titanpack_external_require__");
            args.push("y: __titanpack_external_import__");
        }
        if this.options.refresh {
            args.push("k: __titanpack_refresh__");
        }
        ...
        if this.options.wasm {
            args.push("w: __titanpack_wasm__");
            args.push("u: __titanpack_wasm_module__");
        }
        ...
        if this.options.async_module.is_some() {
            code += "__titanpack_async_module__(async (__titanpack_handle_async_dependencies__, \
                     __titanpack_async_result__) => { try {";
        }
        ...
        if let Some(opts) = &this.options.async_module {
            write!(
                code,
                "__titanpack_async_result__();\n}} catch(e) {{ __titanpack_async_result__(e); }} \
                 }}, {});",
...
pub struct EcmascriptChunkItemOptions {
    ...
    /// `__titanpack_refresh__` argument.
    pub refresh: bool,
    ...
    pub require: bool,
    /// Whether this chunk item's module factory should include a
    /// `__titanpack_external_require__` argument.
    pub externals: bool,
    ...
    /// `__titanpack_wasm__` to load WebAssembly.
    pub wasm: bool,
    pub placeholder_for_future_extensions: (),
}

        ...
        if changed {
                ...
                .filter_map(|p| p.as_os_str().to_str())
            {
                ...
                if is_workspace {
                    ...
                    let titan_json = path_workspace.join_component("titan.json");

                    let (package_exists, titan_exists) = join!(
                        tokio::fs::try_exists(&package_json),
                        tokio::fs::try_exists(&titan_json)
                    );
                    ...
                    if let Ok(true) = package_exists {
                        data.insert(
                            ...
                            WorkspaceData {
                                package_json,
                                titan_json: titan_exists.unwrap_or_default().then_some(titan_json),
                            },
                        );
                    } else {

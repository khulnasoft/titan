...
fn is_attribute(attr: &Attribute, name: &str) -> bool {
    ...
    match iter.next() {
        Some(seg) if seg.arguments.is_empty() && seg.ident == "titan_tasks" => match iter.next() {
...
fn strip_function_attribute<'a>(item: &'a ImplItem, attrs: &'a [Attribute]) -> Vec<&'a Attribute> {
    ...
    if function_attrs.is_empty() {
        item.span()
            .unwrap()
            .error("#[titan_tasks::function] attribute missing")
            .emit();
    }
    attrs
}
    ...
    fn inherent_value_impl(ty: &Type, ty_ident: &Ident, items: &[ImplItem]) -> TokenStream2 {
        ...
        for item in items.iter() {
            ...
            }) = item
            {
                ...
                let Some(titan_fn) = TitanFn::new(sig, DefinitionContext::ValueInherentImpl) else {
                    return quote! {
                ...
                let titan_block = titan_fn.static_block(&native_function_id_ident);
                exposed_impl_items.push(quote! {
                    #(#attrs)*
                    #vis #titan_signature #titan_block
                });
    ...
    fn trait_value_impl(
        ...
        for item in items.iter() {
            ...
            }) = item
            {
                ...
                let Some(titan_fn) = TitanFn::new(sig, DefinitionContext::ValueTraitImpl) else {
                    return quote! {
                        // An error occurred while parsing the function signature.
                    };
                };
                ...
                trait_registers.push(quote! {
                    value.register_trait_method(<Box<dyn #trait_path> as titan_tasks::VcValueTrait>::get_trait_type_id(), stringify!(#ident).into(), *#native_function_id_ident);
                });
            }
        }
        ...
        quote! {
            ...
            pub(crate) fn #register(value: &mut titan_tasks::ValueType) {
                value.register_trait(<Box<dyn #trait_path> as titan_tasks::VcValueTrait>::get_trait_type_id());
                #(#trait_registers)*
            }

            // NOTE(alexkirsz) We can't have a general `titan_tasks::Upcast<Box<dyn Trait>> for T where T: Trait` because
            ...
            unsafe impl #impl_generics titan_tasks::Upcast<Box<dyn #trait_path>> for #ty #where_clause {}

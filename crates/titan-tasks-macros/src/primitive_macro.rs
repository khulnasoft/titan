...
use titan_tasks_macros_shared::{get_type_ident, PrimitiveInput};
...
pub fn primitive(input: TokenStream) -> TokenStream {
    ...
    let value_debug_impl = quote! {
        ...
        impl titan_tasks::debug::ValueDebug for #ty {
    ...
    let value_type_and_register = value_type_and_register(
        ...
        None,
        quote! {
            titan_tasks::VcTransparentRead<#ty, #ty, #ty>
        },
        quote! {
            titan_tasks::VcCellSharedMode<#ty>
        },
        quote! {
            titan_tasks::ValueType::new_with_any_serialization::<#ty>()
        },
    );

    let value_default_impl = quote! {
        #[titan_tasks::value_impl]
        impl titan_tasks::ValueDefault for #ty {
            #[titan_tasks::function]

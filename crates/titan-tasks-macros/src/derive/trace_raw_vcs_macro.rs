...
pub fn derive_trace_raw_vcs(input: TokenStream) -> TokenStream {
    ...
    for type_param in derive_input.generics.type_params_mut() {
        type_param
            .bounds
            .push(syn::parse_quote!(titan_tasks::trace::TraceRawVcs));
    }
    let (impl_generics, ty_generics, where_clause) = derive_input.generics.split_for_impl();

    let trace_items = match_expansion(&derive_input, &trace_named, &trace_unnamed, &trace_unit);
    quote! {
        impl #impl_generics titan_tasks::trace::TraceRawVcs for #ident #ty_generics #where_clause {
            fn trace_raw_vcs(&self, __context__: &mut titan_tasks::trace::TraceRawVcsContext) {
                #trace_items
            }
        }
    }
...
fn trace_named(_ident: &Ident, fields: &FieldsNamed) -> (TokenStream2, TokenStream2) {
    ...
    (
        ...
        quote! {
            {#(
                titan_tasks::trace::TraceRawVcs::trace_raw_vcs(#fields_idents, __context__);
            )*}
        },
    )
}

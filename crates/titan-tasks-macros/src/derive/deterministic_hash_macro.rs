...
use titan_tasks_macros_shared::{generate_exhaustive_destructuring, match_expansion};
...
pub fn derive_deterministic_hash(input: TokenStream) -> TokenStream {
    ...
    let discriminant = match derive_input.data {
        Data::Enum(_) => {
            quote! {
                titan_tasks_hash::DeterministicHash::deterministic_hash(&std::mem::discriminant(self), __state__);
            }
        }
        _ => quote! {},
    };
    ...
    quote! {
        impl titan_tasks_hash::DeterministicHash for #ident {
            fn deterministic_hash<H: titan_tasks_hash::DeterministicHasher>(&self, __state__: &mut H) {
                #discriminant
                #match_hash
            }
        }

...
use titan_tasks_macros_shared::{generate_destructuring, match_expansion};
...
pub fn derive_value_debug_format(input: TokenStream) -> TokenStream {
    ...
    for type_param in derive_input.generics.type_params_mut() {
        type_param
            .bounds
            .push(syn::parse_quote!(titan_tasks::debug::ValueDebugFormat));
    ...
    quote! {
        impl #impl_generics titan_tasks::debug::ValueDebugFormat for #ident #ty_generics #where_clause {
            fn value_debug_format<'a>(&'a self, depth: usize) -> titan_tasks::debug::ValueDebugFormatString<'a> {
                titan_tasks::debug::ValueDebugFormatString::Async(
                    Box::pin(async move {
                        if depth == 0 {
                            return Ok(stringify!(#ident).to_string());
                        }

                        use titan_tasks::debug::internal::*;
...
fn format_field(value: TokenStream2) -> TokenStream2 {
    quote! {
        titan_tasks::macro_helpers::value_debug_format_field(#value.value_debug_format(depth.saturating_sub(1))).await
    }
}

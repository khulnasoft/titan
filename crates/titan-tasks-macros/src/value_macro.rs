...
pub fn value(args: TokenStream, input: TokenStream) -> TokenStream {
    ...
    let cell_mode = match cell_mode {
        CellMode::New => quote! {
            titan_tasks::VcCellNewMode<#ident>
        },
        CellMode::Shared => quote! {
            titan_tasks::VcCellSharedMode<#ident>
        },
    };
    ...
    let (cell_prefix, cell_access_content, read) = if let Some(inner_type) = inner_type {
        (
            ...
            quote! {
                titan_tasks::VcTransparentRead::<#ident, #inner_type, #ident>
            },
        )
    } else {
        (
            ...
            quote! { content },
            quote! {
                titan_tasks::VcDefaultRead::<#ident>
            },
        )
    };
    ...
    let cell_struct = quote! {
        ...
        #cell_prefix fn cell(self) -> titan_tasks::Vc<Self> {
            let content = self;
            titan_tasks::Vc::cell_private(#cell_access_content)
        }
    };

    let into = if let IntoMode::New | IntoMode::Shared = into_mode {
        quote! {
            impl Into<titan_tasks::Vc<#ident>> for #ident {
                fn into(self) -> titan_tasks::Vc<#ident> {
                    self.cell()
                }
            }
        }
    ...
    let derive = match serialization_mode {
        SerializationMode::None | SerializationMode::Custom | SerializationMode::CustomForInput => {
            quote! {
                #[derive(titan_tasks::trace::TraceRawVcs)]
            }
        }
        SerializationMode::Auto | SerializationMode::AutoForInput => quote! {
            #[derive(titan_tasks::trace::TraceRawVcs, serde::Serialize, serde::Deserialize)]
        },
    };
    let debug_derive = if inner_type.is_some() {
        ...
        quote! {
            #[derive(titan_tasks::debug::ValueDebugFormat, titan_tasks::debug::internal::ValueDebug)]
        }
    };
    ...
    let new_value_type = match serialization_mode {
        SerializationMode::None => quote! {
            titan_tasks::ValueType::new::<#ident>()
        },
        SerializationMode::Auto | SerializationMode::Custom => {
            quote! {
                titan_tasks::ValueType::new_with_any_serialization::<#ident>()
            }
        }
        SerializationMode::AutoForInput | SerializationMode::CustomForInput => {
            quote! {
                titan_tasks::ValueType::new_with_magic_serialization::<#ident>()
            }
        }
    };
    ...
    let for_input_marker = match serialization_mode {
        ...
        SerializationMode::AutoForInput | SerializationMode::CustomForInput => quote! {
            impl titan_tasks::TypedForInput for #ident {}
        },
    };
...
pub fn value_type_and_register(
    ...
    quote! {
        #[doc(hidden)]
        static #value_type_init_ident: titan_tasks::macro_helpers::OnceCell<
            titan_tasks::ValueType,
        > = titan_tasks::macro_helpers::OnceCell::new();
        #[doc(hidden)]
        pub(crate) static #value_type_ident: titan_tasks::macro_helpers::Lazy<&titan_tasks::ValueType> =
        ...
        #[doc(hidden)]
        static #value_type_id_ident: titan_tasks::macro_helpers::Lazy<titan_tasks::ValueTypeId> =
            titan_tasks::macro_helpers::Lazy::new(|| {
                titan_tasks::registry::get_value_type_id(*#value_type_ident)
            });
        ...
        pub(crate) fn #register_value_type_ident(
            ...
            f: impl FnOnce(&mut titan_tasks::ValueType),
        ) {
            #value_type_init_ident.get_or_init(|| {
        ...
        unsafe impl #impl_generics titan_tasks::VcValueType for #ty #where_clause {
            type Read = #read;
            type CellMode = #cell_mode;

            fn get_value_type_id() -> titan_tasks::ValueTypeId {
                *#value_type_id_ident
            }
        }
    }

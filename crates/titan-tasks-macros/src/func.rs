...
impl TitanFn {
            ...
            match possibly_receiver {
                FnArg::Receiver(
                    ...
                    inputs.push(Input {
                        ...
                        ty: parse_quote! { titan_tasks::Vc<Self> },
                    });
                }
                FnArg::Typed(typed) => {
    ...
    /// converted to a standard titan_tasks function signature.
    pub fn signature(&self) -> Signature {
        ...
        parse_quote! {
            fn #ident(#exposed_inputs) -> <#output as titan_tasks::task::TaskOutput>::Return
        }
    }
    ...
    fn converted_inputs(&self) -> Punctuated<Expr, Token![,]> {
        self.inputs
            ...
            .map(|Input { ty, ident }| -> Expr {
                parse_quote! {
                    <#ty as titan_tasks::task::TaskInput>::into_concrete(#ident)
                }
            })
            .collect()
    }
    ...
    pub fn dynamic_block(&self, trait_type_id_ident: &Ident) -> Block {
        ...
        parse_quote! {
            {
                <#output as titan_tasks::task::TaskOutput>::try_from_raw_vc(
                    titan_tasks::trait_call(
                        *#trait_type_id_ident,
...
pub enum DefinitionContext {
    // The function is defined as a naked #[titan_tasks::function].
    NakedFn,
    // The function is defined as a #[titan_tasks::value_impl] inherent implementation method.
    ValueInherentImpl,
    // The function is defined as a #[titan_tasks::value_impl] trait implementation method.
    ValueTraitImpl,
    // The function is defined as a #[titan_tasks::value_trait] default method.
    ValueTrait,
}
...
impl DefinitionContext {
    pub fn function_type(&self) -> &'static str {
        match self {
            DefinitionContext::NakedFn => "#[titan_tasks::function] naked functions",
            DefinitionContext::ValueInherentImpl => "#[titan_tasks::value_impl] inherent methods",
            DefinitionContext::ValueTraitImpl => "#[titan_tasks::value_impl] trait methods",
            DefinitionContext::ValueTrait => "#[titan_tasks::value_trait] methods",
        }
    }
}
...
impl NativeFn {
    ...
    pub fn ty(&self) -> Type {
        parse_quote! { titan_tasks::macro_helpers::Lazy<titan_tasks::NativeFunction> }
    }
    ...
    pub fn definition(&self) -> Expr {
        ...
        parse_quote! {
            titan_tasks::macro_helpers::Lazy::new(|| {
                #[allow(deprecated)]
                titan_tasks::NativeFunction::new(#function_path_string.to_owned(), #function_path)
            })
        }
    }

    pub fn id_ty(&self) -> Type {
        parse_quote! { titan_tasks::macro_helpers::Lazy<titan_tasks::FunctionId> }
    }

    pub fn id_definition(&self, native_function_id_path: &Path) -> Expr {
        parse_quote! {
            titan_tasks::macro_helpers::Lazy::new(|| {
                titan_tasks::registry::get_function_id(&*#native_function_id_path)
            })
        }
    }
}

...
use titan_tasks_macros_shared::{get_native_function_id_ident, get_native_function_ident};
...
pub fn function(_args: TokenStream, input: TokenStream) -> TokenStream {
    ...
    let Some(titan_fn) = TitanFn::new(sig, DefinitionContext::NakedFn) else {
        return quote! {
    ...
    let exposed_signature = titan_fn.signature();
    let exposed_block = titan_fn.static_block(&native_function_id_ident);

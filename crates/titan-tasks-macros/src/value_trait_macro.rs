...
use crate::func::{DefinitionContext, NativeFn, TitanFn};
...
pub fn value_trait(args: TokenStream, input: TokenStream) -> TokenStream {
    ...
    if unsafety.is_some() {
        item.span()
            .unwrap()
            .error("unsafe traits are not supported in #[titan_tasks::value_trait]")
            .emit();
    }
    ...
    if auto_token.is_some() {
        item.span()
            .unwrap()
            .error("auto traits are not supported in #[titan_tasks::value_trait]")
            .emit();
    }
    ...
    if !generics.params.is_empty() {
        item.span()
            .unwrap()
            .error("generic traits are not supported in #[titan_tasks::value_trait]")
            .emit();
    }
    ...
    if generics.where_clause.is_some() {
        item.span()
            .unwrap()
            .error("where clauses are not supported in #[titan_tasks::value_trait]")
            .emit();
    }
    ...
    for item in raw_items.iter() {
        let TraitItem::Method(TraitItemMethod {
            ...
            item.span()
                .unwrap()
                .error("only methods are allowed in a #[titan_tasks::value_trait] trait")
                .emit();
            continue;
        };
        ...
        let Some(titan_fn) = TitanFn::new(sig, DefinitionContext::ValueTrait) else {
            return quote! {
        ...
        let titan_signature = titan_fn.signature();
        let dynamic_block = titan_fn.dynamic_block(&trait_type_id_ident);
        dynamic_trait_fns.push(quote! {
            #titan_signature #dynamic_block
        });
        ...
        let default = if let Some(block) = default {
            ...
            Some(titan_fn.static_block(&native_function_id_ident))
        } else {
            None
        };

        items.push(TraitItem::Method(TraitItemMethod {
            sig: titan_fn.trait_signature(),
            default,
            attrs: attrs.clone(),
    ...
    let value_debug_impl = if debug {
        quote! {
            ...
            impl titan_tasks::debug::ValueDebug for Box<dyn #trait_ident> {
                #[titan_tasks::function]
                pub fn dbg(self: titan_tasks::Vc<Self>) -> titan_tasks::Vc<titan_tasks::debug::ValueDebugString> {
                    use titan_tasks::debug::ValueDebug;
                    self.dbg_depth(usize::MAX)
                }

                #[titan_tasks::function]
                pub async fn dbg_depth(self: titan_tasks::Vc<Self>, depth: usize) -> anyhow::Result<titan_tasks::Vc<titan_tasks::debug::ValueDebugString>> {
                    use titan_tasks::debug::ValueDebugFormat;
                    let string = self.value_debug_format(depth).try_to_value_debug_string().await?.await?;
                    Ok(titan_tasks::debug::ValueDebugString::new(format!(concat!(stringify!(#trait_ident), "({})"), string)))
                }
            }
        }
    } else {
    ...
    let expanded = quote! {
        ...
        #[doc(hidden)]
        pub(crate) static #trait_type_ident: titan_tasks::macro_helpers::Lazy<titan_tasks::TraitType> =
            titan_tasks::macro_helpers::Lazy::new(|| {
                let mut trait_type = titan_tasks::TraitType::new(stringify!(#trait_ident).to_string());;
                #(#default_method_registers)*
                trait_type
            });
        #[doc(hidden)]
        static #trait_type_id_ident: titan_tasks::macro_helpers::Lazy<titan_tasks::TraitTypeId> =
            titan_tasks::macro_helpers::Lazy::new(|| {
                titan_tasks::registry::get_trait_type_id(&#trait_type_ident)
            });

        impl titan_tasks::VcValueTrait for Box<dyn #trait_ident> {
            fn get_trait_type_id() -> titan_tasks::TraitTypeId {
                *#trait_type_id_ident
            }
        }

        unsafe impl titan_tasks::Dynamic<Box<dyn #trait_ident>> for Box<dyn #trait_ident> {}
        ...
        unsafe impl titan_tasks::Upcast<Box<dyn #trait_ident>> for Box<dyn #trait_ident> {}

        impl<T> #trait_ident for T
        where
            T: titan_tasks::Dynamic<Box<dyn #trait_ident>> #(+ #supertraits)*,
        {
            #(#dynamic_trait_fns)*
        }

        #(
            unsafe impl titan_tasks::Dynamic<Box<dyn #supertraits>> for Box<dyn #trait_ident> {}
            unsafe impl titan_tasks::Upcast<Box<dyn #supertraits>> for Box<dyn #trait_ident> {}
        )*

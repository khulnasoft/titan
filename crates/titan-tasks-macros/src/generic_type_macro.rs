...
use titan_tasks_macros_shared::{get_type_ident, GenericTypeInput};
...
pub fn generic_type(input: TokenStream) -> TokenStream {
    ...
    let value_type_and_register = value_type_and_register(
        ...
        quote! {
            titan_tasks::VcTransparentRead<#ty, #ty, #repr>
        },
        quote! {
            titan_tasks::VcCellSharedMode<#ty>
        },
        quote! {
            titan_tasks::ValueType::new_with_any_serialization::<#repr>()
        },
    );
    ...
    quote! {
        ...
        impl #impl_generics Vc<#ty> #where_clause {
            ...
            fn to_repr(vc: Self) -> Vc<#repr> {
                unsafe {
                    titan_tasks::Vc::from_raw(Vc::into_raw(vc))
                }
            }

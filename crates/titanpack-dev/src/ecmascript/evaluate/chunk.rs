...
use indoc::writedoc;
use serde::Serialize;
use titan_tasks::{ReadRef, TryJoinIterExt, Value, ValueToString, Vc};
...
/// * Contains the Titanpack dev runtime code; and
...
impl EcmascriptDevEvaluateChunk {
    ...
    async fn code(self: Vc<Self>) -> Result<Vc<Code>> {
        ...
        // We still use the `TITANPACK` global variable to store the chunk here,
        ...
        match chunking_context.runtime_type() {
            RuntimeType::Default => {
                let runtime_code = titanpack_ecmascript_runtime::get_dev_runtime_code(
                    environment,
            ...
            #[cfg(feature = "test")]
            RuntimeType::Dummy => {
                let runtime_code = titanpack_ecmascript_runtime::get_dummy_runtime_code();

declare const __titanpack_external_require__: {
...
try {
  ({ runLoaders } = require("@khulnasoft/titanpack/loader-runner"));
} catch {
  ({ runLoaders } = __titanpack_external_require__("loader-runner"));
}
    ...
    runLoaders(
      {
        ...
        loaders: loadersWithOptions.map((loader) => ({
          loader: __titanpack_external_require__.resolve(loader.loader, {
            paths: [resourceDir],
          }),

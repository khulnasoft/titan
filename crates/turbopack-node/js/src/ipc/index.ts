  ...
  function send(message: any): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      socket.write(packet, (err) => {
        process.stderr.write(`TITANPACK_OUTPUT_D\n`);
        process.stdout.write(`TITANPACK_OUTPUT_D\n`);
        if (err != null) {
          reject(err);
        } else {
          resolve();
...
const improveConsole = (name: string, stream: string, addStack: boolean) => {
  ...
  console[name] = (...args: any[]) => {
    stdio.write(`TITANPACK_OUTPUT_B\n`);
    original(...args);
    if (addStack) {
      const stack = new Error().stack?.replace(/^.+\n.+\n/, "") + "\n";
      stdio.write("TITANPACK_OUTPUT_S\n");
      stdio.write(stack);
    }
    stdio.write("TITANPACK_OUTPUT_E\n");
  };
};

...
use titan_tasks::{duration_span, mark_finished, util::SharedError, RawVc, ValueToString, Vc};
use titan_tasks_bytes::{Bytes, Stream};
...
use titanpack_dev_server::source::{Body, ProxyResult};
...
#[titan_tasks::value(eq = "manual", cell = "new", serialization = "none")]
struct RenderStreamSender {
    #[titan_tasks(trace_ignore, debug_ignore)]
    get: Box<dyn Fn() -> UnboundedSender<RenderItemResult> + Send + Sync>,
}

#[titan_tasks::value(transparent)]
struct RenderStream(#[titan_tasks(trace_ignore)] Stream<RenderItemResult>);
...
fn render_stream(
    ...
    let cell = titan_tasks::macro_helpers::find_cell_by_type(*RENDERSTREAM_VALUE_TYPE_ID);

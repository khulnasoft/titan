    ...
    let stylesheet = if use_lightningcss {
        ...
        StyleSheetLike::Swc {
            ...
            css_modules: if matches!(ty, CssModuleAssetType::Module) {
                ...
                let path_hash = titan_tasks_hash::hash_xxh3_hash64(ident_str) as u32;
    ...
    fn generate_source_map(&self) -> Vc<OptionSourceMap> {
        match self {
            ParseCssResultSourceMap::Parcel { source_map } => {
                ...
                Vc::cell(Some(
                    titanpack_core::source_map::SourceMap::new_regular(builder.into_sourcemap())
                        .cell(),
                ))
            }
            ParseCssResultSourceMap::Swc {
                ...
                Vc::cell(Some(
                    titanpack_core::source_map::SourceMap::new_regular(map).cell(),
                ))
            }
        }
    }

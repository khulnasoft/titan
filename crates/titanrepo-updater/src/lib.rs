...
//! Titanrepo's update notifier. Indicates to the user that there is a new
//! version of `titan` available.
...
const NOTIFIER_DISABLE_VARS: [&str; 2] = ["NO_UPDATE_NOTIFIER", "TITAN_NO_UPDATE_NOTIFIER"];
...
pub fn check_for_updates(
    ...
    if let Ok(Some(version)) = informer.check_version() {
        ...
        let update_cmd = style("npx @titan/codemod@latest update").cyan().bold();

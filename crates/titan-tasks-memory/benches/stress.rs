...
use titan_tasks::{TitanTasks, TryJoinIterExt, Vc};
...
pub fn fibonacci(c: &mut Criterion) {
    if matches!(
        std::env::var("TITANPACK_BENCH_STRESS").ok().as_deref(),
    ...
    let mut group = c.benchmark_group("titan_tasks_memory_stress");
...
/// Computes a fibonacci number in a recursive matter. Due to titan-tasks this
/// will result in a lot of cached tasks, so its performance is only O(n)
/// (compared to non-titan-tasks O(1.6^N)).

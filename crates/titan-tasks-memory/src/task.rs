...
impl Task {
    ...
    pub(crate) fn set_root(
        ...
        let mut aggregation_context = TaskAggregationContext::new(titan_tasks, backend);
        {
    ...
    fn clear_dependencies(
        ...
        for dep in dependencies.into_iter() {
            Task::remove_dependency(dep, self.id, backend, titan_tasks);
        }
    }
    ...
    pub(crate) fn execute(
        ...
        let future;
        {
            ...
            future = self.make_execution_future(state, backend, titan_tasks);
        }
        aggregation_context.apply_queued_updates();
        self.clear_dependencies(dependencies, backend, titan_tasks);
        ...
        match &self.ty {
            ...
            TaskType::Persistent { ty, .. } => match &**ty {
                ...
                PersistentTaskType::ResolveNative(ref native_fn, inputs) => {
                    ...
                    let titan_tasks = titan_tasks.pin();
            ...
            InProgress { .. } => match result {
                Ok(Ok(result)) => {
                    if state.output != result {
                        ...
                        state.output.link(result, titan_tasks)
                    }
                }
                Ok(Err(mut err)) => {
                    ...
                    state.output.error(err, titan_tasks)
                }
                Err(message) => state.output.panic(message, titan_tasks),
            },
            InProgressDirty { .. } => {
    ...
    pub(crate) fn execution_completed(
        ...
        let mut schedule_task = false;
        {
            ...
            let mut dependencies = DEPENDENCIES_TO_TRACK.with(|deps| deps.take());
            {
                ...
                state
                    .stats
                    .register_execution(duration, titan_tasks.program_duration_until(instant));
                match state.state_type {
                    InProgress {
                        ref mut event,
                        count_as_finished,
    ...
    fn make_dirty(
        ...
        self.make_dirty_internal(false, backend, titan_tasks);
    }
                ...
                Dirty {
                    ...
                    if force_schedule {
                        ...
                        state.aggregation_leaf.change(
                            &TaskAggregationContext::new(titan_tasks, backend),
                            &TaskChange {
    ...
    pub(crate) fn schedule_when_dirty_from_aggregation(
        ...
        let aggregation_context = TaskAggregationContext::new(titan_tasks, backend);
        let mut state = self.full_state_mut();
    ...
    pub(crate) fn invalidate(
        ...
        self.make_dirty(backend, titan_tasks)
    }
    ...
    pub(crate) fn recompute(
        ...
        self.make_dirty_internal(true, backend, titan_tasks)
    }
                ...
                backend.with_task(child_id, |child| {
                    if child.is_dirty() {
                        let active = self
                            ...
                            .get_root_info(&aggregation_context, &RootInfoType::IsActive);
                        if active {
                            child.schedule_when_dirty_from_aggregation(backend, titan_tasks);
                        }
                    }
                });
                job();
                    ...
                    if missing_durations.is_empty() {
                        ...
                        if !active {
                            ...
                            if new_priority <= max_priority {
                                // Unload task
                                if self.unload(state, backend, titan_tasks) {
                                    stats.unloaded += 1;
                                    return None;
                                } else {
    ...
    fn unload(
        ...
        for cells in cells.into_values() {
            for cell in cells {
                cell.gc_drop(titan_tasks);
            }
        }
        output.gc_drop(titan_tasks);

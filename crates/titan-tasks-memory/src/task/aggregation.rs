...
pub struct TaskAggregationContext<'a> {
    pub titan_tasks: &'a dyn TitanTasksBackendApi<MemoryBackend>,
    ...
    pub fn apply_queued_updates(&mut self) {
        {
            ...
            if let Some(tasks) = tasks.as_mut() {
                ...
                if !tasks.is_empty() {
                    ...
                    self.backend
                        .schedule_when_dirty_from_aggregation(tasks, self.titan_tasks);
                }
            }
        }
        ...
        if let Some(tasks) = tasks.as_mut() {
            let tasks = take(tasks);
            if !tasks.is_empty() {
                self.titan_tasks.schedule_notify_tasks_set(&tasks);
            }
        }
    }

...
pub struct MemoryBackendWithPersistedGraph<P: PersistedGraph + 'static> {
    ...
    partial_lookup: titan_tasks::util::OnceConcurrentlyMap<PersistentTaskType, bool>,
    #[cfg(not(feature = "unsafe_once_map"))]
    partial_lookup: titan_tasks::util::SafeOnceConcurrentlyMap<PersistentTaskType, bool>,
...
impl<P: PersistedGraph> MemoryBackendWithPersistedGraph<P> {
    pub fn new(pg: P) -> Self {
        ...
        Self {
            ...
            #[cfg(feature = "unsafe_once_map")]
            partial_lookup: titan_tasks::util::OnceConcurrentlyMap::new(),
            #[cfg(not(feature = "unsafe_once_map"))]
            partial_lookup: titan_tasks::util::SafeOnceConcurrentlyMap::new(),
            #[cfg(feature = "log_running_tasks")]
    ...
    fn state_mut(
        &self,
        task: TaskId,
        titan_tasks: &dyn TitanTasksBackendApi<MemoryBackendWithPersistedGraph<P>>,
    ) -> (MutexGuard<'_, TaskState>, &Task) {
        let task_info = self.tasks.get(*task).unwrap();
        let mut state = task_info.task_state.lock().unwrap();
        self.ensure_task_initialized(task, task_info, &mut state, titan_tasks);
        (state, task_info)
    }
    ...
    fn mem_state_mut(
        ...
        loop {
            ...
            let mut state = task_info.task_state.lock().unwrap();
            self.ensure_task_in_memory(task, &mut state, &mut delayed_activate, titan_tasks);
            ...
            loop {
                for task in take(&mut delayed_activate) {
                    let (state, task_info) = self.state_mut(task, titan_tasks);
    ...
    fn ensure_task_initialized(
        ...
        if task_state.memory.is_none() && task_state.persisted.is_none() {
            if let TaskType::Persistent(_) = &task_info.task_type {
                if self.pg_is_persisted(task, titan_tasks) {
    ...
    fn ensure_task_in_memory(
        ...
        if task_state.memory.is_none() {
            if let Some((data, state)) = self.pg_read(task, titan_tasks) {
    ...
    fn lookup(
        ...
        for i in 0..task_type.len() {
            ...
            let complete = complete_cached.unwrap_or_else(|| {
                self.partial_lookup.action(&partial, || {
                    let complete = self.pg_lookup(&partial, titan_tasks);
        ...
        self.pg_lookup_one(task_type, titan_tasks)
    }
    ...
    fn connect(
        ...
        let (mut state, _) = self.mem_state_mut(parent_task, titan_tasks);
        ...
        if memory_active {
            self.increment_active_parents(task, 1, titan_tasks);
        }
    }
    ...
    fn connect_already_counted(
        ...
        if !mem_state.children.insert(task) {
            ...
            drop(state);
            self.decrement_active_parents(task, 1, titan_tasks);
            return;
        }
        #[cfg(feature = "log_connect_tasks")]
        println!(
    ...
    fn schedule_background_job(
        ...
        titan_tasks.schedule_backend_background_job(id);
    }
    ...
    fn activate_persisted(
        ...
        }) = self.pg_activate_when_needed(task, titan_tasks)
        {
            for task in more_tasks_to_activate {
                self.schedule_background_job(BackgroundJob::ActivatePersisted(task), titan_tasks);
            }
            let (mut state, task_info) = self.state_mut(task, titan_tasks);
                ...
                && state.persisted_to_mem_active != keeps_external_active
            {
                ...
                if keeps_external_active {
                    self.activate_task(task, state, task_info, titan_tasks)
                } else {
                    self.deactivate_task(task, state, task_info, titan_tasks)
                }
            }
        }
    }
    ...
    fn deactivate_persisted(
        ...
        }) = self.pg_deactivate_when_needed(task, titan_tasks)
        {
            for task in more_tasks_to_deactivate {
                self.schedule_background_job(BackgroundJob::DeactivatePersisted(task), titan_tasks);
            }
            let (mut state, task_info) = self.state_mut(task, titan_tasks);
    ...
    fn increment_active_parents(
        ...
        let mut delayed_activate = Vec::new();
        self.try_increment_active_parents(task, true, by, &mut delayed_activate, titan_tasks);
    ...
    fn try_increment_active_parents(
        ...
        if prev == 0 {
            ...
            self.ensure_task_initialized(task, task_info, &mut state, titan_tasks);
            self.activate_task_inner(task, state, task_info, delayed_activate, titan_tasks);
        }
    }
    ...
    fn activate_task(
        ...
        let mut delayed_activate = Vec::new();
        self.activate_task_inner(task, state, task_info, &mut delayed_activate, titan_tasks);
        while !delayed_activate.is_empty() {
            for task in take(&mut delayed_activate) {
                let (state, task_info) = self.state_mut(task, titan_tasks);
                self.activate_task_inner(
    ...
    fn activate_task_inner(
        ...
        if persisted.is_some() && !*mem_to_persisted_active && has_active_parents {
            *mem_to_persisted_active = true;
            if self.pg_set_externally_active(task, titan_tasks) {
                activate_persisted = true;
            }
        }
        if *mem_to_persisted_active && !has_active_parents {
            *mem_to_persisted_active = false;
            if self.pg_unset_externally_active(task, titan_tasks) {
        ...
        if activate_persisted {
            self.activate_persisted(task, titan_tasks);
        }
        if deactivate_persisted {
            self.deactivate_persisted(task, titan_tasks);
        }
    }
    ...
    fn decrement_active_parents(
        ...
        self.decrement_active_parents_limited(&[(task, by)], 0, titan_tasks);
    }
        ...
        loop {
                            ...
                            if let TaskType::Persistent(_task_type) = &task_info.task_type {
                                if let &Some(Ok(output)) = &output {
                                    if *has_changes || persisted.is_none() {
                                        ...
                                        let task_state =
                                            titan_tasks::persisted_graph::PersistTaskState {
                                                externally_active,
                                            };
                                        if let Some(PersistResult {
                                            tasks_to_activate,
                                            tasks_to_deactivate,
                                        }) = self.pg_persist(task, data, task_state, titan_tasks)
                                        {
                                            *persisted =
                                        ...
                                        self.pg_make_clean(task, titan_tasks);
                                        return true;
                                    }
                                }
                            }
            ...
            for queue in self.persist_queue_by_duration.iter() {
                ...
                if let Some((_dur, task)) = queue.pop() {
                    let (state, _) = self.state_mut(task, titan_tasks);
                    if let Some(MemoryTaskState {
                        dependencies: _, ..
                    }) = &state.memory
                    {
        ...
        loop {
                ...
                .is_ok()
            {
                for _ in 0..can_start {
                    titan_tasks.schedule_backend_background_job(self.persist_job);
                }
            }
        }
    }
...
impl<P: PersistedGraph> Backend for MemoryBackendWithPersistedGraph<P> {
    fn startup(&self, titan_tasks: &dyn TitanTasksBackendApi<MemoryBackendWithPersistedGraph<P>>) {
        let (tasks_to_activate, tasks_to_deactivate) =
            self.pg_get_pending_active_update(titan_tasks);
        let tasks = self.pg_get_active_external_tasks(titan_tasks);
        let dirty_tasks = self.pg_get_dirty_active_tasks(titan_tasks);
    ...
    fn stop(&self, titan_tasks: &dyn TitanTasksBackendApi<MemoryBackendWithPersistedGraph<P>>) {
        self.pg_stop(titan_tasks);
    }
    ...
    fn invalidate_task(
        ...
        let (mut state, _) = self.state_mut(task, titan_tasks);
        ...
        if let Some(PersistedTaskState { clean, .. }) = &mut state.persisted {
            if *clean != Some(false) && self.pg_make_dirty(task, titan_tasks) {
                *clean = Some(false);
                if !state.scheduled {
                    state.scheduled = true;
                    titan_tasks.schedule(task);
    ...
    fn invalidate_tasks(
        ...
        for &task in tasks {
            self.invalidate_task(task, titan_tasks);
        }
    }
    ...
    fn try_start_task_execution(
        ...
        let (mut state, task_info) = self.mem_state_mut(task, titan_tasks);
        let mem_state = state.memory.as_mut().unwrap();
        ...
        drop(state);
        for dep in deps {
            let (mut state, _) = self.mem_state_mut(dep.get_task_id(), titan_tasks);
            let mem_state = state.memory.as_mut().unwrap();
            match dep {
                RawVc::TaskOutput(_) => {
        ...
        for child in children {
            self.decrement_active_parents(child, 1, titan_tasks);
        }
        let future = match &task_info.task_type {
            TaskType::Persistent(t) => t.clone().run(titan_tasks.pin()),
    ...
    fn task_execution_result(
        ...
        let (mut state, _task_info) = self.mem_state_mut(task, titan_tasks);
        ...
        if !dependent.is_empty() {
            titan_tasks.schedule_notify_tasks_set(&dependent);
        }
    }
        ...
        *scheduled = false;
        ...
        if let TaskType::Persistent(_) = task_info.task_type {
            if has_changes && (is_persisted || !self.only_known_to_memory_tasks.contains(&task)) {
                for task in self.pg_make_dependent_dirty(RawVc::TaskOutput(task), titan_tasks) {
                    let (mut state, _) = self.state_mut(task, titan_tasks);
                    if !state.scheduled {
                        state.scheduled = true;
                        #[cfg(feature = "log_scheduled_tasks")]
            ...
            if has_changes || is_dirty_persisted {
                ...
                self.persist_queue_by_duration[*task % self.persist_queue_by_duration.len()]
                    ...
                    .unwrap()
                    .push((duration, task));
                self.increase_persist_workers(1, titan_tasks);
            }
        }
    ...
    fn run_backend_job<'a>(
        &'a self,
        id: BackendJobId,
        titan_tasks: &'a dyn TitanTasksBackendApi<MemoryBackendWithPersistedGraph<P>>,
    ) -> Pin<Box<dyn Future<Output = ()> + Send + 'a>> {
        if id == self.persist_job {
            return Box::pin(async {
                if self.persist(titan_tasks) {
                    titan_tasks.schedule_backend_background_job(self.persist_job);
                } else {
            ...
            match job {
                BackgroundJob::DeactivateTasks(tasks) => Box::pin(async move {
                    for chunk in tasks.chunks(10) {
                        self.deactivate_tasks(chunk, titan_tasks)
                    }
                }),
                BackgroundJob::ActivatePersisted(task) => {
                    Box::pin(async move { self.activate_persisted(task, titan_tasks) })
                }
                BackgroundJob::DeactivatePersisted(task) => {
                    Box::pin(async move { self.deactivate_persisted(task, titan_tasks) })
                }
            }
        } else {
            Box::pin(async {})
    ...
    fn try_read_task_output(
        ...
        drop(state);
        if need_dependency {
            let (mut state, _) = self.mem_state_mut(reader, titan_tasks);
            let mem_state = state.memory.as_mut().unwrap();
    ...
    fn try_read_task_output_untracked(
        ...
        let (state, task_info) = self.mem_state_mut(task, titan_tasks);
    ...
    fn try_read_task_cell_untracked(
        ...
        let (mut state, _) = self.mem_state_mut(task, titan_tasks);
        let TaskState {
            ref mut scheduled,
            ref mut memory,
            ..
    ...
    fn try_read_own_task_cell_untracked(
        ...
        let (state, _) = self.mem_state_mut(task, titan_tasks);
        let mem_state = state.memory.as_ref().unwrap();
    ...
    fn read_task_collectibles(
        ...
        _reader: TaskId,
        _titan_tasks: &dyn TitanTasksBackendApi<MemoryBackendWithPersistedGraph<P>>,
    ) -> AutoMap<RawVc, i32> {
        todo!()
    }
        ...
        *cell = TaskCell::Content(content);
        ...
        if let TaskType::Persistent(_) = task_info.task_type {
            if is_persisted || !self.only_known_to_memory_tasks.contains(&task) {
                for task in self.pg_make_dependent_dirty(RawVc::TaskCell(task, index), titan_tasks)
                {
                    let (mut state, _) = self.state_mut(task, titan_tasks);
                    if !state.scheduled {
                        state.scheduled = true;
    ...
    fn get_or_create_persistent_task(
        ...
        if let Some(task) = self.cache.get(&task_type) {
            self.connect(parent_task, *task, titan_tasks);
            return *task;
        }
        if let Some(task) = self.lookup(&task_type, titan_tasks) {
            // a return value from lookup was already added to the cache by the id mapping
            self.connect(parent_task, task, titan_tasks);
            return task;
        }

        let task = titan_tasks.get_fresh_task_id();
        let task = task.into();
        let new_task = Task {
        ...
        match self.cache.entry(task_type) {
            Entry::Occupied(e) => {
                ...
                unsafe {
                    ...
                    titan_tasks.reuse_task_id(task);
                }
                self.connect(parent_task, existing_task, titan_tasks);
                existing_task
            }
            Entry::Vacant(e) => {
                e.insert(task);
                ...
                titan_tasks.schedule(task);
                self.connect_already_counted(parent_task, task, titan_tasks);
                task
            }
        }
    }
    ...
    fn dispose_root_task(&self, _task: TaskId, _titan_tasks: &dyn TitanTasksBackendApi<Self>) {
        todo!()
    }
}
...
impl<'a, P: PersistedGraph> PersistedGraphApi for MemoryBackendPersistedGraphApi<'a, P> {
    fn get_or_create_task_type(&self, task_type: PersistentTaskType) -> TaskId {
        ...
        let task = self.titan_tasks.get_fresh_task_id();
        let task = task.into();
        // SAFETY: It's a fresh task id
        unsafe {
        ...
        match cache.entry(task_type) {
            Entry::Occupied(e) => {
                ...
                unsafe {
                    let task = Unused::new_unchecked(task);
                    self.titan_tasks.reuse_task_id(task);
                }
                value
            }
            Entry::Vacant(e) => {
...
impl<P: PersistedGraph> MemoryBackendWithPersistedGraph<P> {
    ...
    fn pg_stop(&self, titan_tasks: &dyn TitanTasksBackendApi<MemoryBackendWithPersistedGraph<P>>) {
        self.pg

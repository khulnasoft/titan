...
impl MemoryBackend {
    ...
    fn connect_task_child(
        ...
        self.with_task(parent, |parent| {
            parent.connect_child(child, self, titan_tasks)
        });
    }
    ...
    fn try_get_output<T, F: FnOnce(&mut Output) -> Result<T>>(
        ...
        self.with_task(id, |task| {
            task.get_or_wait_output(strongly_consistent, func, note, self, titan_tasks)
        })
    }
    ...
    pub fn run_gc(&self, idle: bool, titan_tasks: &dyn TitanTasksBackendApi<MemoryBackend>) {
        if let Some(gc_queue) = &self.gc_queue {
            const MAX_COLLECT_FACTOR: u8 = u8::MAX / 8;

            let mem_limit = self.memory_limit;

            let usage = titan_tasks_malloc::TitanMalloc::memory_usage();
            let target = if idle {
                mem_limit * 3 / 4
            } else {
                mem_limit * 7 / 8
            ...
            if usage < target {
                if idle {
                    ...
                    gc_queue.run_gc(0, self, titan_tasks);
                }
                return;
            }
            ...
            let collected = gc_queue.run_gc(collect_factor, self, titan_tasks);

            if idle {
                if let Some((_collected, _count, _stats)) = collected {
                    let job = self.create_backend_job(Job::GarbageCollection);
                    titan_tasks.schedule_backend_background_job(job);
                } else {
        ...
        let result_task = match task_cache.entry(key) {
            ...
            Entry::Occupied(entry) => {
                ...
                unsafe {
                    ...
                    titan_tasks.reuse_task_id(new_id);
                }
                *entry.get()
            }
        };
        self.connect_task_child(parent_task, result_task, titan_tasks);
        result_task
    }
        ...
        Q: Hash + Eq + ?Sized,
    {
        task_cache.get(key).map(|task| {
            self.connect_task_child(parent_task, *task, titan_tasks);
    ...
    pub(crate) fn schedule_when_dirty_from_aggregation(
        ...
        for task in set {
            self.with_task(task, |task| {
                task.schedule_when_dirty_from_aggregation(self, titan_tasks)
            });
        }
    }
}
...
impl Backend for MemoryBackend {
    fn idle_start(&self, titan_tasks: &dyn TitanTasksBackendApi<MemoryBackend>) {
        if self
            .idle_gc_active
    ...
    fn invalidate_task(&self, task: TaskId, titan_tasks: &dyn TitanTasksBackendApi<MemoryBackend>) {
        self.with_task(task, |task| task.invalidate(self, titan_tasks));
    }
    ...
    fn invalidate_tasks(
        ...
        for &task in tasks {
            self.with_task(task, |task| {
                task.invalidate(self, titan_tasks);
            });
        }
    }
    ...
    fn try_start_task_execution(
        ...
        self.with_task(task, |task| task.execute(self, titan_tasks))
    }
    ...
    fn task_execution_result(
        ...
        self.with_task(task_id, |task| {
            ...
            task.execution_result(result, self, titan_tasks);
        })
    }
    ...
    fn task_execution_completed(
        ...
        let reexecute = self.with_task(task_id, |task| {
            task.execution_completed(duration, instant, stateful, self, titan_tasks)
        });
        if !reexecute {
            self.run_gc(false, titan_tasks);
            if let Some(gc_queue) = &self.gc_queue {
            ...
            self.with_task(task_id, |task| {
                match task.with_cell_mut(index, |cell| {
                    ...
                    Err(RecomputingCell { listener, schedule }) => {
                        if schedule {
                            task.recompute(self, titan_tasks);
                        }
                        Ok(Err(listener))
                    }
                }
    ...
    fn try_read_own_task_cell_untracked(
        ...
        current_task: TaskId,
        index: CellId,
        _titan_tasks: &dyn TitanTasksBackendApi<MemoryBackend>,
    ) -> Result<CellContent> {
    ...
    fn read_task_collectibles(
        ...
        Task::read_collectibles(id, trait_id, reader, self, titan_tasks)
    }
    ...
    fn emit_collectible(
        ...
        self.with_task(id, |task| {
            task.emit_collectible(trait_type, collectible, self, titan_tasks)
        });
    }
    ...
    fn unemit_collectible(
        ...
        self.with_task(id, |task| {
            task.unemit_collectible(trait_type, collectible, count, self, titan_tasks);
        });
    }
    ...
    fn update_task_cell(
        ...
        self.with_task(task, |task| {
            task.with_cell_mut(index, |cell| cell.assign(content, titan_tasks))
        })
    }
    ...
    fn get_or_create_persistent_task(
        ...
        if let Some(task) =
            self.lookup_and_connect_task(parent_task, &self.task_cache, &task_type, titan_tasks)
        {
    ...
    fn mark_own_task_as_finished(
        ...
        self.with_task(task, |task| task.mark_as_finished(self, titan_tasks))
    }
    ...
    fn create_transient_task(
        ...
        let id = titan_tasks.get_fresh_task_id();
        let stats_type = titan_tasks.stats_type();
        let id = id.into();
        match task_type {
            TransientTaskType::Root(f) => {
                ...
                Task::set_root(id, self, titan_tasks);
            }
            TransientTaskType::Once(f) => {
                let task = Task::new_once(id, f, stats_type);
                // SAFETY: We have a fresh task id where nobody knows about yet
                unsafe { self.memory_tasks.insert(*id, task) };
                Task::set_once(id, self, titan_tasks);
            }
        };
        id
    }

    fn dispose_root_task(&self, task: TaskId, titan_tasks: &dyn TitanTasksBackendApi<Self>) {
        Task::unset_root(task, self, titan_tasks);
    }
}
    ...
    async fn run(
        ...
        match self {
            Job::GarbageCollection => {
                ...
                backend.run_gc(true, titan_tasks);
            }
        }
    }
}

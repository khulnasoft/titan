...
use titanpack_cli_utils::issue::{ConsoleUi, IssueSeverityCliOption, LogOptions};
use titanpack_core::{
    ...
    for entry in result.results.values() {
        if let DirectoryEntry::File(path) = entry {
            ...
            let module = asset_context
                .process(
                    source,
                    Value::new(titanpack_core::reference_type::ReferenceType::Undefined),
                )
                .module();
            list.push(module);
        }
...
pub async fn start(
    args: Arc<Args>,
    titan_tasks: Option<&Arc<TitanTasks<MemoryBackend>>>,
    ...
    run(
        args.clone(),
        || {
            titan_tasks.cloned().unwrap_or_else(|| {
                TitanTasks::new(MemoryBackend::new(memory_limit.unwrap_or(usize::MAX)))
            })
        },
...
async fn run<B: Backend + 'static, F: Future<Output = ()>>(
    args: Arc<Args>,
    create_tt: impl Fn() -> Arc<TitanTasks<B>>,
    final_finish: impl FnOnce(Arc<TitanTasks<B>>, TaskId, Duration) -> F,
    module_options: Option<ModuleOptionsContext>,
    resolve_options: Option<ResolveOptionsContext>,
) -> Result<Vec<String>> {
    let &CommonArgs {
    ...
    let start = Instant::now();
    let finish = |tt: Arc<TitanTasks<B>>, root_task: TaskId| async move {
        if watch {

...
pub struct WorkspaceInfo {
    ...
    pub transitive_dependencies: Option<HashSet<titanrepo_lockfiles::Package>>,
}
...
impl PackageGraph {
    ...
    pub fn transitive_external_dependencies<'a, I: IntoIterator<Item = &'a WorkspaceName>>(
        &self,
        workspaces: I,
    ) -> HashSet<&titanrepo_lockfiles::Package> {
        workspaces
            .into_iter()
    ...
    pub fn changed_packages(
        ...
        let closures = titanrepo_lockfiles::all_transitive_closures(previous, external_deps)?;
...
mod test {
    ...
    #[derive(Debug)]
    struct MockLockfile {}
    impl titanrepo_lockfiles::Lockfile for MockLockfile {
        fn resolve_package(
            &self,
            _workspace_path: &str,
            name: &str,
            _version: &str,
        ) -> std::result::Result<Option<titanrepo_lockfiles::Package>, titanrepo_lockfiles::Error>
        {
            Ok(match name {
                "a" => Some(titanrepo_lockfiles::Package::new("key:a", "1")),
                "b" => Some(titanrepo_lockfiles::Package::new("key:b", "1")),
                "c" => Some(titanrepo_lockfiles::Package::new("key:c", "1")),
                _ => None,
            })
        }
        ...
        fn all_dependencies(
            &self,
            key: &str,
        ) -> std::result::Result<Option<HashMap<String, String>>, titanrepo_lockfiles::Error>
        {
            match key {
                "key:a" => Ok(Some(
                    [("c", "1")]
        ...
        fn subgraph(
            ...
            _packages: &[String],
        ) -> std::result::Result<Box<dyn Lockfile>, titanrepo_lockfiles::Error> {
            unreachable!("lockfile pruning not necessary for package graph construction")
        }

        fn encode(&self) -> std::result::Result<Vec<u8>, titanrepo_lockfiles::Error> {
    ...
    async fn test_lockfile_traversal() {
        ...
        let bar_deps = pkg_graph
            ...
            .as_ref()
            .unwrap();
        let a = titanrepo_lockfiles::Package::new("key:a", "1");
        let b = titanrepo_lockfiles::Package::new("key:b", "1");
        let c = titanrepo_lockfiles::Package::new("key:c", "1");

...
import {
  addMessageListener as titanSocketAddMessageListener,
  sendMessage as titanSocketSendMessage,
} from "./websocket";
...
export function connect({
  ...
  addMessageListener = titanSocketAddMessageListener,
  // TODO(WEB-1465) Remove this backwards compat fallback once
  // vercel/next.js#54586 is merged.
  sendMessage = titanSocketSendMessage,
}: ClientOptions) {
  addMessageListener((msg) => {
    switch (msg.type) {
      case "titanpack-connected":
  ...
  const queued = globalThis.TITANPACK_CHUNK_UPDATE_LISTENERS;
...
function subscribeToUpdates(
  ...
  sendJSON(sendMessage, {
    type: "titanpack-subscribe",
    ...resource,
  });

  return () => {
    sendJSON(sendMessage, {
      type: "titanpack-unsubscribe",
      ...resource,
    });
  };
}

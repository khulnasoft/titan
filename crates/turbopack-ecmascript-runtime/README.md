# titanpack-ecmascript-runtime

This crate contains Titanpack's ECMAScript runtimes. These include:
...
The runtime-agnostic code (`js/src/shared`) was originally placed in `titanpack-ecmascript`, and the runtime-specific
code (`js/src/{build,dev}`) in `titanpack-{build,dev}`.

...
use titanpack_ecmascript::{CustomTransformer, TransformContext};

/// A wrapper around an SWC's ecma transform wasm plugin module bytes, allowing
/// it to operate with the titan_tasks caching requirements.
...
pub struct SwcPluginModule(
    ...
    // Dummy field to avoid titan_tasks macro complaining about empty struct.
...
impl Issue for UnsupportedSwcEcmaTransformPluginsIssue {
    ...
    fn description(&self) -> Vc<OptionStyledString> {
        Vc::cell(Some(
            StyledString::Text(
                "Titanpack does not yet support running SWC EcmaScript transform plugins on this \
                 platform."
                    .to_string(),
            )
            .cell(),
...
impl CustomTransformer for SwcEcmaTransformPluginsTransformer {
    ...
    async fn transform(&self, program: &mut Program, ctx: &TransformContext<'_>) -> Result<()> {
        ...
        #[cfg(not(feature = "swc_ecma_transform_plugin"))]
        {
            use titanpack_core::issue::IssueExt;

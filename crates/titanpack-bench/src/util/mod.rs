...
pub const BINDING_NAME: &str = "__titanpackBenchBinding";
...
pub async fn create_browser() -> Browser {
    let with_head = read_env_bool("TITANPACK_BENCH_WITH_HEAD");
    let with_devtools = read_env_bool("TITANPACK_BENCH_DEVTOOLS");
...
pub fn resume_on_error<F: FnOnce() + UnwindSafe>(f: F) {
    ...
    let ignore_errors = read_env_bool("TITANPACK_BENCH_IGNORE_ERRORS");
...
impl<'a, 'b, A: AsyncExecutor> AsyncBencherExtension<A> for AsyncBencher<'a, 'b, A, WallTime> {
    ...
    where
        ...
        F: Future<Output = Result<Duration>>,
    {
        let log_progress = read_env_bool("TITANPACK_BENCH_PROGRESS");

...
fn bench_hmr_internal(
    ...
    let runtime = Runtime::new().unwrap();
    let browser = Lazy::new(|| runtime.block_on(create_browser()));
    let hmr_warmup = read_env("TITANPACK_BENCH_HMR_WARMUP", 10).unwrap();
                            ...
                            |mut guard, iters, m, verbose| {
                                ...
                                async move {
                                    ...
                                    while iter < iters {
                                        ...
                                        {
                                            Err(_) => {
                                                // Some bundlers (e.g. Titanpack and Vite) can drop
...
fn insert_code(
    ...
    const PRAGMA_EVAL_START: &str = "/* @titanpack-bench:eval-start */";
    const PRAGMA_EVAL_END: &str = "/* @titanpack-bench:eval-end */";
...
fn bench_startup_cached_internal(
    ...
    if !read_env_bool("TITANPACK_BENCH_CACHED") {
        return;
    }
...
fn get_module_counts() -> Vec<usize> {
    read_env_list("TITANPACK_BENCH_COUNTS", vec![1_000usize]).unwrap()
}

...
use titanpack_core::{asset::AssetContent, source::Source, virtual_source::VirtualSource};
...
/// necessary imports and exports to be processed by [titanpack_ecmascript].
...
pub(crate) async fn instantiating_loader_source(
    ...
    writedoc!(
        code,
        r#"
            const {{ {exports} }} = await __titanpack_wasm__(wasmPath, {imports});
...
pub(crate) async fn compiling_loader_source(
    ...
    let code = formatdoc! {
        r#"
            ...
            const mod = await __titanpack_wasm_module__(wasmPath);

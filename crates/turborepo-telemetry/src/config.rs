...
static ENABLED_ENV_VAR: &str = "TITAN_TELEMETRY_ENABLED";

static DEBUG_ENV_VAR: &str = "TITAN_TELEMETRY_DEBUG";
static DISABLED_ENV_VAR: &str = "TITAN_TELEMETRY_DISABLED";
static DISABLED_MESSAGE_ENV_VAR: &str = "TITAN_TELEMETRY_MESSAGE_DISABLED";
...
impl TelemetryConfig {
    pub fn new() -> Result<TelemetryConfig, ConfigError> {
        ...
        // isolated from other titanrepo config, try to remove the entire config
        ...
        if !self.has_seen_alert() && self.is_enabled() && Self::is_telemetry_warning_enabled() {
            println!(
                ...
                color!(
                    ...
                    GREY,
                    "{}",
                    "Titanrepo now collects completely anonymous telemetry regarding usage."
                ),
                color!(
                    ui,
                    GREY,
                    "{}",
                    "This information is used to shape the Titanrepo roadmap and prioritize \
                     features."
                ),
                color!(
                    ui,
    ...
    pub fn is_enabled(&self) -> bool {
        ...
        let titan_telemetry_disabled = env::var(DISABLED_ENV_VAR).unwrap_or("0".to_string());

        if do_not_track == "1"
            || do_not_track == "true"
            || titan_telemetry_disabled == "1"
            || titan_telemetry_disabled == "true"
        {
            return false;
        }
    ...
    pub fn is_telemetry_warning_enabled() -> bool {
        let titan_telemetry_msg_disabled =
            env::var(DISABLED_MESSAGE_ENV_VAR).unwrap_or("0".to_string());
        titan_telemetry_msg_disabled != "1" && titan_telemetry_msg_disabled != "true"
    }
...
fn get_config_path() -> Result<String, ConfigError> {
    if cfg!(test) {
        ...
        // stored as a sibling to the titan global config
        let config_path = config_dir.join("titanrepo").join("telemetry.json");
        Ok(config_path.to_str().unwrap().to_string())
    }
}

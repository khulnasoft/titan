...
use titanpath::{AbsoluteSystemPathBuf, AnchoredSystemPathBuf, PathError};
...
#[no_mangle]
pub extern "C" fn get_titan_data_dir() -> Buffer {
    ...
    let dirs =
        directories::ProjectDirs::from_path("titanrepo".into()).expect("user has a home dir");

    let dir = dirs.data_dir().to_string_lossy().to_string();
    proto::TitanDataDirResp { dir }.into()
}
...
pub extern "C" fn changed_files(buffer: Buffer) -> Buffer {
    ...
    let response = match titanrepo_scm::git::changed_files(
        req.git_root.into(),
        req.titan_root.into(),
...
pub extern "C" fn previous_content(buffer: Buffer) -> Buffer {
    ...
    let response = match titanrepo_scm::git::previous_content(
        req.git_root.into(),
        &req.from_commit,
        req.file_path,
    ) {
...
pub extern "C" fn recursive_copy(buffer: Buffer) -> Buffer {
    ...
    let response = match titanrepo_fs::recursive_copy(src, dst) {
...
pub extern "C" fn verify_signature(buffer: Buffer) -> Buffer {
    ...
    let authenticator =
        titanrepo_cache::signature_authentication::ArtifactSignatureAuthenticator::new(
            req.team_id,
...
pub extern "C" fn get_package_file_hashes(buffer: Buffer) -> Buffer {
    ...
    let titan_root = match AbsoluteSystemPathBuf::new(req.titan_root) {
        Ok(titan_root) => titan_root,
        Err(err) => {
            let resp = proto::GetPackageFileHashesResponse {
                response: Some(proto::get_package_file_hashes_response::Response::Error(
                    err.to_string(),
    ...
    let hasher = titanrepo_scm::SCM::new(&titan_root);
    let response = match hasher.get_package_file_hashes(&titan_root, &package_path, inputs) {
        Ok(hashes) => {
...
pub extern "C" fn get_hashes_for_files(buffer: Buffer) -> Buffer {
    ...
    match hasher.get_hashes_for_files(&titan_root, &files, allow_missing) {
        Ok(hashes) => {
            let mut to_return = HashMap::new();
            for (filename, hash) in hashes {
                let filename = filename.to_string();
...
pub extern "C" fn get_global_hashable_env_vars(buffer: Buffer) -> Buffer {
    ...
    match titanrepo_env::get_global_hashable_env_vars(

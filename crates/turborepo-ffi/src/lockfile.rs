...
enum Error {
    ...
    #[error(transparent)]
    BerryParse(#[from] titanrepo_lockfiles::BerryError),
...
fn npm_transitive_closure_inner(
    ...
    let dependencies = titanrepo_lockfiles::all_transitive_closures(
        &lockfile,
...
fn berry_transitive_closure_inner(
    ...
    let resolutions =
        resolutions.map(|r| titanrepo_lockfiles::BerryManifest::with_resolutions(r.resolutions));
...
fn yarn_transitive_closure_inner(
    ...
    let lockfile =
        Yarn1Lockfile::from_bytes(contents.as_slice()).map_err(titanrepo_lockfiles::Error::from)?;
    let dependencies = titanrepo_lockfiles::all_transitive_closures(
        &lockfile,
        workspaces.into_iter().map(|(k, v)| (k, v.into())).collect(),
    )?;
...
fn bun_transitive_closure_inner(
    ...
    let lockfile =
        BunLockfile::from_bytes(contents.as_slice()).map_err(titanrepo_lockfiles::Error::from)?;
    let dependencies = titanrepo_lockfiles::all_transitive_closures(
        &lockfile,
        workspaces.into_iter().map(|(k, v)| (k, v.into())).collect(),
    )?;
...
fn subgraph_inner(buf: Buffer) -> Result<Vec<u8>, Error> {
    ...
    let contents = match package_manager {
        proto::PackageManager::Npm => {
            titanrepo_lockfiles::npm_subgraph(&contents, &workspaces, &packages)?
        }
        proto::PackageManager::Berry => titanrepo_lockfiles::berry_subgraph(
            &contents,
            &workspaces,
            &packages,
        ...
        proto::PackageManager::Pnpm => {
            titanrepo_lockfiles::pnpm_subgraph(&contents, &workspaces, &packages)?
        }
        proto::PackageManager::Yarn => titanrepo_lockfiles::yarn_subgraph(&contents, &packages)?,
...
fn global_change_inner(buf: Buffer) -> Result<bool, Error> {
    ...
    match request.package_manager() {
        proto::PackageManager::Npm => Ok(titanrepo_lockfiles::npm_global_change(
            &request.prev_contents,
            &request.curr_contents,
        )?),
        proto::PackageManager::Berry => Ok(titanrepo_lockfiles::berry_global_change(
            &request.prev_contents,
            &request.curr_contents,
        )?),
        proto::PackageManager::Pnpm => Ok(titanrepo_lockfiles::pnpm_global_change(
            &request.prev_contents,
            &request.curr_contents,
        )?),
...
impl From<HashMap<String, HashSet<titanrepo_lockfiles::Package>>> for proto::WorkspaceDependencies {
    fn from(value: HashMap<String, HashSet<titanrepo_lockfiles::Package>>) -> Self {

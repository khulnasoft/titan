...
pub enum SignatureError {
    #[error(
        "signature secret key not found. You must specify a secret key in the \
         TITAN_REMOTE_CACHE_SIGNATURE_KEY environment variable"
    )]
...
impl ArtifactSignatureAuthenticator {
    ...
    fn secret_key(&self) -> Result<Vec<u8>, SignatureError> {
        ...
        Ok(env::var_os("TITAN_REMOTE_CACHE_SIGNATURE_KEY")
...
mod tests {
    ...
    fn test_signature(test_case: TestCase) -> Result<()> {
        env::set_var("TITAN_REMOTE_CACHE_SIGNATURE_KEY", test_case.secret_key);
        ...
        // Change the key
        env::set_var("TITAN_REMOTE_CACHE_SIGNATURE_KEY", "some other key");

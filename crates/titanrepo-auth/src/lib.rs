...
#![deny(clippy::all)]
//! Titanrepo's library for authenticating with the Vercel API.
...
pub const TITANREPO_AUTH_FILE_NAME: &str = "auth.json";
pub const TITANREPO_LEGACY_AUTH_FILE_NAME: &str = "config.json";
pub const TITANREPO_CONFIG_DIR: &str = "titanrepo";
...
mod tests {
    ...
    async fn test_read_or_create_auth_file_existing_auth_file() {
        ...
        let tempdir_path = tempdir.path().join(TITANREPO_AUTH_FILE_NAME);
    ...
    async fn test_read_or_create_auth_file_existing_config_file() {
        ...
        let auth_file_path = tempdir_path.join(TITANREPO_AUTH_FILE_NAME);
        let config_file_path = tempdir_path.join(TITANREPO_LEGACY_AUTH_FILE_NAME);

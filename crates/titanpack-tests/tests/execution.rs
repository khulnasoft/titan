...
use titanpack_node::{debug::should_debug, evaluate::evaluate};
use titanpack_test_utils::jest::JestRunResult;
...
async fn prepare_test(resource: String) -> Result<Vc<PreparedTest>> {
    ...
    let tests_path = project_fs.root().join("crates/titanpack-tests".to_string());
...
async fn run_test(prepared_test: Vc<PreparedTest>) -> Result<Vc<RunTestResult>> {
    ...
    let compile_time_info = CompileTimeInfo::builder(env)
        .defines(
            compile_time_defines!(
                process.titanpack = true,
                process.env.TITANPACK = true,
...
async fn snapshot_issues(
    ...
    titanpack_test_utils::snapshot::snapshot_issues(
        plain_issues,

...
use titan_tasks::{debug::ValueDebugFormat, trace::TraceRawVcs, Vc};
use titan_tasks_fs::{File, FileContent, FileSystemPath};
use titanpack_core::{
...
fn load_image_internal(
    ...
    // [NOTE]
    // Workaround for missing codec supports in Titanpack,
    ...
    if matches!(format, Some(ImageFormat::Avif)) {
        ImageProcessingIssue {
            ...
            message: StyledString::Text(
                "This version of Titanpack does not support AVIF images, will emit without \
    ...
    if matches!(format, Some(ImageFormat::WebP)) {
        ImageProcessingIssue {
            path: ident.path(),
            message: StyledString::Text(
                "This version of Titanpack does not support WEBP images, will emit without \
                 optimization or encoding"
                    .to_string(),
            )
            .cell(),

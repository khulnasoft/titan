...
use titanrepo_api_client::Error as APIError;
...
pub enum Error {
    ...
    FailedToFetchUser(#[source] titanrepo_api_client::Error),
    #[error("failed to fetch token metadata: {source}")]
    FailedToFetchTokenMetadata { source: titanrepo_api_client::Error },
    #[error(
    ...
    FailedToValidateSSOToken(titanrepo_api_client::Error),

    // File write errors
    #[error("failed to write to auth file at {auth_path}: {error}")]
    FailedToWriteAuth {
        auth_path: titanpath::AbsoluteSystemPathBuf,
        error: io::Error,
    },

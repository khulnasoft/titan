...
impl From<MockApiError> for titanrepo_api_client::Error {
    fn from(error: MockApiError) -> Self {
        match error {
            MockApiError::EmptyToken => titanrepo_api_client::Error::UnknownStatus {
...
impl Client for MockApiClient {
    ...
    async fn get_user(&self, token: &str) -> titanrepo_api_client::Result<UserResponse> {
        if token.is_empty() {
    ...
    async fn get_teams(&self, token: &str) -> titanrepo_api_client::Result<TeamsResponse> {
        if token.is_empty() {
            return Err(MockApiError::EmptyToken.into());
        }
    ...
    async fn get_team(
        ...
        _token: &str,
        _team_id: &str,
    ) -> titanrepo_api_client::Result<Option<Team>> {
    ...
    ) -> titanrepo_api_client::Result<CachingStatusResponse> {
    ...
    async fn get_spaces(
        ...
        token: &str,
        _team_id: Option<&str>,
    ) -> titanrepo_api_client::Result<SpacesResponse> {
        if token.is_empty() {
            return Err(MockApiError::EmptyToken.into());
        }
        Ok(SpacesResponse {
    ...
    async fn verify_sso_token(
        ...
        token: &str,
        _: &str,
    ) -> titanrepo_api_client::Result<VerifiedSsoUser> {
        Ok(VerifiedSsoUser {
    ...
    async fn get_token_metadata(
        &self,
        _token: &str,
    ) -> titanrepo_api_client::Result<TokenMetadata> {
    ...
    async fn put_artifact(
        ...
        _team_id: Option<&str>,
        _team_slug: Option<&str>,
    ) -> titanrepo_api_client::Result<()> {
        unimplemented!("put_artifact")
    }
    async fn handle_403(_response: Response) -> titanrepo_api_client::Error {
    ...
    async fn fetch_artifact(
        ...
        _team_id: Option<&str>,
        _team_slug: Option<&str>,
    ) -> titanrepo_api_client::Result<Option<Response>> {
    ...
    ) -> titanrepo_api_client::Result<PreflightResponse> {

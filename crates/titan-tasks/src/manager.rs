...
pub trait TitanTasksCallApi: Sync + Send {
...
pub trait TitanTasksApi: TitanTasksCallApi + Sync + Send {
    fn pin(&self) -> Arc<dyn TitanTasksApi>;
...
pub enum StatsType {
    /// Only report stats essential to Titan Tasks' operation.
    Essential,
...
pub trait TitanTasksBackendApi<B: Backend + 'static>:
    TaskIdProvider + TitanTasksCallApi + Sync + Send
{
    fn pin(&self) -> Arc<dyn TitanTasksBackendApi<B>>;
...
impl<B: Backend + 'static> TaskIdProvider for &dyn TitanTasksBackendApi<B> {
    fn get_fresh_task_id(&self) -> Unused<TaskId> {
...
pub struct TitanTasks<B: Backend + 'static> {
    this: Weak<Self>,
    backend: B,
...
task_local! {
    /// The current TitanTasks instance
    static TITAN_TASKS: Arc<dyn TitanTasksApi>;
...
impl<B: Backend + 'static> TitanTasks<B> {
    // TODO better lifetime management for titan tasks
    // consider using unsafe for the task_local titan tasks
    // that should be safe as long tasks can't outlife titan task
    // so we probably want to make sure that all tasks are joined
    // when trying to drop titan tasks
    pub fn new(mut backend: B) -> Arc<Self> {
        ...
        let this = Arc::new_cyclic(|this| Self {
            ...
            event: Event::new(|| "TitanTasks::event".to_string()),
            event_start: Event::new(|| "TitanTasks::event_start".to_string()),
            event_foreground: Event::new(|| "TitanTasks::event_foreground".to_string()),
            event_background: Event::new(|| "TitanTasks::event_background".to_string()),
    ...
    pub async fn run_once<T: TraceRawVcs + Send + 'static>(
        ...
        raw_result
            .into_read_untracked_with_titan_tasks::<Completion>(self)
            .await?;
    ...
    pub(crate) fn native_call(&self, func: FunctionId, inputs: Vec<ConcreteTaskInput>) -> RawVc {
        RawVc::TaskOutput(self.backend.get_or_create_persistent_task(
            ...
            current_task("titan_function calls"),
            self,
        ))
    }
    ...
    pub(crate) fn schedule_background_job<
        T: FnOnce(Arc<TitanTasks<B>>) -> F + Send + 'static,
...
impl<B: Backend + 'static> TitanTasksCallApi for TitanTasks<B> {
    fn dynamic_call(&self, func: FunctionId, inputs: Vec<ConcreteTaskInput>) -> RawVc {
...
impl<B: Backend + 'static> TitanTasksApi for TitanTasks<B> {
    fn pin(&self) -> Arc<dyn TitanTasksApi> {
        self.pin()
    }
...
impl<B: Backend + 'static> TitanTasksBackendApi<B> for TitanTasks<B> {
    fn pin(&self) -> Arc<dyn TitanTasksBackendApi<B>> {
        self.pin()
    }
    fn backend(&self) -> &B {
...
impl<B: Backend + 'static> TaskIdProvider for TitanTasks<B> {
    fn get_fresh_task_id(&self) -> Unused<TaskId> {
        // Safety: This is a fresh id from the factory
...
pub(crate) fn current_task(from: &str) -> TaskId {
    match CURRENT_TASK_ID.try_with(|id| *id) {
        Ok(id) => id,
        Err(_) => panic!(
            "{} can only be used in the context of titan_tasks task execution",
            from
        ),
    }
}
...
pub struct Invalidator {
    task: TaskId,
    titan_tasks: Weak<dyn TitanTasksApi>,
    handle: Handle,
}
...
impl Invalidator {
    pub fn invalidate(self) {
        ...
        let _ = handle.enter();
        if let Some(titan_tasks) = titan_tasks.upgrade() {
            titan_tasks.invalidate(task);
        }
    }
    ...
    pub fn invalidate_with_reason<T: InvalidationReason>(self, reason: T) {
        ...
        let _ = handle.enter();
        if let Some(titan_tasks) = titan_tasks.upgrade() {
            titan_tasks.invalidate_with_reason(
                task,
...
pub async fn run_once<T: Send + 'static>(
    ...
    raw_result
        .into_read_untracked_with_titan_tasks::<Completion>(&*tt)
        .await?;
...
/// see [TitanTasks] `dynamic_call`
pub fn dynamic_call(func: FunctionId, inputs: Vec<ConcreteTaskInput>) -> RawVc {
    with_titan_tasks(|tt| tt.dynamic_call(func, inputs))
}

/// see [TitanTasks] `trait_call`
pub fn trait_call(
    trait_type: TraitTypeId,
    trait_fn_name: Cow<'static, str>,
    inputs: Vec<ConcreteTaskInput>,
) -> RawVc {
    with_titan_tasks(|tt| tt.trait_call(trait_type, trait_fn_name, inputs))
}

pub fn titan_tasks() -> Arc<dyn TitanTasksApi> {
    TITAN_TASKS.with(|arc| arc.clone())
}

pub fn with_titan_tasks<T>(func: impl FnOnce(&Arc<dyn TitanTasksApi>) -> T) -> T {
    TITAN_TASKS.with(|arc| func(arc))
}

pub fn weak_titan_tasks() -> Weak<dyn TitanTasksApi> {
    TITAN_TASKS.with(Arc::downgrade)
}

pub fn with_titan_tasks_for_testing<T>(
    tt: Arc<dyn TitanTasksApi>,
    current_task: TaskId,
...
pub fn spawn_detached(f: impl Future<Output = Result<()>> + Send + 'static) {
    tokio::spawn(titan_tasks().detached(Box::pin(f.in_current_span())));
}
...
pub fn get_invalidator() -> Invalidator {
    ...
    Invalidator {
        task: current_task("titan_tasks::get_invalidator()"),
        titan_tasks: weak_titan_tasks(),
        handle,
    }
}
...
pub fn mark_finished() {
    with_titan_tasks(|tt| {
        tt.mark_own_task_as_finished(current_task("titan_tasks::mark_finished()"))
    });
}
...
pub fn notify_scheduled_tasks() {
    with_titan_tasks(|tt| tt.notify_scheduled_tasks())
}

pub fn emit<T: VcValueTrait + Send>(collectible: Vc<T>) {
    with_titan_tasks(|tt| tt.emit_collectible(T::get_trait_type_id(), collectible.node))
}
...
pub fn find_cell_by_type(ty: ValueTypeId) -> CurrentCellRef {
    CELL_COUNTERS.with(|cell| {
        let current_task = current_task("celling titan_tasks values");

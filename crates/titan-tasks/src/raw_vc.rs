...
use crate::{
    backend::CellContent,
    event::EventListener,
    manager::{read_task_cell, read_task_output, TitanTasksApi},
    registry::{
        get_value_type, {self},
    },
    titan_tasks,
...
impl RawVc {
    ...
    pub(crate) fn into_read_untracked_with_titan_tasks<T: Any + VcValueType>(
        self,
        titan_tasks: &dyn TitanTasksApi,
    ) -> ReadRawVcFuture<T, VcValueTypeCast<T>> {
        ReadRawVcFuture::new_untracked_with_titan_tasks(self, titan_tasks)
    }
...
pub struct ReadRawVcFuture<T: ?Sized, Cast = VcValueTypeCast<T>> {
    titan_tasks: Arc<dyn TitanTasksApi>,
...
impl<T: ?Sized, Cast: VcCast> ReadRawVcFuture<T, Cast> {
    ...
    fn new_untracked_with_titan_tasks(vc: RawVc, titan_tasks: &dyn TitanTasksApi) -> Self {
        let tt = titan_tasks.pin();
        ReadRawVcFuture {
            titan_tasks: tt,
            strongly_consistent: false,
            current: vc,
...
impl<T: ?Sized, Cast: VcCast> Future for ReadRawVcFuture<T, Cast> {
    ...
    fn poll(self: Pin<&mut Self>, cx: &mut std::task::Context<'_>) -> Poll<Self::Output> {
        self.titan_tasks.notify_scheduled_tasks();
        ...
        'outer: loop {
            ...
            let mut listener = match this.current {
                ...
                RawVc::TaskCell(task, index) => {
                    let read_result = if this.untracked {
                        this.titan_tasks.try_read_task_cell_untracked(task, index)
                    } else {
                        this.titan_tasks.try_read_task_cell(task, index)
                    };
                    match read_result {
                        Ok(Ok(content)) => {

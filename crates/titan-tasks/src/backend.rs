...
use crate::{
    event::EventListener, manager::TitanTasksBackendApi, raw_vc::CellId, registry,
...
pub trait Backend: Sync + Send {
    ...
    #[allow(unused_variables)]
    fn startup(&self, titan_tasks: &dyn TitanTasksBackendApi<Self>) {}

    #[allow(unused_variables)]
    fn stop(&self, titan_tasks: &dyn TitanTasksBackendApi<Self>) {}

    #[allow(unused_variables)]
    fn idle_start(&self, titan_tasks: &dyn TitanTasksBackendApi<Self>) {}

    fn invalidate_task(&self, task: TaskId, titan_tasks: &dyn TitanTasksBackendApi<Self>);

    fn invalidate_tasks(&self, tasks: &[TaskId], titan_tasks: &dyn TitanTasksBackendApi<Self>);
    fn invalidate_tasks_set(&self, tasks: &TaskIdSet, titan_tasks: &dyn TitanTasksBackendApi<Self>);
    ...
    fn try_start_task_execution(
        &self,
        task: TaskId,
        titan_tasks: &dyn TitanTasksBackendApi<Self>,
    ...
    fn run_backend_job<'a>(
        &'a self,
        id: BackendJobId,
        titan_tasks: &'a dyn TitanTasksBackendApi<Self>,
    ...
    fn try_read_own_task_cell_untracked(
        ...
        match self.try_read_task_cell_untracked(current_task, index, titan_tasks)? {
    ...
    fn mark_own_task_as_finished(
        &self,
        _task: TaskId,
        _titan_tasks: &dyn TitanTasksBackendApi<Self>,
    ) {
...
impl PersistentTaskType {
    pub async fn run_resolve_native<B: Backend + 'static>(
        ...
        titan_tasks: Arc<dyn TitanTasksBackendApi<B>>,
    ) -> Result<RawVc> {
        let span = tracing::trace_span!(
            "titan_tasks::resolve_call",
        ...
        async move {
            ...
            Ok(titan_tasks.native_call(fn_id, inputs))
        }
        .instrument(span)
        .await
    }
    ...
    pub async fn run_resolve_trait<B: Backend + 'static>(
        ...
        let span = tracing::trace_span!(
            "titan_tasks::resolve_trait_call",
            ...
            if let Some(this) = iter.next() {
                ...
                match this_value.get_trait_method(trait_type, name) {
                    Ok(native_fn) => {
                        ...
                        Ok(titan_tasks.dynamic_call(native_fn, resolved_inputs))
                    }
                    Err(name) => {
    ...
    pub fn run<B: Backend + 'static>(
        ...
        match self {
            ...
            PersistentTaskType::ResolveNative(fn_id, inputs) => {
                Box::pin(Self::run_resolve_native(fn_id, inputs, titan_tasks))
            }
            PersistentTaskType::ResolveTrait(trait_type, name, inputs) => Box::pin(
                Self::run_resolve_trait(trait_type, name, inputs, titan_tasks),
            ),
        }
    }
}

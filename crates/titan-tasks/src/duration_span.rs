...
macro_rules! duration_span {
    ($name:literal) => {
        titan_tasks::duration_span::DurationSpanGuard::new(|duration| {
            titan_tasks::macro_helpers::tracing::info!(name = $name, duration = duration);
        })
    };
    ($name:literal, $($arg:tt)+) => {
        titan_tasks::duration_span::DurationSpanGuard::new(|duration| {
            titan_tasks::macro_helpers::tracing::info!(name = $name, $($arg)+, duration = duration);
        })
    };
}

...
import { TITAN_BIN, type TTFTData } from "../helpers";
import { run } from "./run";
...
// process.cwd() should be packages/titan-benchmark/ here.
...
if (!fs.existsSync(TITAN_BIN)) {
  throw new Error("No titan binary found");
}

cp.execSync(`${TITAN_BIN} --version`, { stdio: "inherit" });
...
for (const item of profileJSON) {
  ...
  if (args.titan_version) {
    ttftData.titanVersion = `${args.titan_version}`;
  }
...
function printUsageMessage() {
  console.log("Usage:\n\npnpm -F @titan/benchmark ttft <path>");
}

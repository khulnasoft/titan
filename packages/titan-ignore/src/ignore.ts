...
export function titanIgnore(
  workspaceArg: TitanIgnoreArg,
  opts: TitanIgnoreOptions
) {
  const inputs = {
  ...
  if (process.env.TITAN_FORCE === "true") {
    info("`TITAN_FORCE` detected");
    return continueBuild();
  }

  // find the monorepo root
  const root = getTitanRoot(inputs.directory);
  if (!root) {
    error("Monorepo root not found. titan-ignore inferencing failed");
    return continueBuild();
  }
  ...
  const command = `npx titan run ${task} --filter="${workspace}...[${comparison.ref}]" --dry=json`;

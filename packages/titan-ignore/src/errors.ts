...
export const NON_FATAL_ERRORS: NonFatalErrors = {
  MISSING_LOCKFILE: {
    ...
    message: `titan-ignore could not complete - no lockfile found, please commit one to your repository`,
  },
  NO_PACKAGE_MANAGER: {
    regex: [
      /run failed: We did not detect an in-use package manager for your project/i,
    ],
    message: `titan-ignore could not complete - no package manager detected, please commit a lockfile, or set "packageManager" in your root "package.json"`,
  },
  UNREACHABLE_PARENT: {
    regex: [/failed to resolve packages to run: commit HEAD\^ does not exist/i],
    message: `titan-ignore could not complete - parent commit does not exist or is unreachable`,
  },
  INVALID_COMPARISON: {
    regex: [
    ...
    message: `titan-ignore could not complete - a ref or SHA is invalid. It could have been removed from the branch history via a force push, or this could be a shallow clone with insufficient history`,
  },
};

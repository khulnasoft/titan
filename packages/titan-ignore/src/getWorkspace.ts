...
import type { TitanIgnoreOptions } from "./types";

export function getWorkspace(args: TitanIgnoreOptions): string | null {
  ...
  try {
    ...
    error(
      `"${packageJsonPath}" could not be found. titan-ignore inferencing failed`
    );
    return null;
  }
}

import { spyConsole, validateLogs } from "@titan/test-utils";
...
describe("getWorkspace()", () => {
  ...
  it("getWorkspace used current directory if not specified", async () => {
    expect(getWorkspace({})).toEqual("titan-ignore");
    expect(mockConsole.log).toHaveBeenCalledWith(
      "≫  ",
      'Inferred "titan-ignore" as workspace from "package.json"'
    );
  });
  ...
  it("getWorkspace returns null when no arg is provided and package.json can be found", async () => {
    ...
    expect(mockConsole.error).toHaveBeenCalledWith(
      "≫  ",
      '"__fixtures__/no-app/package.json" could not be found. titan-ignore inferencing failed'
    );
  });
});

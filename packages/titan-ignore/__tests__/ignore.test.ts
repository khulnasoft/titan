...
import {
  ...
  mockEnv,
  validateLogs,
} from "@titan/test-utils";
import { titanIgnore } from "../src/ignore";
...
describe("titanIgnore()", () => {
  ...
  it("throws error and allows build when exec fails", () => {
    ...
    titanIgnore("test-workspace", {});

    expect(mockExec).toHaveBeenCalledWith(
      `npx titan run build --filter="test-workspace...[HEAD^]" --dry=json`,
      expect.anything(),
      expect.anything()
    );
  ...
  it("throws pretty error and allows build when exec fails", () => {
    ...
    validateLogs(
      [
        `titan-ignore could not complete - no package manager detected, please commit a lockfile, or set "packageManager" in your root "package.json"`,
      ],
      mockConsole.warn,
      { prefix: "≫  " }
    );
  ...
  it("throws pretty error and allows build when can't find previous sha", () => {
    ...
    expect(mockExec).toHaveBeenCalledWith(
      `npx titan run build --filter="test-workspace...[too-far-back]" --dry=json`,
      expect.anything(),
      expect.anything()
    );
    ...
    validateLogs(
      [
        `titan-ignore could not complete - a ref or SHA is invalid. It could have been removed from the branch history via a force push, or this could be a shallow clone with insufficient history`,
      ],
      mockConsole.warn,
      { prefix: "≫  " }
    );
  ...
  it("throws pretty error and allows build when fallback fails", () => {
    ...
    titanIgnore("test-workspace", { fallback: "HEAD^" });
    ...
    validateLogs(
      [
        `titan-ignore could not complete - parent commit does not exist or is unreachable`,
      ],
      mockConsole.warn,
      { prefix: "≫  " }
    );
  ...
  it("skips checks and allows build when no workspace can be found", () => {
    titanIgnore(undefined, { directory: "__fixtures__/no-app" });
    validateLogs(
      [
        () => [
          "≫  ",
          expect.stringContaining(
            " could not be found. titan-ignore inferencing failed"
          ),
        ],
      ],
      mockConsole.error,
  ...
  it("skips checks and allows build when a workspace with no name is found", () => {
    titanIgnore(undefined, { directory: "__fixtures__/invalid-app" });
    validateLogs(
      [
        () => [
          "≫  ",
  ...
  it("skips checks and allows build when no monorepo root can be found", () => {
    titanIgnore(undefined, { directory: "/" });
    expectBuild(mockExit);
    expect(mockConsole.error).toHaveBeenLastCalledWith(
      "≫  ",
      "Monorepo root not found. titan-ignore inferencing failed"
    );
  });

  it("skips checks and allows build when TITAN_FORCE is set", () => {
    process.env.TITAN_FORCE = "true";
    titanIgnore(undefined, { directory: "test-workspace" });
    expect(mockConsole.log).toHaveBeenNthCalledWith(
      2,
      "≫  ",
      "`TITAN_FORCE` detected"
    );
    expectBuild(mockExit);
  });
  ...
  it("allows build when no comparison is returned", () => {
    ...
    process.env.VERCEL_GIT_COMMIT_REF = "my-branch";
    titanIgnore("test-app", { directory: "__fixtures__/app" });
    expect(mockConsole.log).toHaveBeenNthCalledWith(
      4,
      "≫  ",
  ...
  it("skips build for `previousDeploy` comparison with no changes", () => {
    ...
    titanIgnore(undefined, { directory: "__fixtures__/app" });
    validateLogs(
      [
        ...
        'Analyzing results of `npx titan run build --filter="test-app...[last-deployed-sha]" --dry=json`',
  ...
  it("allows build for `previousDeploy` comparison with changes", () => {
    ...
    validateLogs(
      [
        ...
        'Found previous deployment ("last-deployed-sha") for "test-app" on branch "my-branch"',
        'Analyzing results of `npx titan run "workspace#build" --filter="test-app...[last-deployed-sha]" --dry=json`',
  ...
  it("allows build for unavailable `previousDeploy` comparison with fallback", () => {
    ...
    validateLogs(
      [
        ...
        'Analyzing results of `npx titan run "workspace#build" --filter="test-app...[HEAD^2]" --dry=json`',
        'This commit affects "test-app"',
        () => expect.stringContaining("✓ Proceeding with deployment"),
      ],
      mockConsole.log,
  ...
  it("throws error and allows build when json cannot be parsed", () => {
    ...
    expect(mockExec).toHaveBeenCalledWith(
      `npx titan run build --filter="test-app...[HEAD^]" --dry=json`,
      expect.anything(),
      expect.anything()
    );
    validateLogs(
      [
        'Failed to parse JSON output from `npx titan run build --filter="test-app...[HEAD^]" --dry=json`.',
      ],
      mockConsole.error,
      { prefix: "≫  " }
    );
  ...
  it("runs full titan-ignore check when commit message contains a conflicting string", () => {
    process.env.VERCEL = "1";
  ...
  it("passes max buffer to titan exectuion", () => {
    const mockExec = jest
      .spyOn(child_process, "exec")
      .mockImplementation((command, options, callback) => {
        if (callback) {
    ...
    titanIgnore(undefined, { directory: "__fixtures__/app", maxBuffer: 1024 });

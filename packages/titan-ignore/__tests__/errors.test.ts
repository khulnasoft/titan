...
describe("shouldWarn()", () => {
  it("it detects errors when packageManager is missing", async () => {
    const result = shouldWarn({
      err: `run failed: We did not detect an in-use package manager for your project. Please set the "packageManager" property in your root package.json (https://nodejs.org/api/packages.html#packagemanager) or run \`npx @titan/codemod add-package-manager\` in the root of your monorepo.`,
    });

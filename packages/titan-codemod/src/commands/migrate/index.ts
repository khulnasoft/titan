...
import { getWorkspaceDetails, type Project } from "@titan/workspaces";
...
import { getTitanUpgradeCommand } from "./steps/getTitanUpgradeCommand";
...
Migration is done in 4 steps:
  -- gather information
  1. find the version (x) of titan to migrate from (if not specified)
  2. find the version (y) of titan to migrate to (if not specified)
  ...
  5. update the titan version (optionally)
**/
export async function migrate(
  ...
  if (!fromVersion) {
    return endMigration({
      success: false,
      message: `Unable to infer the version of titan being used by ${project.name}`,
    });
  }
  ...
  if (!toVersion) {
    return endMigration({
      success: false,
      message: `Unable to fetch the latest version of titan`,
    });
  }
  ...
  // shutdown the titan daemon before running codemods and upgrading
  ...
  // step 4
  logger.log(
    `Upgrading titan from ${chalk.bold(fromVersion)} to ${chalk.bold(
      toVersion
    )} (${
      codemods.length === 0
  ...
  if (options.install) {
    if (options.dry) {
      logger.log(
        `Upgrading titan with ${chalk.bold(upgradeCommand)} ${chalk.dim(
          "(dry run)"
        )}`,
        os.EOL
      );
    } else {
      logger.log(`Upgrading titan with ${chalk.bold(upgradeCommand)}`, os.EOL);
      try {
        execSync(upgradeCommand, { stdio: "pipe", cwd: project.paths.root });
      } catch (err: unknown) {
        return endMigration({
          success: false,
          message: `Unable to upgrade titan: ${String(err)}`,
        });
      }
    }
  } else {
    logger.log(`Upgrade titan with ${chalk.bold(upgradeCommand)}`, os.EOL);
  }

...
const { titanGradient, titanLoader, info, error, warn } = logger;
...
function handleErrors(err: unknown) {
  ...
  if (err instanceof TransformError) {
    ...
    // handle errors from @titan/workspaces
  } else if (err instanceof ConvertError && err.type !== "unknown") {
    error(chalk.red(err.message));
    process.exit(1);
    // handle download errors from @titan/utils
...
export async function create(
  ...
  logger.log(chalk.bold(titanGradient(`\n>>> TITANREPO\n`)));
  info(`Welcome to Titanrepo! Let's get you set up with a new codebase.`);
  logger.log();
  ...
  tryGitInit(root, `feat(create-titan): create ${exampleName}`);
    ...
    for (const transform of transforms) {
      try {
        ...
        if (transformResult.result === "success") {
          ...
          tryGitAdd();
          tryGitCommit(
            `feat(create-titan): apply ${transformResult.name} transform`
          );
        }
      } catch (err) {
        handleErrors(err);
  ...
  info("Created a new Titanrepo with the following:");
  logger.log();
  ...
  if (hasPackageJson && !skipInstall) {
    ...
    if (
      ...
      const loader = titanLoader("Installing dependencies...").start();
      await install({
        project,
      ...
      tryGitCommit("feat(create-titan): install dependencies");
      loader.stop();
    }
  }
  ...
  if (projectDirIsCurrentDir) {
    logger.log(
      `${chalk.bold(
        titanGradient(">>> Success!")
      )} Your new Titanrepo is ready.`
    );
  } else {
    logger.log(
      `${chalk.bold(
        titanGradient(">>> Success!")
      )} Created a new Titanrepo at "${relativeProjectDir}".`
    );
  }
  ...
  if (packageManagerMeta && hasPackageJson) {
    ...
    logger.log(`Titanrepo will cache locally by default. For an additional`);
    logger.log(`speed boost, enable Remote Caching with Vercel by`);
    logger.log(`entering the following command:`);
    logger.log();
    logger.log(chalk.cyan(`  ${packageManagerMeta.executable} titan login`));
    logger.log();

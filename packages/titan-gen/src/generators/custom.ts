...
export async function generate({
  ...
  if (!generators.length) {
    ...
    if (answer) {
      ...
      logger.log();
      logger.dimmed(
        "Learn more about custom Titanrepo generators - https://turbo.build/repo/docs/core-concepts/monorepos/code-generation#custom-generators"
      );
      return;
    }
  }
  ...
  try {
    ...
    if (isOnboarding) {
      logger.log();
      logger.info(`Congrats! You just ran your first Titanrepo generator`);
      logger.dimmed(
        "Learn more about custom Titanrepo generators - https://turbo.build/repo/docs/core-concepts/monorepos/code-generation#custom-generators"
      );
    }
  ...
  logger.log();
  logger.bold(logger.titanGradient(">>> Success!"));
}

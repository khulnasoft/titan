import { convertCase, logger } from "@titan/utils";
import { workspace, type TitanGeneratorCLIOptions } from "../workspace";
 ...
 *
 * Used by the titan Rust cli to handoff commands to the \@titan/gen binary
 */
export async function raw(command: string, options: { json: string }) {
  if (command === "workspace") {
    ...
    await workspace(parsedArgs as TitanGeneratorCLIOptions);
  } else if (command === "run") {

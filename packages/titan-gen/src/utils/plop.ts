...
import { searchUp, getTitanConfigs, logger } from "@titan/utils";
import { GeneratorError } from "./error";

const SUPPORTED_CONFIG_EXTENSIONS = ["ts", "js", "cjs"];
const TITAN_GENERATOR_DIRECTORY = path.join("titan", "generators");
...
const SUPPORTED_WORKSPACE_GENERATOR_CONFIGS = SUPPORTED_CONFIG_EXTENSIONS.map(
  (ext) => path.join(TITAN_GENERATOR_DIRECTORY, `config.${ext}`)
);
...
function injectTitanrepoData({
  ...
  let titanConfigs = {};
  try {
    titanConfigs = getTitanConfigs(generator.basePath);
  } catch (e) {
    // ignore
  }
...
export async function runCustomGenerator({
  ...
  const results = await gen.runActions(
    { ...answers, ...injectTitanrepoData({ project, generator: gen }) },
    {

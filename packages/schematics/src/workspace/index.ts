...
import { toFileName } from '@khulnasoft/schematics';
...
function updatePackageJson() {
  return (host: Tree) => {
    ...
    if (!packageJson.dependencies['@khulnasoft/titan']) {
      packageJson.dependencies['@khulnasoft/titan'] = titanVersion;
    }
    ...
    if (!packageJson.devDependencies['@khulnasoft/schematics']) {
      packageJson.devDependencies['@khulnasoft/schematics'] = schematicsVersion;
    }
...
function updateAngularCLIJson(options: Schema) {
  return (host: Tree) => {
    ...
    angularCliJson.defaults.schematics['collection'] = '@khulnasoft/schematics';
...
function updateTsLintJson(options: Schema) {
  return (host: Tree) => {
    updateJsonFile('tslint.json', json => {
      ...
      json['titan-enforce-module-boundaries'] = [true, { npmScope: npmScope(options), lazyLoad: [] }];
    });
    return host;
  };
}

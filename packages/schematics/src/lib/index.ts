...
import { addImportToModule, insert, names, toClassName, toFileName, toPropertyName } from '@khulnasoft/schematics';
...
function updateTsLint(schema: Schema): Rule {
  return (host: Tree) => {
    ...
    if (
      tsLint['rules'] &&
      tsLint['rules']['titan-enforce-module-boundaries'] &&
      tsLint['rules']['titan-enforce-module-boundaries'][1] &&
      tsLint['rules']['titan-enforce-module-boundaries'][1]['lazyLoad']
    ) {
      tsLint['rules']['titan-enforce-module-boundaries'][1]['lazyLoad'].push(toFileName(schema.name));

...
const meta: Rule.RuleMetaData = {
  ...
  docs: {
    description:
      "Do not allow the use of `process.env` without including the env key in any titan.json",
    category: "Configuration Issues",
    recommended: true,
    url: `https://github.com/khulnasoft/titan/tree/main/packages/eslint-plugin-titan/docs/rules/${RULES.noUndeclaredEnvVars}.md`,
  },
  schema: [
    {
      type: "object",
...
function create(context: RuleContextWithOptions): Rule.RuleListener {
  ...
  const hasWorkspaceConfigs = project.projectWorkspaces.some(
    (workspaceConfig) => Boolean(workspaceConfig.titanConfig)
  );
  ...
  const checkKey = (node: Node, envKey?: string) => {
    ...
    let message = `{{ envKey }} is not listed as a dependency in ${
      hasWorkspaceConfigs ? "root titan.json" : "titan.json"
    }`;
    if (workspaceConfig?.titanConfig) {
      if (cwd) {
        // if we have a cwd, we can provide a relative path to the workspace config
        message = `{{ envKey }} is not listed as a dependency in the root titan.json or workspace (${path.relative(
          cwd,
          workspaceConfig.workspacePath
        )}) titan.json`;
      } else {
        message = `{{ envKey }} is not listed as a dependency in the root titan.json or workspace titan.json`;
      }
    }

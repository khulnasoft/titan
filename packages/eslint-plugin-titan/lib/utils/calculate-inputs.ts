...
import type { WorkspaceConfig } from "@titan/utils";
import { getWorkspaceConfigs } from "@titan/utils";
import type { Pipeline } from "@titan/types";
...
function processGlobal(
  workspacePath: string,
  rootTitanJson: RootSchema
): EnvironmentConfig {
  return {
    legacyConfig: processLegacyConfig(rootTitanJson.globalDependencies),
    env: processEnv(rootTitanJson.globalEnv),
    passThroughEnv: processPassThroughEnv(rootTitanJson.globalPassThroughEnv),
    dotEnv: processDotEnv(workspacePath, rootTitanJson.globalDotEnv),
  };
}
...
function getTaskAddress(taskName: string): {
  ...
  if (taskName.length === 0) {
    throw new Error("Invalid task name found in titan.json.");
  }
...
export class Project {
  ...
  generateKey(): ProjectKey {
    ...
    if (this.projectRoot?.titanConfig && !("extends" in this.projectRoot)) {
      const rootTitanJson = this.projectRoot;

      global = processGlobal(
        this.projectRoot.workspacePath,
        this.projectRoot.titanConfig
      );

      Object.entries(this.projectRoot.titanConfig.pipeline).forEach(
    ...
    this.projectWorkspaces.forEach((projectWorkspace) => {
      if (!projectWorkspace.titanConfig) {
        return;
      }

      Object.entries(projectWorkspace.titanConfig.pipeline).forEach(
        ([taskName, taskDefinition]) => {
          const { workspaceName: erroneousWorkspaceName, scriptName } =
            getTaskAddress(taskName);
          if (erroneousWorkspaceName) {
            throw new Error(
              "May not specify workspace name in non-root titan.json"
            );
          }

...
export interface BaseSchema {
  ...
  pipeline: {
    /**
     * The name of a task that can be executed by titan. If titan finds a workspace
...
export interface WorkspaceSchema extends BaseSchema {
   ...
   * and cannot be used in your root titan.json.
   *
   * Tells titan to extend your root `titan.json`
...
export interface Pipeline {
   ...
   *
   * Prefixing an item in dependsOn with a ^ prefix tells titan that this task depends
   ...
   *
   * Note: If you are migrating from a titan version 1.5 or below,
   ...
   *
   * Titanrepo captures task logs for all tasks. This enables us to cache tasks whose runs

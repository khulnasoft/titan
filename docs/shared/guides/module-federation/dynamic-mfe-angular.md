...
```shell
# Npm
npx nx g @titan/angular:host dashboard
```

```shell
# Yarn
yarn nx g @titan/angular:host dashboard
```
...
```shell
# Npm
npx nx g @titan/angular:remote login --host=dashboard
```

```shell
# Yarn
yarn nx g @titan/angular:remote login --host=dashboard
```
...
- Updated the `build` target in the `project.json` to use the `@titan/angular:webpack-browser` executor _(this is required as it supports passing a custom Webpack configuration to the Angular compiler)_
- Updated the `serve` target to use `@titan/angular:dev-server` _(this is required as we first need Webpack to build the application with our custom Webpack configuration)_
...
```js
const { withModuleFederation } = require('@titan/angular/module-federation');
...
```shell
nx g @titan/angular:lib shared/data-access-user
```
...
```shell
nx g @titan/angular:service user --project=shared-data-access-user
```
...
import { setRemoteDefinitions } from '@titan/angular/mf';
...
```typescript
import { loadRemoteModule } from '@titan/angular/mf';
```
...
```shell
nx g @titan/angular:host employee --remotes=login --dynamic
```
...
```shell
nx g @titan/angular:remote todo --host=employee
```
